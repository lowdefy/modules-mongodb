...
Model it on `modules/reporting/api/query-data.yaml` — same signed-in guard, same
...
1. **Create `modules/reporting/api/chart-data.yaml`.** Structure (follow `query-data.yaml`'s
...
2. **Register it.** In `modules/reporting/module.lowdefy.yaml`:
...
- `modules/reporting/api/chart-data.yaml` — create.
- `modules/reporting/module.lowdefy.yaml` — modify — `api:` ref, `exports.api` entry, version bump.
