...
Options precedence becomes: declared `options` → `optionsQuery` rows → catalog enum `values`. Declaring **both** `options` and `optionsQuery` is rejected — two sources for one list is an agent mistake, not a merge.
...
| ------------------------------------------------------------ | --------------------------------------------------------------- |
| Query failed validation or was denied by the viewer's roles  | Options failed to load, plus the gate's message                 |
