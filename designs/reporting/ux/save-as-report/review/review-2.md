...
Line 55 argues the conversation link works "because the sheet **is a page** calling `CallAPI` where `_state: conversationId` is in hand". But everywhere else the sheet is a `Modal`: "The sheet is otherwise all existing blocks: `Modal` + …" (line 49), "New `modules/reporting/pages/components/save_report_sheet.yaml`" mounted on chat.yaml (lines 69-70). These can't both be true.
