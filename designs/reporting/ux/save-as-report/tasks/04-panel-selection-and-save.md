...
The panel today (`modules/reporting/pages/chat.yaml`) accumulates three state arrays — `charts`,
...
`modules/reporting/api/emit-data-parts.yaml` mints an `id` and a `created` onto every part
...
- Table card: edit `modules/reporting/pages/chat/components/table_card.yaml` to add
  `tables.$.selected`.
...
Extract the assembly to `modules/reporting/pages/chat/actions/save_as_report.yaml` and `_ref` it
...
- `modules/reporting/pages/chat.yaml` — modify — chart/download card ticks, Save-as-report button,
  mount the sheet, `_ref` the assembly action.
- `modules/reporting/pages/chat/components/table_card.yaml` — modify — table card tick.
- `modules/reporting/pages/chat/actions/save_as_report.yaml` — create — the assembly + open action.
