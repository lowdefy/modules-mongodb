...
- **Consumes:** `modules/reporting/defaults/new_report.yaml` (task 1) — `_ref` with
...
**Create `modules/reporting/api/create-report.yaml`**, an `Api` mirroring `generate-report.yaml`'s
...
**Register and export in `modules/reporting/module.lowdefy.yaml`:**
...
- `modules/reporting/api/create-report.yaml` — create — validate-and-insert, conversation_id from payload.
- `modules/reporting/module.lowdefy.yaml` — modify — `api:` `_ref` and `exports.api` entry.
