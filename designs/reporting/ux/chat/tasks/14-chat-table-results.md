...
All in `modules/reporting/pages/chat.yaml`.
...
- `modules/reporting/pages/chat.yaml` — modify — the `tables` state array in all four places, the
  `onDataPart` branch, the Tables section
- `modules/reporting/pages/chat/components/*.yaml` — create, if the card is extracted
