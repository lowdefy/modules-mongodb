...
These tasks implement `designs/reporting/ux/chat/design.md`: the chat page reworked so the report
...
- **Persisted field names are `snake_case`**; `created` / `updated` / `deleted` are change stamps of
  ...
  `modules/reporting/defaults/change_stamp.yaml`.
...
- **Every var carries full `description` / `type` / `default` in `modules/reporting/module.lowdefy.yaml`**,
  then `pnpm docs:gen`. Never hand-edit `docs/reporting/reference/vars.md` or `docs/llms.txt`.
...
`modules/reporting/module.lowdefy.yaml`.** There is no logic dependency between the rail's delete
...
**Source:** `designs/reporting/ux/chat/design.md`
**Context read:** `designs/reporting/ux/design.md` (the parent's data model, cross-cutting
invariants and endpoint inventory) and `designs/reporting/ux/ownership/tasks/tasks.md` (the sibling
decomposition's global constraints, for consistency). Verified against source: every file the design
names — `modules/reporting/pages/chat.yaml`, the eight conversation and query endpoints, the agent
