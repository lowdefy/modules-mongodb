...
All in `modules/reporting/pages/chat.yaml`, inside `chat_panel`.
...
- `modules/reporting/pages/chat.yaml` — modify — the block's `welcome` removed, the two-track empty
  state added
- `modules/reporting/pages/chat/components/*.yaml` — create, if a track is extracted
