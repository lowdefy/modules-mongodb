...
`modules/reporting/api/emit-data-parts.yaml` is the onFinish hook that turns a turn's tool
...
**`modules/reporting/api/emit-data-parts.yaml`:**
...
**`modules/reporting/api/get-conversation-results.yaml`:** add a third `tables` branch to the
...
- `modules/reporting/api/emit-data-parts.yaml` — modify — table specs, the second `:for`, minted
  `id` / `created`, `$slice: -50`
- `modules/reporting/api/get-conversation-results.yaml` — modify — the third branch
