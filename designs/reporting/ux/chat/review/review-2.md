...
`modules/reporting/api/save-conversation.yaml:39-47` already `$set`s `messages`, `owner` and
...
- a new `modules/reporting/api/render-table.yaml` mirroring `render-chart.yaml` — a `payloadSchema`,
...
- registration in `modules/reporting/agents/reporting-assistant.yaml:179-191`'s `tools:` list, plus
