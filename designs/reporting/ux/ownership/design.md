...
- `modules/reporting/api/list-reports.yaml` — own-only (`owner.user_id` match), `deleted.timestamp: { $exists: false }`, sort `updated.timestamp: -1`, `limit: 200`, projection `title/description/created/updated`. No scope, search, sort or paging parameters.
...
**Section ids are positional, so the call carries a guard.** `validateReportSpec` derives them as `s0`, `s1`… from array order at read time; they are not stored on the document. A `section_id` is therefore an index under another name — meaningful only against the spec the caller just read — and removing one renumbers everything after it.

The ordinary sequence is safe, because the report page re-resolves after a removal and the next click works from fresh ids. What is not safe is two calls from the **same** render: a double-click sends `s2` twice, the first removes the intended section, and the second removes whatever slid into that slot. A section then disappears from a report that has no undo, since nothing versions a spec.

So the payload is `{ report_id, section_id, expected_type, expected_label }`, and the endpoint rejects when the section at that position is not the one described. The second call of a double-click fails instead of destroying something, and the page refetches. Deliberately server-side rather than a button disabled while the call is in flight: this design's position throughout is that a hidden or disabled control is a UX affordance and never the boundary — relying on the button here would be the one place it broke its own rule.

**Not stored section ids, for now.** Giving sections durable ids would make this correct by construction, and make any future reference to a section — a log line, a fix-in-chat context — stable rather than positional. It is the better shape and it is not worth it yet: it changes the spec contract for all three writers (the agent, the save sheet, this endpoint), and nothing today needs a durable reference. The guard is what the concrete need justifies.
...
| `remove-report-section` | new     | `{ report_id, section_id, expected_type, expected_label }` — owner-only. Server-side read → guard the position → remove → cascade filter bindings → revalidate → write; stamps `updated`. Rejects on a guard mismatch, and when the removal plus its cascade would leave no sections. [Why it is not a `$pull`](#dropping-a-section-is-the-one-spec-write-and-it-has-to-cascade).                      |
| `duplicate-report`      | new     | `{ report_id }` → new doc: `title` / `description` / `spec` copied, `visibility: private`, owner = caller, `favourite_of: []`, **`conversation_id: null`**, its own `created` / `updated`, `deleted: null`. Readable-report check.                                                                                                                                                                     |
