...
**`spec` holds `{ sections }` only.** `title` and `description` are document fields, not spec fields — the single source for the list, for search, for sort and for `set-report-title`, which then writes one field and never touches the spec. `resolve-report` composes `{ title, description, sections }` from the document before re-validating, so neither `validateReportSpec` nor `compileReport` changes. The split is right independently of the duplication it removes: `spec` is the AI-authored contract, `title` is user-editable metadata, and review 2's decision that `search` matches title and description "and explicitly not the spec" becomes automatic once the spec carries no prose.
...
- **`spec`** — `{ sections }`, the validator's output rather than the writer's input, with each section carrying a durable id. See [the stored spec is the validator's output](#the-stored-spec-is-the-validators-output).
...
| Index                                                     | Serves                                        |
| --------------------------------------------------------- | --------------------------------------------- |
| `owner.user_id`, `deleted.timestamp`, `updated.timestamp` | Mine, the deleted scope, and the default sort |
| `favourite_of`                                            | The Favourites scope                          |
| `visibility`, `deleted.timestamp`                         | Shared                                        |
...
- `modules/reporting/api/generate-report.yaml` — insert shape gains `visibility: "private"`, `favourite_of: []` and `spec_version: 1`, and **stores `_state: validated` rather than `_payload: spec`**, with `spec` holding `{ sections }` only and `title` / `description` staying document fields. (`conversation_id` is already on the document, still null on this path.) `create-report` writes the same shape — see [save-as-report](../save-as-report/design.md).
- `modules/reporting/api/resolve-report.yaml` — read match opened to shared; returns the owner flag; and composes `{ title, description, sections }` from the document before `querySections` / `compileReport`, so the compiler keeps reading `validated.title` unchanged.
- `plugins/modules-mongodb-plugins/src/analytics/validateReportSpec.js` — preserves a section's supplied `id` instead of always deriving one from position (it already accepts `id` on a filter section and ignores it), so a re-validated stored spec keeps its durable ids. No change to what it validates.
- `modules/reporting/module.lowdefy.yaml` — `share_roles`, the `reports_collection` description note, plus the new endpoint exports.
- `docs/reporting/` — a concepts page for ownership / visibility / retirement, the expected indexes, and regenerated `reference/vars.md`.
