...
Create `modules/reporting/api/set-report-visibility.yaml`, `type: Api`.
...
Register the endpoint in `modules/reporting/module.lowdefy.yaml`: a `_ref` under `api:` and an entry under `exports.api` with a one-line description.

## Acceptance Criteria

`apps/demo/e2e/reporting/report-visibility.spec.js`, using two seeded users — one holding a `share_roles` role, one not:
...
- `modules/reporting/api/set-report-visibility.yaml` — create
- `modules/reporting/module.lowdefy.yaml` — modify — `_ref` under `api:` and an `exports.api` entry
- `apps/demo/e2e/reporting/report-visibility.spec.js` — create — the six-cell auth matrix plus the deleted and single-field cases
