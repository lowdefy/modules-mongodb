...
`modules/reporting/api/list-reports.yaml` is today a `MongoDBFind`: own-only (`owner.user_id` match), not-deleted, sorted `updated.timestamp: -1`, `limit: 200`, projecting `title/description/created/updated`. No scope, search, sort or paging.
...
- **Consumes:** the document shape from task 3 (`visibility`, `favourite_of`, `spec_version`, `spec: { sections }`, `title` / `description` as document fields); `callEndpoint` and `reportDoc` from `apps/demo/e2e/reporting/helpers.js`.
...
- `apps/demo/e2e/reporting/report-scopes.spec.js`: **one spec per scope, all five, each asserting both what it returns and what it withholds.** The negatives are the point:
...
- `modules/reporting/api/list-reports.yaml` — modify — rewritten as `MongoDBAggregation` with the scope `$match`, `$regex` search, `$addFields`, the sort branch and `$facet` paging
- `apps/demo/e2e/reporting/report-scopes.spec.js` — create — five scope specs plus paging, sort-replacement and the `favourite_of` projection
