...
**Today's insert** (`modules/reporting/api/generate-report.yaml`) already runs the validator into routine state:
...
- **Produces:**
  ...
  - `apps/demo/e2e/reporting/helpers.js` — `callEndpoint(page, endpointId, payload)` and `reportDoc({...})`, used by every spec in tasks 4–8 and 11.

## Task

### 1. `modules/reporting/api/generate-report.yaml` — the insert shape
...
### 2. `modules/reporting/api/resolve-report.yaml` — the read
...
### 3. `apps/demo/e2e/reporting/helpers.js` — the shared spec helpers
...
Plus a `reportDoc({ id, title, owner, visibility, favouriteOf, deleted, spec, specVersion })` factory returning a document in the new shape, defaulting `visibility: "private"`, `favourite_of: []`, `spec_version: 1`, `deleted: null`, `conversation_id: null`, and both change stamps — modelled on the existing `reportDoc` in `apps/demo/e2e/reporting/formatted-report.spec.js`, which should be left where it is (that spec is a regression guard for the identity key and is not this task's to move).
...
### 4. `apps/demo/e2e/reporting/report-resolve-shared.spec.js`
...
- `modules/reporting/api/generate-report.yaml` — modify — insert shape, `_state: validated`, rewritten header comment
- `modules/reporting/api/resolve-report.yaml` — modify — signed-in guard, readable predicate, composed spec at both call sites, `is_owner` in the return
- `apps/demo/e2e/reporting/helpers.js` — create — `callEndpoint`, `reportDoc`, `SPEC`
- `apps/demo/e2e/reporting/report-resolve-shared.spec.js` — create — private/shared/deleted read matrix and `is_owner`
