...
Create `modules/reporting/api/remove-report-section.yaml`, `type: Api`. Owner-only. The routine is read → remove → cascade → revalidate → write.
...
Register the endpoint in `modules/reporting/module.lowdefy.yaml`.

## Acceptance Criteria

`apps/demo/e2e/reporting/report-remove-section.spec.js`. **The two cascade cases are the point — a test that only drops a standalone KPI proves nothing:**
...
- `modules/reporting/api/remove-report-section.yaml` — create
- `modules/reporting/module.lowdefy.yaml` — modify — `_ref` under `api:` and an `exports.api` entry
- `apps/demo/e2e/reporting/report-remove-section.spec.js` — create
