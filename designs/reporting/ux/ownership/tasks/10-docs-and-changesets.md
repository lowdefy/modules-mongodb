...
**Docs.** `docs/` is the source of truth for consumer-observable authoring behaviour. `docs/reporting/` currently holds `index.md`, two concepts pages (`open-query-engine.md`, `implementation-walkthrough.md`), two how-to guides, and three reference pages (`catalog.md`, `presentation-contract.md`, the generated `vars.md`). Nothing there tells a consumer that reports have an audience, a life cycle, or a set of indexes the host app must create.
...
### 1. `docs/reporting/concepts/ownership.md`
...
- Link the new page(s) from `docs/reporting/index.md`.
...
- `docs/reporting/concepts/ownership.md` exists with valid front-matter and covers all nine bullets above.
- The index table and its caveat are documented, and `docs/reporting/index.md` links to the new page(s).
...
- `docs/reporting/concepts/ownership.md` — create
- `docs/reporting/reference/indexes.md` — create — the expected indexes and the unindexable-sort caveat
- `docs/reporting/index.md` — modify — link the new pages
...
- **Do not hand-edit `docs/reporting/reference/vars.md` or `docs/llms.txt`.** Both are generated; `pnpm docs:check` fails on drift and runs on every PR.
- Source-side READMEs (`modules/reporting/README.md`) are stubs that point into `docs/` — do not add content to them.
