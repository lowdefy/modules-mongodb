...
fixtures; the demo catalog is `apps/demo/modules/reporting/catalog.yaml`; the demo reporting
module vars are `apps/demo/modules/reporting/vars.yaml`.
...
2. **A shared report over a role-gated catalog collection**, opened by a demo user who does NOT
   ...
   `apps/demo/modules/reporting/catalog.yaml` — the first demo coverage catalog role-gating has
...
- `apps/demo/modules/reporting/catalog.yaml` — modify: add one `roles`-gated collection entry.
- `apps/demo/modules/reporting/vars.yaml` and demo user/role config — modify as needed for the role-gated user setup.
