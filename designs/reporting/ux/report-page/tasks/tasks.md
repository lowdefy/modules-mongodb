...
Implements `designs/reporting/ux/report-page/design.md`: a provenance line, per-section CSV
...
**Source:** `designs/reporting/ux/report-page/design.md`
**Context read:** the design's referenced source — `modules/reporting/api/resolve-report.yaml`, `modules/reporting/pages/report.yaml`, `plugins/modules-mongodb-plugins/src/analytics/compileReport.js`, `plugins/modules-mongodb-plugins/src/analytics/validatePipeline.js`; sibling designs `ownership`, `save-as-report`, `reports-from-chat` (all shipped); demo seeds under `apps/demo/api/reporting-seed-*.yaml` and `apps/demo/modules/reporting/catalog.yaml`; `docs/reporting/`.
