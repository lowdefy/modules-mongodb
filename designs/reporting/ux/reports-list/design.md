...
- `modules/reporting/api/list-reports.yaml` — own-only, no parameters at all. Rewritten by [ownership](../ownership/design.md#endpoints) to take `{ scope, search?, sort?, cursor? }`.
...
- **Shared, nothing published** — states that nobody has published a report, which is a fact about the app, not a prompt to act.
- **A search or scope with zero results** — offers to clear the search, and does not teach anything.

Collapsing them into one message would put "make your first report" in front of someone whose search simply missed.

## Files changed (anticipated)

- `modules/reporting/pages/reports-list.yaml` — rebuilt as an `AgGridBalham` with toolbar (scope segmented control, search, sort), `buttons` / `tag` / `date` cells, the row action menu, footer recovery link, delete confirm, and the three empty states.
...
- **A grid over a server-paged, server-scoped source is easy to get subtly wrong** — a stale cursor after a scope switch, or a client-side sort silently disagreeing with the server's. Each scope change resets the cursor, and sort is a request parameter only; the grid does no sorting of its own.
