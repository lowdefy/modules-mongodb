...
1. **Create `modules/reporting/pages/reports-deleted.yaml`** (page `id: reports-deleted`). On mount,
...
6. **Export the page.** In `modules/reporting/module.lowdefy.yaml` add `- id: reports-deleted` under
...
- `modules/reporting/pages/reports-deleted.yaml` — create — the recovery page.
- `modules/reporting/module.lowdefy.yaml` — modify — add the `reports-deleted` page export and `_ref`.
