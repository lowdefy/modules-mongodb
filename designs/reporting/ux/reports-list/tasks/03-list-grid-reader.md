...
`modules/reporting/pages/reports-list.yaml` is today a `Box` with a stacked-card `List` hardcoded
...
- `modules/reporting/pages/reports-list.yaml` — rewrite — the AgGrid reader described above.
...
- **Verify the chat pre-select contract:** check whether `modules/reporting/pages/chat.yaml` reads a
