...
**Task 5** depends on task 4 (the document shape is finalised by then so the trimmed columns line up with what's actually written) and on task 6 (the export's round-trip can only be verified end-to-end once the demo collection holds the new shape). The structural edit can be authored after task 4; full verification has to wait for task 6.
