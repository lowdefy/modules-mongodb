...
- `pages/login.yaml` — add the magic-link send affordance (gated on `magicLink.enabled`), the `link-sent` result state + resend, and the email-only shape when `emailAndPassword.enabled` is false. Add `INVALID_TOKEN` to the error table and set `callbackUrl` + `newUserCallbackUrl` on the send (`errorCallbackUrl` is left to the engine default → `authPages.error`). Gate the `nav` footer's "Create an account" link on `emailAndPassword.enabled` — passwordless builds no signup page, so the link would 404 and misdescribe the collapsed flow.
- `pages/accept.yaml` — the invitation-accept page's "sign in / create an account" link to `signup` is `_build.if`-gated on `emailAndPassword.enabled` (→ `signup` when password on, → `login` when off), since passwordless builds no signup page.
