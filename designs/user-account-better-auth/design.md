...
- **New shared fragment**: `write-profile` — pairs the contact write (change-stamped) with the `UpdateUserProfile` re-denorm, so the denormalized `user.profile` bag can't drift. Exported by this module and `_ref`'d by user-admin's profile-save routine; this is the cross-module freshness invariant of Decision 6, enforced mechanically by the same "one correct way" reasoning that produced the shared `create-or-link-contact` fragment (Decision 7). Each module wraps it with its own target ids, audit event, and write-stage extensions. (user-admin's design must `_ref` it in turn — tracked in that design.)
...
| Components  | `profile-avatar`, `user-selector`, `user-multi-selector`, `user-avatar`, `create-or-link-contact` (shared upsert-on-`lowercase_email` fragment, `_ref`'d by user-admin's invite), `write-profile` (shared contact-write + `UpdateUserProfile` re-denorm fragment, `_ref`'d by user-admin's profile save) |
