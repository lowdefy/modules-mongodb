...
- [ ] Verified but no membership → **MEMBERSHIP_REQUIRED** "no access" state — _not testable under `signup: open` (everyone auto-joins); runs under the **Invite-only posture** section in [`03-user-admin-console.md`](./03-user-admin-console.md), which flips to `invite-only` and asserts reject-and-write-nothing_
