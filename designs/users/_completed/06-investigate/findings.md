...
> **Superseded by the auth-testing campaign.** This is a historical triage snapshot. The
> still-open item (F12) is now tracked live in
> [`../auth-testing/findings/`](../../auth-testing/findings/index.md); new findings go there, not
> here. F11 and F19 were fixed upstream (`callback-url-default`, `update-session-store-refresh`).
