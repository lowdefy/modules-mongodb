...
- `visible` gated on the resolved status entry being non-null (so an unmatched/null `status` hides the pill — no grey placeholder).
- Layout: `flex: 0 0 auto`, sits to the left of the title. Style per the visual spec (`padding: 15px 14px`, `borderRadius: 6`, `fontSize: 15`, `fontWeight: 500`, single line, vertically centred — not `height: 100%`).
...
- Structurally the eyebrow sits directly above the title. Since the title block is a separate sibling that can skeleton, arrange the eyebrow + title-block so the eyebrow renders above the title visually (a wrapping column box around eyebrow + title block, or the eyebrow as a preceding sibling — choose whichever keeps the eyebrow outside any block that carries `loading`).
