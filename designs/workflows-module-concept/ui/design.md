...
- Each button only renders when the FSM lists an outgoing transition for it from the action's current state — a button whose signal the current state doesn't accept is hidden (same gating the form templates apply). No "pick any status" affordance exists. Visibility is resolved **server-side**: `GetWorkflowAction` returns per-signal `buttons.{signal}` booleans (FSM source-stage ∧ per-verb role gate ∧ `allow_not_required` for `not_required`), and the surface renders them dumb ([Part 46 § D5](../../workflows-module/parts/_completed/46-debundle-workflow-config/design.md), [Part 40 § D2](../../workflows-module/parts/40-simple-action-surfaces/design.md)). `not_required` is additionally gated by the action-root `allow_not_required` flag (opt-in, engine-enforced, any kind — [Part 40 § D3](../../workflows-module/parts/40-simple-action-surfaces/design.md)); no other per-action button config exists for check actions.
...
**Error recovery for check actions is a `resolve_error` button on `workflow-action-view`** ([Part 40 § D4](../../workflows-module/parts/40-simple-action-surfaces/design.md), resolving Open Question 4). A check action lands in `error` only via a pre-hook `error` cascade ([state-machine](../state-machine/design.md)); the view page renders `resolve_error` only when the stage is `error` (FSM `error → resolve_error → in-review`), reusing the comment field as the recovery note. There is **no `check-error` page** — the engine's link table routes the check `error` verb to `workflow-action-view`.

**In-context modal.** Beyond the three pages, the live working surfaces (`actions-on-entity` and the event-timeline action cards) can open a check action in place via the standalone `check-action-modal` component, which `_ref`s the same shared `check-action-surface` the pages render ([Part 40 § D5](../../workflows-module/parts/40-simple-action-surfaces/design.md), and Decision 3 above). The page stays the canonical, addressable target — notifications and deep links always navigate.
