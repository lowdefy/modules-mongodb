...
Pages live in the workflows module's page tree, generated by `makeActionPages` at the host app's build time. Each generated page gets the module entry's id prefix automatically (e.g. `workflows/workflow-lead-onboarding-qualify-edit`).

Apps reference these via `_module.pageId: { id: "workflow-{workflow_type}-{action_type}-{verb}", module: workflows }`.
...
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `form`    | Per-action `workflow-{workflow_type}-{action_type}-edit` / `-view` / `-review` / `-error`. Per-verb page is emitted only when the verb key is present in the action's `access.{app_name}` map. All four verbs are gated identically; `error` follows the same rule as `edit` / `view` / `review`. |
...
- `templates/edit.yaml.njk` — edit form. Ships the template-shipped `submit` and `progress` buttons, plus `not_required` when the action opts in via `pages.edit.buttons.not_required.visible: true`. Buttons call `workflow-{workflow_type}-{action_type}-submit` with the matching `signal` value.
...
- `templates/review.yaml.njk` — read-only form display + template-shipped `approve` / `request_changes` buttons calling `workflow-{workflow_type}-{action_type}-submit` with `signal: approve` / `signal: request_changes`. Also ships an `Edit` navigation button (a `Link` back to the edit page with `input: { skip_status_redirect: true }`) when the `edit` verb is declared in this app's `access` map, so reviewers can fix small issues themselves rather than bouncing the action back with `request_changes`.
- `templates/error.yaml.njk` — recovery surface for actions in `error` status. The page is reached **only via author-driven entry** to `error` (a pre-hook firing the `error` signal against another action — `actions: [{ type, signal: error }]` — or external system writes; see [engine § Action error transition](../engine/spec.md#action-error-transition)). Engine sub-step failures no longer route here; they throw to `CallApi` and the user retries the same submit ([Part 29 § D1](../../workflows-module/parts/_completed/29-error-model-cleanup/design.md#d1-why-throwing-is-safer-than-force-writing-error)). Template ships:
  ...
  - **Template-shipped `resolve_error` button** wired to `workflow-{workflow_type}-{action_type}-submit` with `signal: resolve_error` (form FSM `error → resolve_error → in-review`); fires the author's `pages.error.events.onSubmit` first for page-state work. Title and optional confirm modal overridable via `pages.error.buttons.submit.{title, modal}`. Extra buttons go in `formFooter:`.

The `-error` page is gated identically to the other verbs: emitted iff the `error` verb key is present in the action's `access.{app_name}` map. `pages.error` is purely a chrome-override slot (like `pages.edit`); the template ships sensible defaults when it's absent. Per-app visibility of the recovery link from other pages is additionally controlled at the status-map level — omit `status_map.error.{app_name}` to suppress the recovery link even when the page exists.
...
These are the *button-surfaced* signals (the "interactions"); the FSM also accepts engine/pre-hook-only signals (`unblock`, `activate`, `block`, `internal_*`) that no template surfaces. On click, each button (1) fires the matching `pages.{verb}.events.{handler}` author-supplied event (for page-state work — set state, fire requests, validate), then (2) calls `workflow-{workflow_type}-{action_type}-submit` with `signal: <name>` and the standard payload (`form`, `form_review`, `fields`, `current_key`). Authors who need pre-write logic register a pre-hook (`hooks.{signal}.pre`, submit-pipeline Decision 4); authors who just need page-state work register the matching event verb.

**Static simple-action pages** at `pages/`:

- `pages/simple-edit.yaml` — `assignees` multi-select, `due_date` picker, `description` text input, comment field (rich text), and the template-shipped signal buttons (`submit`, `progress`, `not_required` — the same bar `edit.yaml.njk` ships; see Decision 7). **No status selector** — simple actions advance through the same nullary signal buttons as form actions ([state-machine](../state-machine/design.md) "Simple kind"). The `submit` button calls `workflow-{workflow_type}-{action_type}-submit` with `signal: submit` (nullary — no `target_status`; the FSM resolves `in-review` vs `done` from the action's `review` verb, exactly as for form actions), `fields:` block, `form:` (empty for simple actions), and a top-level `comment` field (the resolver-emitted API maps it to `event.metadata.comment` before the event hits the events module).
...
- Module-shipped requests fired on mount:
  - `get_workflow_overview_data` — joins workflow doc + action docs filtered by the `view` verb's gate in `access.{app_name}` (the action declares `view` and its gate passes the caller's per-app roles), ordered by `display_order` / `sort_order`. Returns one row per action with status, status_map, universal fields, the per-verb `visible_verbs` bag, and (for keyed actions) the key value.
...
- **Page ID:** `workflow-{workflow_type}-{action_type}-{verb}`. No key segment.
...
Updates flow through `workflow-{workflow_type}-{action_type}-submit` like any other action change. The template-shipped button composes the `fields:` payload from form-state plus an optional top-level `comment` field (the resolver-emitted API maps it to `event.metadata.comment` — see part 13 design § Comment mapping).
...
4. Each action's status-map link / message rendered from `status_map[currentStage].{app_name}`. Each action payload carries `visible_verbs: { view, edit, review, error }` from the query-time per-verb check; the component renders only the verb affordances whose bool is `true` (e.g. an edit-link cell shows only when `visible_verbs.edit`). Actions returned at all have at least one `true` verb (the query drops all-`false` actions).
...
- **Fully access-restricted workflow** (current user has no `visible_verbs` `true` on any action): hide entirely. v1 default.
...
Reusable access-check primitive. Reads the current user's per-app roles (`_user.apps.{app_name}.roles` on the `user_contacts` doc) and the action's `access.{app_name}` verb-gate map, and populates a **per-verb** `_state.action_allowed: { view, edit, review, error }` (matching the `visible_verbs` bag `get-entity-workflows` returns). Each verb's bool is `true` when its gate is `true` or the user's roles intersect the gate array; `false` for undeclared verbs. Consumers read the specific verb they care about (`_state.action_allowed.edit` on the edit page, `.review` on the review page). Mirrors the query-time check client-side for conditional rendering — defence in depth; the server-side query/submit checks are the real gate.
