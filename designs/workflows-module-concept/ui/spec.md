...
| `custom`  | None — the app supplies the working pages; navigation rides the author's `status_map` `link:` cells (routed into the per-verb link map), with the shared `{workflow_type}-action` page as the read-only observer fallback ([Part 28](../../workflows-module/parts/28-custom-action-kind/design.md)) |
