...
Replace the routine-orchestrated submit-action flow with an engine-orchestrated one. A new plugin request `SubmitWorkflowAction` owns the full submit lifecycle (pre-hook → engine writes → side effects → post-hook); a resolver emits one `{workflow_type}-{action_type}-submit` Lowdefy API per form/simple action; page templates declare per-template button bars that call those APIs with a `signal` value naming what the user fired.
...
1. **Button bars per template.** The default bars are settled in [state-machine](../state-machine/design.md) "Templates and buttons" (`edit`: `submit` / `progress` / `not_required`; `view`: `request_changes` + Edit link; `review`: `approve` / `request_changes`; `error`: `resolve_error`). Remaining edge cases to confirm during review:
   ...
   - How the shared simple pages surface `error` *recovery* — a `simple-error` page vs. a `resolve_error` button on `workflow-action-view` (ui follow-on; the simple `error` row is reachable via cascade but no simple page ships an error surface yet).
