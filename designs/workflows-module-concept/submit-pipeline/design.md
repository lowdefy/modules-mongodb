...
| ----------------- | ---------------------------------------------------------------------- | ------------------- |
| `submit_edit`     | `edit` template (form + simple edit)                                   | `submit_edit`       |
...
1. **Engine default per interaction.**
   ...
   | Interaction       | Default target status (form action)                                                                              | Default target status (simple action)                       |
   | ----------------- | ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
   | `submit_edit`     | `in-review` if the action has `review` in any `access.{app_name}` verb list, else `done`                         | caller-supplied (status selector on `simple-edit`)          |
   ...
   Simple `submit_edit` is special: the shared `simple-edit` page surfaces a status selector (see ui Decision 7), so the payload carries `current_status` directly. Form `submit_edit` doesn't — the engine derives it from whether reviewers exist.
