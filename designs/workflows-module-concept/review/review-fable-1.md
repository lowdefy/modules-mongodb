...
**`request_changes` verb contradiction — VERIFIED shipped, RESOLVED engine-side → Part 49:** the contradiction is live in the implementation. Engine: `loadWorkflowState.js`'s `SIGNAL_VERBS` maps `request_changes → 'review'`, fail-closed when the verb is absent. Template: `view.yaml.njk`'s `button_request_changes` (opt-in via `page_config.buttons.request_changes.visible`, default `false`) gates on `_state.action_allowed.view`. With the shipped engine mapping, every click from a viewer-without-review-role is rejected server-side after the mandatory comment modal — and on actions with no `review` verb at all (the case the view-bar button exists for), every click from everyone is rejected. Blast radius today is zero because the button is opt-in default-hidden.

**Resolution (Sam):** the engine's mapping is the wrong side — `review` permission gates the reviewer's _judgement_ power (`approve`: in-review → done) and review-page access; `request_changes` is "flag a problem, send it back," which anyone who can see the action may raise. If an author opts the button onto the view page, everyone with `view` should be able to click it. Fix: engine-side — `SIGNAL_VERBS.request_changes` becomes **`view` OR `review`** (pass if either gate passes; the `review` arm covers the legal-but-lint-warned reviewer-without-view edge so a reviewer is never rejected firing it from their own review page). Templates are untouched (the view-gate was correct all along). Accepted consequence, consistent with the design's "buttons are UI affordance; the verb gate + FSM are the real rules" posture: any viewer can fire `request_changes` via the API against `in-review`/`done` actions regardless of button opt-in; every fire carries a mandatory comment and logs an event. Doc updates: action-authoring D3 "Interaction → required verb" table; state-machine "Templates and buttons" view-row justification (currently argues the view-gate as a workaround; it's simply the correct gate); submit-pipeline OQ1's "should be reviewer-gated (state-machine review-1 finding 7)" bullet is resolved the other way. Part 34 D6's original mapping gets a deviation note in the new part (Part 34 itself is read-only history). Scoped as **Part 49 — request-changes verb gate** (`designs/workflows-module/parts/_next/49-request-changes-verb-gate/`).
