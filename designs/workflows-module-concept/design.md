...
- The submit-time API surface — resolver-emitted per-action `update-action-{action_type}` endpoints (one per form / simple action; tracker actions get none); engine-orchestrated lifecycle with pre/post hooks per interaction; fixed button vocabulary on templates — [submit-pipeline](submit-pipeline/design.md).
...
- `schedule-followup` — **simple action** (gated on `send-quote`; no domain form, just a status selector + assignees + due date — typical "remember to call them on X date" step)
