...
2. **The error case is worth naming**: called when the caller has no `twoFactor` row or `twoFactorEnabled` is false, BetterAuth throws `TWO_FACTOR_NOT_ENABLED`. Consumers gate the trigger on enrolment state, so this is a guard rather than a routine path, but the action should surface it as a normal thrown error like the rest of the catalog.
