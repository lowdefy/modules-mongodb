...
The current submit pipeline doesn't have a post-write `context.action` to hand in. [`handleSubmit.js:63-65, 100-101`](../../../../../../plugins/modules-mongodb-plugins/src/connections/WorkflowAPI/SubmitWorkflowAction/handleSubmit.js) fetches the action once via `getCurrentAction` _before_ step 4, stashes it on `context.action`, and never refreshes it. Step 4's per-entry write loop ([handleSubmit.js:226-249](../../../../../../plugins/modules-mongodb-plugins/src/connections/WorkflowAPI/SubmitWorkflowAction/handleSubmit.js)) does an in-memory cache update on the matching doc in `context.workflowActions`, but those are different object references from `context.action` (one comes from `getCurrentAction`, the other from `getActions(workflow._id)`). By step 7 (`buildDefaultLogEventPayload({ action: context.action, ... })` at [handleSubmit.js:319-328](../../../../../../plugins/modules-mongodb-plugins/src/connections/WorkflowAPI/SubmitWorkflowAction/handleSubmit.js)), `context.action.metadata`, `context.action.status[0].stage`, and every newly-written `action[slug]` subdoc are the values that were on disk _before_ this submit started.
