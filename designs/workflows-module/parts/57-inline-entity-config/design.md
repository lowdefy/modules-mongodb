...
Today a workflow's entity wiring is scattered across three places: `entity_collection` and `entity_ref_key` sit flat on the workflow definition in `workflows_config`, while the host-app routing metadata the module needs to deep-link back into an entity page (`page_id`, `id_query_key`, `title`) lives in a separate `entities` module var — a map keyed by `entity_collection`, declared in the app's `vars` block away from the workflows it serves. This design consolidates **all** of a workflow's entity wiring into one nested `entity:` block on the workflow definition — `collection`, `ref_key`, `page_id`, `id_query_key`, `title` — so a developer declares everything about a workflow's entity in one place, and removes the `entities` var entirely.

The block is **authoring sugar**: `makeWorkflowsConfig` lifts `entity.collection`/`entity.ref_key` back to the flat `entity_collection`/`entity_ref_key` names the engine, documents, queries, and indexes already use, so nothing downstream of the resolver changes. Nesting the entity object through the persistence/runtime layer (documents, `StartWorkflow` param, queries, indexes) is a larger, separate change tracked as its own part (see Dependents).

## Proposed change

1. Add a required `entity:` block to each workflow definition in `workflows_config`, holding `collection`, `ref_key`, `page_id`, `title`, and optional `id_query_key` (defaults to `_id`) — the single home for everything about the workflow's entity.
...
4. In `makeWorkflowsConfig`, validate the `entity:` block (required-string `collection`, `ref_key`, `page_id`, `title`; optional `id_query_key` defaulting to `_id`) and **lift `entity.collection`/`entity.ref_key` to the flat `entity_collection`/`entity_ref_key`** the materialized config already exposes — so the engine, doc-creation path, queries, and indexes are untouched. The `entity` block carries the routing fields (`page_id`, `id_query_key`, `title`) through to the connection via `WORKFLOW_FIELDS`.
...
**The separate map is unvalidated today.** The manifest description for `entities` claims "part 4's `makeWorkflowsConfig` validator confirms every `entity_collection` referenced in `workflows_config` has a matching key here." That cross-check is **not implemented** — `makeWorkflowsConfig.js` contains no reference to `entities` (the var is read only by the connection at runtime). So today a missing or misspelled entry silently yields a `null` `entity_link` (no back-link) with no build error. Folding the fields into the workflow's `entity:` block lets `makeWorkflowsConfig` validate every entity field at build time — turning a silent runtime degradation into a build-time error. It also closes a pre-existing gap: today only `entity_ref_key` is required-checked, while `entity_collection` rides through `WORKFLOW_FIELDS` unvalidated (a workflow omitting it builds clean and writes documents with an undefined collection). Under the new block both `entity.collection` and `entity.ref_key` are required-checked in one place.

**Per-workflow link variation falls out, and is a supported use case.** Co-locating the routing fields means two workflows on the same `entity_collection` can link to different entity pages — a different view per use-case — which a map keyed by `entity_collection` can't do without contortion. This is not the driver for the change (the two reasons above carry it on their own); it's a capability the new shape supports and that we intend to keep. The cost is duplication when several workflows share one collection and one link target — minimal here: the fields are few and static, and most entities carry a single workflow.

### Decision: one `entity:` block, lifted to flat names in the materialized config

All of a workflow's entity fields go in one nested `entity:` block:

```yaml
entity:
  collection: leads-collection
  ref_key: lead_ids
  page_id: lead-view
  title: Lead
...
- **Consistency / one mental model.** Everything about the workflow's entity is described in one place. A developer doesn't have to know that "collection" is a flat field but "page link" is a nested block — it's all `entity.*`.
...
**Authoring shape vs materialized shape.** `entity.collection` and `entity.ref_key` are more than config: `entity_collection` (with `entity_id`) is written onto every workflow and action document, queried (`GetEntityWorkflows`), indexed, and denormalized to parent/child workflows; `entity_ref_key` is read when dispatching events. Renaming those *persisted/queried* names is a separate, cross-cutting change (its own part — see Dependents). So this block is **authoring sugar only**: `makeWorkflowsConfig` lifts `entity.collection`→`entity_collection` and `entity.ref_key`→`entity_ref_key` into the flat fields the materialized config already exposes, leaving only the routing fields (`page_id`, `id_query_key`, `title`) inside the materialized `entity` block. The engine, doc-creation path, every query, and the indexes keep reading the flat names — unchanged. The one accepted cost: the authored name (`entity.collection`) differs from the persisted/queried field (`entity_collection`), so a developer debugging sees `entity.collection` in config and `entity_collection` in the database.
...
The prior design (part 17) chose the keyed map deliberately, on three arguments: it avoids duplicating fields across workflows that share a collection; the routing metadata is conceptually an *entity-kind* fact (especially part 26's future `get_entity_endpoint`, "how to fetch a Lead"); and the engine consumes it keyed by `entity_collection` off the document. On re-examination:

- The runtime argument does not hold: every read method already resolves the workflow's config entry by `workflow_type` *before* building `entity_link` (`GetWorkflowOverview.js:44`, `GetEntityWorkflows.js:81`, `GetWorkflowAction.js:147`, `GetWorkflowActionGroupOverview.js:43`), so reading `wfConfig.entity` is exactly as direct as `entities[entity_collection]` — no added indirection.
...
- `entity.collection` (required) — the entity's MongoDB collection connection id. Lifted to the flat `entity_collection` in the materialized config (the field the engine, documents, queries, and indexes use).
- `entity.ref_key` (required) — the event-references key (e.g. `lead_ids`) written into event docs so events surface on the entity. Lifted to the flat `entity_ref_key`.
...
After the resolver lifts `collection`/`ref_key`, the materialized `entity` block carries only the routing fields (`page_id`, `id_query_key`, `title`). The engine block becomes (using the already-resolved `wfConfig`):
...
`GetWorkflowAction` continues to source the id from the action doc (`action.entity_id`) while reading the routing fields from `wfConfig.entity` (resolved via `action.workflow_type`).

## Validation

In `makeWorkflowsConfig.js`:

- In `validateWorkflow` (~line 573), require `workflow.entity` to be an object with non-empty string `collection`, `ref_key`, `page_id`, and `title`; `id_query_key` is optional and defaults to `_id`. Fail the build with a precise message (e.g. `workflow "onboarding": missing required "entity.page_id" — the host-app page id the workflow back-link navigates to`). Keep the existing legacy `entity_type` rejection. The existing top-level-`title` type-check (the workflow's own display name) is unaffected — that field stays top-level.
- **Lift the persisted fields in the normalized output:** set the normalized workflow's flat `entity_collection` from `entity.collection` and `entity_ref_key` from `entity.ref_key`, and keep the routing fields (`page_id`, `id_query_key` defaulted, `title`) in the normalized `entity` block carried via `WORKFLOW_FIELDS`. This replaces today's flat `entity_collection`/`entity_ref_key` picks in `WORKFLOW_FIELDS`; the materialized shape the connection and engine see (`{ ..., entity_collection, entity_ref_key, entity: { page_id, id_query_key, title } }`) is unchanged.
...
- `modules/workflows/module.lowdefy.yaml` — remove the `entities` var (lines 75–87); rewrite the `workflows_config` description to document the unified `entity:` block (`collection`, `ref_key`, `page_id`, `id_query_key` default `_id`, `title`) instead of flat `entity_collection`/`entity_ref_key`.
- `modules/workflows/connections/workflow-api.yaml` — remove the `entities:` property (lines 17–18).
- `modules/workflows/resolvers/makeWorkflowsConfig.js` — validate the `entity:` block in `validateWorkflow` (`collection`/`ref_key`/`page_id`/`title` required strings, `id_query_key` optional default `_id`); lift `entity.collection`/`entity.ref_key` to the flat `entity_collection`/`entity_ref_key` in the normalized config; carry `entity` (routing) via `WORKFLOW_FIELDS` and drop the now-obsolete flat `entity_collection`/`entity_ref_key` picks. Keep the legacy `entity_type` rejection.
...
The modules are unreleased and have no external consumers, so there is no migration burden — the only `vars.entities` user is the demo, updated as part of this change. Each demo workflow's flat `entity_collection`/`entity_ref_key` and the app's `vars.entities` routing entry for its collection move into that workflow's `entity:` block; the `entities` var is then deleted. Build validation enforces the new shape (a workflow missing a required `entity.*` field fails the build).

## Non-goals

- **Nesting the entity object through the persistence/runtime layer** — entity identity stays flat (`entity_collection`, `entity_id`) on documents, queries, the `StartWorkflow` param, and indexes. The `entity:` block is authoring-only sugar, lifted to those flat names by the resolver. Restructuring the persisted/runtime layer to a nested `entity` object is a separate part (see Dependents).
- No per-workflow `page_id` *override-with-fallback* mechanism — each workflow declares its `entity:` block outright. (There is no shared map left to fall back to.)
...
- **Entity object end-to-end** — Part 59 (`parts/59-entity-instance-pointer`, "Nested entity instance pointer"). This design stops at authoring sugar: the `entity:` block is lifted to the flat `entity_collection`/`entity_ref_key` names the persistence/runtime layer already uses. Part 59 nests the entity object through that layer too — `{ entity: { collection, id } }` on workflow/action documents, an `entity` param object on `StartWorkflow`, `entity.collection`/`entity.id` query and index keys, and nested parent/child denormalization — so entity identity is represented one way top to bottom. It is cross-cutting (~8 engine source files + `StartWorkflow` + indexes + most test suites) and, since the modules are unreleased, needs no data migration. Scoped separately to keep this design self-contained.
