...
- `workflows-module-concept/ui/design.md` — Decisions 2, 3, and 7, Open Question 4, and the "no per-action customisation" simple-pages paragraph.
...
1. **`ui/design.md` Open Question 4 → resolve.** Simple-action error recovery is a `resolve_error` button on `workflow-action-view`, rendered only at stage `error` (FSM `error → resolve_error → in-review`); **no `simple-error` page**. Move it out of Open Questions and into Decision 7's body.
...
3. **`ui/design.md` "no per-action customisation" (simple-pages paragraph)** — record the single exception: the `allow_not_required` flag (D3).
...
- There is **no** `global.simple_action_buttons` and no resolver-emitted button-config global — that model was dropped (D3). If any concept-doc prose still references it, that is stale and should be replaced with the `allow_not_required` model.
