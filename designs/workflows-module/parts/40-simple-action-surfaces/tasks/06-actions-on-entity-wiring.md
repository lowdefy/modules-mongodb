...
1. **Drop the modal once.** Add a single `_ref` of `components/simple-action-modal.yaml` into `actions-on-entity.yaml`, passing the entity-workflows refetch (the `entity-workflows-refetch` component, or this component's existing `call_entity_workflows` + `set_entity_workflows` sequence) as the modal's `onComplete` `_var`.
...
- `ActionSteps.onActionClick` is wired to set `simple_action_modal.action_id` from the clicked action and open the modal.
