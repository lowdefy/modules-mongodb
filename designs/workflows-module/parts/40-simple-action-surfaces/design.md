...
> **Terminology note.** The action kind once called `simple` was renamed to `check` ([Part 43](../_completed/43-rename-simple-kind-to-check/design.md)); `simple` was itself the interim name `task` carried before [Part 35](../_completed/35-rename-task-kind-to-simple/design.md). This part is written in the final `check` vocabulary throughout — the kind is `check`, the components are `check-action-surface` / `check-action-modal`. The folder name (`40-simple-action-surfaces`) and the task filenames keep their original `simple-*` spelling as historical identifiers; only path/link tokens and statements describing what Part 35/38 *did* still read `simple`.
...
The three shared check-action pages (`workflow-action-edit` / `workflow-action-view` / `workflow-action-review`, renamed `task-*` → `simple-*` by [Part 35](../_completed/35-rename-task-kind-to-simple/design.md), then `simple-*` → `workflow-action-*` by [Part 38 task 18](../_completed/38-engine-rebuild/tasks/18-display-surface-renames.md) per review-14 #1) still run the **old interaction model**: `workflow-action-edit` carries a status **selector** with a `_js` priority filter (`workflow-action-edit.yaml:135`) and a `current_status` payload, and all three pages fire `interaction:` rather than `signal:`. The engine moved to **signals + FSM** ([state-machine](../../../workflows-module-concept/state-machine/design.md), [Part 38](../_completed/38-engine-rebuild/design.md)), and the form templates were migrated by [Part 39](../_completed/39-form-submit-buttons/design.md), which shipped the `enums/button_signal_sources.yaml` build-time visibility enum (read via `_ref`) and **explicitly handed the check surfaces to this sibling** ([Part 39 § Check actions are separate](../_completed/39-form-submit-buttons/design.md)).

This part rewrites the three shared pages to the signal model, deletes the status selector, resolves the check-action **error-recovery** question, and adds an **in-context modal** so the live working surfaces (`actions-on-entity` and the event-timeline action items) can open a check action without a full page navigation. With [Part 46](../46-debundle-workflow-config/design.md) sequenced first, the surfaces **consume `GetAction`'s server-resolved `action.buttons` and `action_allowed`** — no client-side visibility computation, no `action_role_check` mirror, no `button_signal_sources` `_ref`.
...
- **`workflow-action-view.yaml`** — read-only: action header (title + status badge), universal-fields (`mode: display`), **Status History** card (List over `status`), **Comments** card (aggregation over `events` where `action_ids` ∋ this action and `metadata.comment` exists). No button bar, no stale-URL guard. **Note:** [Part 33](../_next/33-comment-rendering/design.md), ordered before this part, deletes the Comments card and replaces it with the shared `events-timeline` `_ref` filtered to the action — by the time this part runs, the view body carries the timeline, not the card.
...
Stale concept prose is already mostly reconciled: [ui Decision 7](../../../workflows-module-concept/ui/design.md) is current (signal buttons, no selector) and Part 39 reconciled `ui` D2/D4 and `submit-pipeline` D3. The remaining open item is [ui Open Question 4](../../../workflows-module-concept/ui/design.md) (check-action error recovery), which this part resolves.
...
On open the modal runs the **same fetch sequence the page's `onMount` runs**, for `check_action_modal.action_id` — the surface depends on it (D6), and none of it runs unless the modal replicates it:
...
- A page that renders the event timeline but not `actions-on-entity`, and **wants** the in-context modal, drops `check-action-modal` itself (once) and wires `EventsTimeline.onActionClick` to it (passing its own timeline refetch as `onComplete`).
...
- **E2E (Part 22 supplements)** on the demo's `schedule-followup` check action: (a) Mark Started (`progress`) on `action-required` lands `in-progress`, persists due-date without advancing; (b) `submit` resolves `in-review` vs `done` per the `review` verb (nullary); (c) a button the `GetAction` response marks `false` (wrong stage or missing verb) is not rendered; (d) error recovery: a cascaded `error` shows `resolve_error` on `workflow-action-view`/modal and recovers to `in-review`; (e) clicking a `check` action in `actions-on-entity` opens the modal and submits without navigation, then the list refetches; (f) clicking a non-check (e.g. form) action in `actions-on-entity` navigates to its page instead of opening the modal; (g) the event-timeline action card opens the modal for a `check` action and navigates for others.
...
(The `allow_not_required` engine-unit coverage — `makeWorkflowsConfig` validation, the load-phase `access_denied` gate for form + check, and `GetAction` resolving `buttons.not_required` off live config — moves with the machinery to [Part 46](../46-debundle-workflow-config/design.md).)
...
- [Part 39 — Form submit buttons](../_completed/39-form-submit-buttons/design.md) — sibling; ships `enums/button_signal_sources.yaml` and the concept reconciliation this part consumes; scopes check actions out to here.
...
- [Part 35](../_completed/35-rename-task-kind-to-simple/design.md), [Part 24](../_next/24-universal-fields/design.md) — upstream renames + the real universal-fields renderer.
