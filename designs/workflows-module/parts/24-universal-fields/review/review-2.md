...
So `payload.fields` is *present* on every submit, the no-clobber guard's "only when present" branch fires, and
...
- **Part 38 doesn't say `planActionTransition.js` sets universal fields at all.** Its spec for that planner is
  ...
  line that Part 38 must first *establish*, and Part 38's design doesn't pin where field-setting lives in the new
  planner split.
...
was only ever added in review-1 #11's *design-level* resolution; it was never implemented — `ACTION_FIELDS` in
