...
- **`shared/phases/planners/planEventDispatch.js`** (amend — Part 38's file, since modified by Part 48) — add an `UpdateActionFields` branch to the `titleTemplate` if/else chain → stamps `action-fields-updated` directly + a default title (reuse `ACTION_FALLBACK_TITLE`, already `'{{ user.profile.name }} updated {{ action.title }}'`), plus the optional `comment` param the fold consumes; Part 33's `foldCommentIntoEvent` call handles the `comment` (if Part 33 hasn't landed, the param flows un-folded — either order works, per Part 33's contract). *(In-tree, `planEventDispatch` does not yet accept a `comment` param — only JSDoc references it; confirm/add it here.)*
