...
5. **Authors needing a different target status or event payload write a pre-hook routine** (inline on the action YAML, per [part 13 § Hook emission](../13-resolver-apis/design.md#hook-emission-replaces-the-build-time-auth-gate)). The resolver emits one hook Api per declared `{interaction}.pre` slot — incremental cost is one Api per overridden interaction.
6. **`mergeStatus` runtime-validates the pre-hook `status` return** against `action_statuses` and throws on miss. The throw fires at the merge step (after step 2 pre-hook invocation, before step 4 writes) so no writes have landed; the action stays in its pre-submit state and retry is naturally idempotent. Throw shape: `UserError` with `isReject: false` (default) — per [Part 29 § D5](../29-error-model-cleanup/design.md#d5-soft-reject-channel----reject-from-a-pre-hook-propagates-transparently), the wrapping endpoint's `runRoutine` classifies it as `{ status: 'error' }` and the calling app's `CallApi` sees a transient error. Not `:reject` — a misspelled status enum is a workflow-author bug, not user-facing validation the submitting user can fix.
...
| `request_changes` | "Send all the way back to draft" (write `action-required` instead of `changes-required`). Distinct semantics, but I cannot find a workflow where this is a static per-action decision rather than a per-incident reviewer judgment.                                                                                                                                                                              | Conditional on reviewer intent → pre-hook, not static. |
...
- **Build-time validation loss for `status` enum-membership.** Today `makeWorkflowsConfig` rejects a typo at build. Move the check to runtime in `mergeStatus.js` (change #6) — costs nothing at submit, prevents an invalid stage being written. Strictly a regression vs. build-time validation (typos ship and only fire when the interaction is exercised) but the runtime check still catches the bug before it corrupts the action doc.
...
- **Per-overridden-interaction Api count grows.** Each new overridden interaction adds one emitted hook Api (`update-action-{type}-{interaction}-pre`). Per § Use cases considered, real workflows are expected to override rarely — incremental Api count is small in practice.
...
| Part                                                                                                    | Status            | Change                                                                                                                                                                                                                                                          |
| ------------------------------------------------------------------------------------------------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Part 4 `makeWorkflowsConfig`](../_completed/04-workflow-config-schema/design.md)                       | shipped           | Drop `interactions:` and `event:` from the per-action schema. Drop their inline validators. Standard schema unknown-key rejection covers any accidental future use; no dedicated migration-message validator (no current users to migrate).                       |
| [Part 9 `hook-invocation`](../09-hook-invocation/design.md)                                             | in progress       | `resolveTargetStatus` drops the layer-2 branch; status resolver becomes 2-layer. `mergeEventOverrides` drops layer 2; event merge becomes 3-layer. `mergeStatus` adds runtime enum check on the pre-hook `status` return. Tests for the dropped layers go away. |
...
| Worked-example YAML in the demo app                                                                     | TBD               | Audit needed — § Use cases considered suggests the demo likely uses zero status overrides. Any `event:` blocks that exist port to pre-hook routines.                                                                                                            |

### Migration

No current users or implementers. No migration needed — drop the fields from the schema, update the demo worked example, update the concept spec. No data migration. No backwards-compatibility shim, no rejecting-validator-with-helpful-message machinery.

The hardest churn is **rewriting in-flight design documents** (parts 4, 9, 13 above) — none of which are sealed, all of which already cite "Layer 2" by that label. Each cross-reference needs to be re-checked.
...
- `makeWorkflowsConfig` no longer accepts `interactions:` or `event:` on an action (standard schema unknown-key rejection covers it).
...
- `mergeStatus` throws a `UserError(isReject: false)` on a pre-hook `status` return that is not a member of `action_statuses`. Test that no writes have landed when the throw fires (action's `status[0]` unchanged from pre-submit), and that the wrapping endpoint classifies it as `'error'` (not `'reject'`).
...
Same destination for the engine default (JS literal in `buildDefaultLogEventPayload`), the dropped Layer 2 path (YAML literal baked into endpoint properties), and the pre-hook path (`Return params.event_overrides`). All three produce equivalent stored event documents; bindings are resolved by the timeline block, not by the engine or by the routine that wrote the override. **No scope shim, no documented divergence, no migration concern.**

## Contract to neighbours

- **Part 4** drops two field definitions and their inline validators; updates the schema commit. No dedicated rejecting-validator.
- **Part 9** drops merge layers and tests; adds the runtime `status` enum check in `mergeStatus`.
