...
5. **Config validation** in `makeWorkflowsConfig` — `start_link` must be `{ pageId: string, urlQuery?: object }` with no other keys (in particular `title:`, familiar from custom-kind cell links, is not part of the engine-link shape and is rejected); `urlQuery` values must be strings, or `true` on exactly the `action_id` / `entity_id` keys (a `true` on any other key is an authoring error — it would silently ship the literal `true` into a URL; non-string statics like `count: 3` are rejected for the same reason).
...
In both cases the tracker row is where the user *sees* "this needs to happen" — `status_map.action-required` says "Create the installation ticket" — but the row offers no way to get there. `computeEngineLinks.js:66-77` emits a tracker link only when `child_workflow_id != null`, and that link points at the *child's* overview. Before the child exists: nothing.
...
| Read APIs | No change — Part 42's shared `resolve_action_link.yaml` surfaces `links.edit` for pre-child tracker rows; Part 44 adds a tracker-row case to its tests (see note below). |
...
**Read-side dependency (Parts 38 + 42).** `planActionTransition` persists the per-verb map `{slug}.links` (Part 34 D7), but the verb-selection step (per-verb `links` + `visible_verbs` → the one rendered `link`) **does not exist yet anywhere**: all three link-projecting read APIs (`get-entity-workflows.yaml:75-79`, `get-workflow-overview.yaml:51-55`, `get-action-group-overview.yaml:61-65`) still project the legacy singular `{slug}.link`, and the shared `visible_verbs.yaml` compute stage that `api/stages/visible_verbs_filter.yaml:16` refs is not on disk — [Part 38](../_completed/38-engine-rebuild/design.md)'s read side is still in flight. Verb selection is owned by [Part 42 D5](../_completed/42-timeline-action-cards/design.md): the shared `modules/shared/workflow/resolve_action_link.yaml` stage does the priority pick over non-null link cells ∩ `visible_verbs`, and all three read APIs adopt it. Part 44 therefore sequences **after** Part 38's read side and Part 42's `resolve_action_link.yaml` land, and needs **no read-API change of its own** — for a pre-child tracker the start link is the only link the engine emits, so the generic pick surfaces it. Part 44's read-side contribution is a tracker-row case in `resolve_action_link`'s tests.
