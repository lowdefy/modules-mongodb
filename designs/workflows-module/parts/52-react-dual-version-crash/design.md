...
**Layer:** workspace dependency pins + the experimental Lowdefy server/vite build (partly upstream). **Size:** S (one-line lever) but **cross-cutting** and coordination-dependent. **Repo:** `pnpm-workspace.yaml` / root `package.json`; the Lowdefy experimental server template (`lowdefy` package — upstream). Surfaces in `plugins/modules-mongodb-plugins/src/blocks/ActionSteps/`.
...
3. **The app pins React 18, the antd stack pulls React 19.** The generated server `package.json` declares `react`/`react-dom` `18.2.0` and `antd 6.3.1`. In the pnpm store the entire antd v6 ecosystem (`@ant-design/*`, `@rc-component/*`) plus `@lowdefy/block-utils`, `@lowdefy/blocks-antd@5.3.0`, `@lowdefy/blocks-basic`, `@lowdefy/plugin-aws` resolve against `react@19.2.6`. (There is also version sprawl: antd `6.3.1` *and* `6.4.3`, and `@lowdefy` packages at four versions — the `5.3.0` line is dragged in by the `@lowdefy/e2e-utils` dev dependency.)
4. **No dedupe in the generated vite config.** `apps/workflows-test/.lowdefy/server/vite.config.js` uses `@vitejs/plugin-react` with **no `resolve.dedupe: ['react','react-dom']` and no alias**, so nothing forces a single copy at bundle time.
5. **`ActionSteps` is correct.** `plugins/modules-mongodb-plugins/package.json` declares `react >=18`, `antd >=6` as **peer** deps and builds with `swc` per-file transpile (no bundling) — it relies on the host to provide one React/antd. The fault is install/resolution + bundling, not the block.
...
| App pins 18, antd is 6.3.1 | `…/.lowdefy/server/package.json` (`react`/`react-dom` `18.2.0`, `antd` `6.3.1`) |
| antd/rc + @lowdefy blocks on React 19 | pnpm store dir names `*_react@19.2.6*` for `@ant-design/*`, `@rc-component/*`, `@lowdefy/block-utils`, `blocks-antd@5.3.0`, `blocks-basic`, `plugin-aws` |
| No react dedupe | `…/.lowdefy/server/vite.config.js` (`plugins: [react()]`, no `resolve.dedupe`/alias) |
