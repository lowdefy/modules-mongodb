...
Today's merge in `plugins/modules-mongodb-plugins/src/connections/shared/mergeEventOverrides.js` is **one level deep** at `display` (`{ ...base.display, ...override.display }`) — an app-keyed override replaces the *whole app bucket* and silently drops the engine title. For the engine title + an author per-app title override + (task 3's) comment to coexist within one app bucket — and so an override targeting one app's `title` doesn't drop sibling engine-written keys — the merge must go two levels: `display → {app} → {title,description}` (design D7).
