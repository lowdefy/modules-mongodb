...
- [ ] **F12** — Timeline action order follows workflow order (currently events `date`-desc, cards by `sort_order` only within an event). **Superseded by [Part 54 — Workflow action ordering](../54-action-ordering/design.md)**, which found the same dead `sort_order` sort across all four read engines and generalized the fix (declaration-order comparator). `GetEventsTimeline.js`.
