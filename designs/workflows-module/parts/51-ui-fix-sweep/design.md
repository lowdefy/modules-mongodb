...
3. **Overview & action cards** — give the workflow-overview a fuller header, add gap and action-group grouping to the overview cards, fix the actions-on-entity header that wraps onto the steps, surface each action's identity (not just its status message), drive `ActionSteps` status colours from the shared enum, and fix the wrong "no data" empty states on done actions in the overview cards (F8, F9, F17, F20, F23, F26).
...
6. **Event message copy** — rework engine default event titles into per-signal verbs from human titles ("Sam Tolmay submitted Send Quote") instead of "marked send-quote as in-review" (F24); polish the demo/events create + lifecycle messages so events are distinguishable (F25).
...
### D4 — Surface action identity, don't rely on self-describing messages {#d4}

A blocked action shows only its `status_map` message ("Awaiting account manager assignment.") — which describes the *blocker*, never the action itself (F20). Action cards/rows display the message as their sole label; the action's own `description`/title is not surfaced. Fix by displaying the action title/`description` alongside the status message on cards, rows, and ActionSteps — so identity is always shown regardless of message text. (Alternative: require authors to write self-describing blocked messages — rejected; fragile and opt-in.) Related to F8 (the overview header is similarly thin on identity).
...
| F8 | Workflow-overview title too thin (just "Onboarding") | `workflow-overview.yaml:63-74` | module (D4) |
...
| F20 | Blocked action message doesn't identify the action | `kickoff-call.yaml:13-15`; cards show only the status message | module (D4) |
| F22 | Timeline cards navigate to page instead of opening modal | `apps/demo/pages/leads/lead-view.yaml:103-116` — `on_action_click` unwired, no modal on page | demo (+ design intent) |
| F23 | ActionSteps in-progress colour not distinct; use enum | `ActionSteps.js:18-27` hardcoded map | block (D3) |
| F24 | Default event messages barebones — per-signal verbs from titles | `planEventDispatch.js:13-23` (`DEFAULT_TITLES`) | engine + config (D6) |
...
- **F22 design intent** — should a standalone timeline default to modal-open or navigate? Today navigate is the default; the demo wants modal. Decide whether to change the default or just wire the demo.
- **F7 avatar** — confirm `GetEventsTimeline` is the place to join the user avatar `src` into each event's user.
- **F24 action title** — should an action `title` be **required** on every action config (consistent authoring, no fallbacks) or **optional** with a `type`-prettified fallback? Affects the manifest schema and every existing demo action config. *(User parked this to decide later.)*
