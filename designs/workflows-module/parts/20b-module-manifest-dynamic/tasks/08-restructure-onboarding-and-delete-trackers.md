...
- Adds the `g1.on_complete` callback that demonstrates the group-fan-out path (dormant until [part 11](../../11-group-on-complete-fanout/design.md) ships).
...
3. **Create `apps/demo/modules/workflows/workflow_config/onboarding/hooks/g1-on-complete.yaml`** — the `on_complete` routine for group `g1`. Authored as a demo-visible step (e.g. a log entry) so the callback fires observably once part 11 lands. Minimal shape:
   ...
   `makeWorkflowApis` emits this as `workflow-onboarding-group-g1-on-complete` Api per [makeWorkflowApis.js:108–122](../../../../../modules/workflows/resolvers/makeWorkflowApis.js).
...
- The `g1.on_complete` callback only fires once [part 11](../../11-group-on-complete-fanout/design.md) ships. Until then the Api is emitted but never invoked.
