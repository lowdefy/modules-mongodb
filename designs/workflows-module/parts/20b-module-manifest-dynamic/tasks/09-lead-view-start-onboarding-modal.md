...
2. **A modal block** elsewhere on the page (typically at the page root or alongside the workflows section). The modal contains:
   ...
   - A Submit button whose `onClick` action sequence:
     1. Calls `workflows/start-workflow` with payload:
        ...
        ```
        (Use the project's preferred operator idiom — see [CLAUDE.md "Operators before `_js`"](../../../../../../CLAUDE.md). If `_array.map` works for this shape, prefer it; fall back to `_js` only when operator chaining is unwieldy.)
...
- Avoid building the `actions:` payload via `_js` if `_array.map` over `_state: devices` produces an equivalent shape — see [CLAUDE.md "Operators before `_js`"](../../../../../../CLAUDE.md).
