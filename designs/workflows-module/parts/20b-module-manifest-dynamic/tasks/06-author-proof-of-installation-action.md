...
`proof-of-installation` is the keyed (instanced) action in the new `onboarding` worked example — `kind: form`, in group `g3` (blocked by `g2`), `key: $device_serial`. Per [action-authoring spec § Instanced actions](../../../../workflows-module-concept/action-authoring/spec.md), `key:` declares a symbolic placeholder; concrete values arrive at workflow-start time via the `actions:` payload on `start-workflow` — one action doc per `(workflow_id, type, key)` triple.
...
Per [action-authoring spec lines 337–355](../../../../workflows-module-concept/action-authoring/spec.md): with `key:`, action identity is `(workflow_id, type, key)`. Form data path is `form_data.{action_type}.{key}.{field}`. `key:` and `tracker:` are mutually exclusive.
