...
The `makeWorkflowApis` resolver ([modules/workflows/resolvers/makeWorkflowApis.js](../../../../../modules/workflows/resolvers/makeWorkflowApis.js)) reads inline `hooks.{interaction}.{phase}.routine` off the action and emits the Lowdefy Api at build time with id `update-action-{type}-{interaction}-{phase}`. The author writes the routine inline; to keep the action file readable, the routine *content* lives in a sibling YAML and is pulled in via `_ref`.
...
1. **Create `apps/demo/modules/workflows/workflow_config/onboarding/qualify.yaml`** — a `kind: form` action declaring:
   ...
   See the worked example in [design.md § Onboarding actions](../design.md#onboarding-actions-replaces-the-three-trackers) lines 97–123 for the canonical shape.
