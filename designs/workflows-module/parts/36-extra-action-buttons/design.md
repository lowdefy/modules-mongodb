...
> **Reconciled to the signal model (Parts 38/39).** This part predates the engine's move from the old *interaction → target-status* model to **signals + a per-kind FSM** ([state-machine](../../../workflows-module-concept/state-machine/design.md), engine in [Part 38](../38-engine-rebuild/design.md), templates in [Part 39](../39-form-submit-buttons/design.md)). The `buttons.extra` slot, the `_build.array.concat` wiring, the modal pattern, and the validator's structural checks are unchanged — only the vocabulary they're described in moves (per Part 39's "Part 36 — rebase notes"). The reserved-id list grows and is renamed to match Part 39's signal buttons.
...
3. **Validator pass.** `makeWorkflowsConfig.js` validates that `pages.{verb}.buttons.extra` is an array (when present), that each entry has a string `id` and `events.onClick` action array, and that no entry uses a reserved id matching a template-shipped signal button on that page. The reserved set (post-Part 39) is `button_submit` (renamed from `button_submit_edit`), `button_progress` (new — the Save Draft button), `button_not_required` (edit), `button_approve`, `button_request_changes` (review + view), `button_resolve_error` (error). The validator reads the reserved ids from a `RESERVED_BUTTON_IDS` constant in `makeWorkflowsConfig.js`. The same ids are also hardcoded as `id:` values in the verb templates (the `Button` blocks in `edit.yaml.njk` / `review.yaml.njk` / `error.yaml.njk` / `view.yaml.njk`), so the constant isn't a true single source — each new signal-button part touches both the template button block and the constant. For a six-id set that's acceptable duplication; collapsing it (e.g. resolver injects ids into each template as a build-time var) is heavier than the duplication is worth. (This mirrors the same hand-maintained-plus-guard tradeoff Part 39 D3 makes for the `button_signal_sources.yaml` source-stage map.)
...
- **Unit tests.**
  - `makeWorkflowsConfig.test.js` — add cases: (a) valid `buttons.extra` array passes; (b) non-array `buttons.extra` rejected; (c) entry missing `id` rejected; (d) entry missing `events.onClick` rejected; (e) entry with `id: button_submit` rejected on the `edit` page; (e2) entry with `id: button_progress` rejected on the `edit` page; (f) entry with `id: button_resolve_error` rejected on the `error` page; (g) `buttons.extra` on `view` rejected (extras slot not offered on view — see Out of scope).
