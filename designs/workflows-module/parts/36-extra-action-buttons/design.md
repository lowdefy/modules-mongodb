...
> **Reconciled to the signal model (Parts 38/39).** This part predates the engine's move from the old *interaction → target-status* model to **signals + a per-kind FSM** ([state-machine](../../../workflows-module-concept/state-machine/design.md), engine in [Part 38](../_completed/38-engine-rebuild/design.md), templates in [Part 39](../39-form-submit-buttons/design.md)). The `buttons.extra` slot, the `_build.array.concat` wiring, the modal pattern, and the validator's structural checks are unchanged — only the vocabulary they're described in moves (per Part 39's "Part 36 — rebase notes"). The reserved-id list grows and is renamed to match Part 39's signal buttons.
