...
| 6   | `06-e2e-help-button.md`         | E2E: Help-button assertion in the existing onboarding happy-path spec (deferred-verification)  | 3          |
...
**Task 6 (e2e)** adds the Help-button assertion to the existing `apps/demo/e2e/workflows/onboarding-happy-path.spec.js`, so it needs task 3. Self-owned — no Part 22 coordination (Part 22 is design-only in `parts/_next/` and its rebased plan keeps the example-workflow specs as tier 1, absorbing the line when it lands). The happy-path spec is itself deferred-verification (written ahead of Part 38 task 17 / Parts 43 / 44 and not yet runnable), so the assertion verifies nothing at ship time; tasks 2–3's build and render checks are the live verification.
