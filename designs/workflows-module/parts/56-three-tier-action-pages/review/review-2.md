...
The *real* `makeWorkflowsConfig` change this part needs is the **validation** Part 4 owns ("validate `slot` is a block ref"), via a new `validateEntityView`-style check alongside the existing `validateAction`/`validateWorkflow` validators. **Fix:** reword the Files-changed line — `makeWorkflowsConfig` gets a new `entity_view` validator (or defer wholly to Part 4), and the "strip" is a no-op `pick()` already performs. As written it implies an edit that isn't needed and omits the edit that is.
