...
[Part 2](../_completed/02-dynamic-module-pages/design.md) (or a parallel API-resolver channel), [part 6](../06-submit-action-writes/design.md) (so the endpoint payload shape matches what the handler accepts). [Part 4](../04-workflow-config-schema/design.md) validates the YAML this resolver consumes but its normalized output is not consumed here — the resolver reads `vars.workflows_config` directly.
