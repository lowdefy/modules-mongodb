...
**Text-only (design D6).** Every comment is text-only — inline images are disabled on all comment inputs (`properties.image.disabled: true`, `s3PostPolicyRequestId` unset), so `comment.fileList` is always empty and the callout in [Part 62](../../62-changes-requested-callout/design.md) never has to render an attachment. This is the generalisation of Part 62's request-changes-only text-only change to **every** comment surface, and it is why a single branch-free fragment is possible (no per-site `allow_images` var). Two consequences for this task:
