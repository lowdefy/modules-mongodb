...
- **`seedStage` mode (added by task 23; consumed by task 17's Start).** An optional input, mutually exclusive with `signal`, insert-only (no `action`; bypasses the `upsert` gate, which guards the signal path): skips `resolveSignal`, takes the declared status as the target stage, and runs every downstream step unchanged. This is how Start's direct-seeded drafts (`starting_actions` / payload `actions:` — legal seeds `action-required` | `blocked`, enforced in Start, not here) get the full composition without an FSM transition; creation at workflow start is not a transition (Part 45 review 1 #2).
