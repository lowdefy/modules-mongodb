...
10. `makeWorkflowApis.test.js` — emitted `hooks` map values are `{ '_module.endpointId': '<workflow>-<action>-<interaction>-<pre|post>' }` objects, not bare strings.
