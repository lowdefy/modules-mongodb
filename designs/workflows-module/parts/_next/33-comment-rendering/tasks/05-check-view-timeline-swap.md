# Task 5: Check view page — delete the bespoke comments card, add the shared events timeline

## Context

The check-action view page (`modules/workflows/pages/workflow-action-view.yaml`) renders a bespoke `comments_card`: a Card running its own `get_comment_events` aggregation (`$match: { action_ids: { $in: [...] }, metadata.comment: { $exists: true } }`) and a `comment_events_list` List rendering `metadata.comment` via an `Html` block (lines ~200–285).
...
- `modules/workflows/pages/workflow-action-view.yaml` — modify — delete `comments_card`, add `activity_card` + timeline `_ref`.
...
- Part 40 (check-action surfaces) is ordered after this part and carries the timeline `_ref` (not the comments card) into its shared surface — keep the addition self-contained at the page level here.
