...
Critical invariant (design D4): the comment is **raw user-typed HTML, stored verbatim, never templated**. The planner renders every string in the display tree as a Nunjucks template (`renderEventDisplay` → `renderTree` → `parseNunjucks`); the fold therefore runs *after* render (task 3), and this helper must pass template syntax (`{{`, `{%`) through untouched.
