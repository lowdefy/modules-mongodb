...
The Related section said "[Part 18 — actions-on-entity](../_completed/) (if archived)". Part 18 *is* archived (it lives at `_completed/18-entity-components/`), so the "(if archived)" hedge is stale, and the link pointed at the bare `_completed/` directory rather than the specific design.
