...
- Part 46 states: *"Part 40 now depends on Part 46 and is paused until this lands"* (line 76) and its button design is *"superseded"* (line 143).
- Part 40's own `design.md` still describes the full **client-side** path — `action_role_check` retained (`40/design.md:72`), `button_signal_sources.yaml` read at build time via `_ref` (`:13, :91`), client `_state` gating (`:100, :123`) — and references Part 46 only as a *future* reopener for per-action button config (`:127, :247`), not as a hard dependency it is paused behind.
