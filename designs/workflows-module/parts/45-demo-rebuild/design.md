...
11. **Part 38 task 20 is superseded** — its migrate-in-place scope is replaced by this rebuild; its non-config concerns (per-verb `action_allowed` template consumers, lifecycle notification policy) carry over here. `tasks/20-demo-migration.md` is reduced to a stub pointing at this part (done), stating the landing chain — Part 38 tasks 1–19 → Part 43 (`kind: check` + `action-*` pages) → Part 44 (`start_link`) → this part — so an implementer picking it up from Part 38's task list knows two other parts land first.
...
| `designs/workflows-module/parts/38-engine-rebuild/tasks/20-demo-migration.md` | Reduced to a stub pointing at this part with the landing chain (done); the `action_allowed` template-consumer and notification-policy items carry over here (items 9, 11). |
