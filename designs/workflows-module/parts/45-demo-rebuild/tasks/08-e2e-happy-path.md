...
3. Check off `site-visit` and `schedule-followup` (shared `action-edit` page); fill and submit `send-quote` → `in-review`. As admin, approve on `action-review` → `done`; the wired notification fires.
...
- **Step 3**: check off `site-visit` and `schedule-followup` via the shared `action-edit` page (click through their rows). Fill `send-quote` (`quote_total`, `notes`), submit → poll `in-review`. Approve via `action-review` → poll `done`. **Notification assertion**: poll `mdb.collection('notifications')` for one doc with `event_type`/source `action-approve` for the session user's contact id (see task 6's recipient).
...
- Prefer navigating via rendered links/rows over hard-coded generated page ids (`workflow-onboarding-qualify-edit`-style URLs) — Part 38 task 18 / Part 43 renamed page surfaces, and click-through is what a user does; the worked example is written that way.
