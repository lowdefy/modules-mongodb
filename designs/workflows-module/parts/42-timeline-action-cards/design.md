...
6. Adopt the same `resolve_action_link.yaml` stage in the workflow read APIs (`get-entity-workflows`, `workflow-group-overview`) so actions-on-entity and the group overview render the **identical** access-aware link — read-side link selection is computed once, server-side, for every surface (D5). The now-superseded "the UI applies the per-verb selection rule" prose is dropped from Part 38; its write contract (the per-verb `links` map) is unchanged.
...
**Reused by every surface.** `get-entity-workflows` (actions-on-entity) and `workflow-group-overview` already run `visible_verbs_filter`; they `_ref` `resolve_action_link.yaml` to project the same `link`. So the timeline card, the entity widget, and the group overview render an identical, access-correct link from one implementation.

**This supersedes Part 38's "UI applies the per-verb selection rule."** Part 38 still *writes* the per-verb `links` map (engine/write concern, unchanged); *which* link renders is a read-side display concern that moves here. Part 38's selection-rule prose (its lines 359, 362, 406, 757) is dropped, leaving the engine to own the map and the display layer to own resolution.
...
| `modules/workflows/api/get-entity-workflows.yaml`, `modules/workflows/pages/workflow-group-overview.yaml` | Adopt `resolve_action_link.yaml` so actions-on-entity / the group overview render the identical server-resolved link (D5). |
| `designs/workflows-module/parts/38-engine-rebuild/design.md` | Drop the read-side "UI applies the per-verb selection rule" prose (lines 359, 362, 406, 757); engine keeps writing the per-verb `links` map (D5). **Part 38 already has generated tasks — re-check them for any UI-selection task that this removes.** |
