...
- **No stale "deferred timeline port / accepted debt" language.** Every remaining "deferred"/"follow-up" mention now scopes to *task-specific* timeline auth/links (the legitimate tasks-module deferral, lines 82/142/185), not the port itself. D6 title and body both say "zero stragglers."
...
- **Scope note (checked, not an issue):** the events-timeline lookup is now in this part's deletion scope but is absent from the "Current state — the readers (verified)" table. This is intentional — that table is scoped to *client config-embed readers*, and `todo-discuss` item F explicitly reasoned that the events-timeline YAML "isn't a config-embed reader" (it already resolves access server-side). The deletions section, D6, and Ripples cover it. Adding a row would expand the table's established scope, so no change.
