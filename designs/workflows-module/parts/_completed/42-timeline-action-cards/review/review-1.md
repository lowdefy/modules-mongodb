...
Post-Part-38 the per-verb `links` map is collapsed to one link *against the user's `visible_verbs`* — the workflow read APIs run `visible_verbs_filter.yaml` and the UI selects among verbs the user actually has ([Part 38 line 359, 408](../38-engine-rebuild/design.md)). The events-timeline pipeline runs only the reference `$match` then the fragment — no `visible_verbs` resolution. So a naive `edit > review > error > view` pick in the fragment can surface an `edit` link to a user who only has `view`, routing them to a page they can't load.
