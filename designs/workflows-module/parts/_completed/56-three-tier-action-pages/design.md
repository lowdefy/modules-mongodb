...
> **⚠️ Layout revised — see [`design-addendum-layout.md`](design-addendum-layout.md).** After reviewing the workspace against real content, the **placement of universal fields and the composition of the right column + action bar** changed. The addendum is authoritative for the layout; this document's structure (full-width header, three columns, per-workflow check page, engine/envelope/config work) stands. In short: universal fields **leave the RHS** — assignees + due → title-bar chips, description → a middle-column callout, edit → a modal; the RHS **drops its tabs** (entity Details stacked above History); the action bar becomes a **floating sticky card** in the middle column. This **supersedes D2, amends D7, partly reverses D8, and revises the shell spec** (each flagged inline below).

> **⚠️ Deviation (Part 65 addendum) — check page now navigates on terminal signals.** The per-workflow check page (`action.yaml.njk`) originally refetched in place after every signal and never navigated. That only made sense for the in-context modal (`check-action-surface.yaml`), which sits on the entity page and has nowhere to link; on a standalone deep-link page it read as broken and diverged from the form pages. The check page now returns to the entity on a successful **terminal** signal (`submit`, `not_required`, `approve`, `request_changes`, `resolve_error`) — only `progress` (Mark Started) still refetches in place. All terminal navigation (check **and** form pages) now routes through the shared `components/return-to-entity.yaml` (success toast + `Link` to `entity_link`, browser-back fallback). The modal is unchanged (closes instead of navigating).
...
_[Amended by addendum DA4: the form/middle split by kind is kept, but the RHS Details is now a **stacked section** above History, not a tab — the RHS `Tabs` wrapper is removed.]_
...
_[Partly reversed by addendum DA1/DA5: assignees + due date now **do** render in the header, as `page_actions` chips (with an `✎` → edit modal). The rest of D8 — subtitle = `message`, status pill on the left, full-content-width title bar, header-as-native-chrome — is unchanged.]_
...
_[Revised by addendum DA4/DA5: the `universal_fields` slot is removed (fields leave the RHS), an `actions` slot is added (floating sticky action card), and the RHS `Tabs` wrapper becomes a stacked Details section + History section. The diagrams and slot list below describe the pre-addendum shape; see the addendum for the current one.]_
