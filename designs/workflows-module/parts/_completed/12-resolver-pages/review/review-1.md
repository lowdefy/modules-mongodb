...
- The shipped resolver at [modules/workflows/resolvers/makeWorkflowsConfig.js:18](modules-mongodb/modules/workflows/resolvers/makeWorkflowsConfig.js) confirms: `WORKFLOW_FIELDS = ['type', 'entity_type', 'display_order', 'starting_actions', 'action_groups']`.
...
> **Resolved.** Added a clarifying paragraph under the vars list in [part 12 design.md](modules-mongodb/designs/workflows-module/parts/_completed/12-resolver-pages/design.md) — `entity_id` is not a build-time var; it resolves at runtime from the `?action_id=` URL query via `get_action.workflow_id → get_workflow.entity_id`.
...
Part 12 (line 14) reads the **normalized** config from part 4. But part 4 strips `pages`, `hooks`, `interactions`, `event`, `form`, `form_review`, `form_error` (see [makeWorkflowsConfig.js:1–5](modules-mongodb/modules/workflows/resolvers/makeWorkflowsConfig.js) comment: "Build-time-only fields … are excluded — they're consumed by build-time resolvers against the raw workflow YAML").
...
> **Resolved.** Added an explicit note in [part 12 design.md](modules-mongodb/designs/workflows-module/parts/_completed/12-resolver-pages/design.md) that the page shell carries context vars only — `events.onInit`, `requests:`, and the `get_action` request `_ref` live inside the template (part 16), not the shell.
...
> **Resolved.** Updated [part 12 design.md](modules-mongodb/designs/workflows-module/parts/_completed/12-resolver-pages/design.md) — `page_ids` only includes keys for emitted verbs. Templates guard sibling references with `_if page_ids.{verb} is defined`. The map never points at a non-existent page id.
...
> **Resolved.** Added an explicit build-time validation entry in [part 12 design.md](modules-mongodb/designs/workflows-module/parts/_completed/12-resolver-pages/design.md) — `vars.app_name` must be non-empty; missing/null/`""` fails the build. Defense in depth with part 20's manifest-level `required: true`.
...
> **Resolved.** Added a "Placeholder templates" section to [part 12 design.md](modules-mongodb/designs/workflows-module/parts/_completed/12-resolver-pages/design.md) — part 12 ships four stub `.yaml.njk` files (replaced in part 16). This also dissolves the design's outstanding open question about the form stub (the placeholder `edit.yaml.njk` is the stub), which was removed.
