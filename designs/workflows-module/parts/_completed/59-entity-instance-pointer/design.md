...
- `WorkflowAPI/GetWorkflowAction/GetWorkflowAction.js` — `entity_link` urlQuery uses `action.entity.id`; **response** returns `entity: { connection_id: action.entity?.connection_id ?? null, id: action.entity?.id ?? null }` in place of the flat `entity_id` / `entity_collection` fields. Note: only `entity.id` has a consumer (`requests/get_entity.yaml.njk` reads `get_workflow_action.entity.id`); the response `connection_id` has none — the template sources its connection id from a build-time var threaded by `makeActionPages`, not the response. It is carried for **shape-symmetry** with the document/denorm pointer (and because the pre-existing flat response already returned `entity_collection`, so this is a 1:1 rename, not new surface), not because anything reads it.
...
- **Comment-only sweep** — three engine files outside the behavioral list carry stale flat-shape references in doc comments only: `shared/errors.js` (Start's `entity_id`/`entity_collection` param list), `shared/phases/runTrackerCascade.js` and `shared/phases/planners/planTrackerLevel.js` (both name `child_entity_id`/`child_entity_collection` in `payload.fields` doc comments). Update these comments to the nested shape; no behavioral change. (In-file comments within already-listed files are updated alongside their code.)
...
- Doc sweep — `docs/workflows/{concepts,how-to}/*.md` and reference pages that show the document shape or the `{ entity_collection, entity_id }` query update to the nested `entity: { connection_id, id }` shape. Two `how-to/` files carry this Part's runtime/document shape: `how-to/write-a-hook.md` (three `_payload: context.workflow.entity_id` reads → `context.workflow.entity.id`) and `how-to/track-a-child-workflow.md` (a start-caller payload with flat `entity_id`/`entity_collection`, the `child_entity_id`/`child_entity_collection` field references → `child_entity: { connection_id, id }`, and the prerequisite prose — but its `entity_id: true` sentinel stays flat per [Where "uniform" stops](#where-uniform-stops)).
...
- `apps/demo/api/leads-create.yaml` — the `start_onboarding` call to the generated `onboarding-start` endpoint maps `entity_id: { _payload: _id }` + `entity_collection: leads-collection`; drop the connection id and nest `entity: { id: { _payload: _id } }`. (A server-side API start caller — listed explicitly so the page-YAML sweep doesn't miss it.)
...
- `apps/workflows-test/` — workflow*config start callers, `get-entity-workflows` callers, and the e2e specs + `workflowFixture.js` that seed or assert documents with `entity_collection` / `entity_id` → nested `entity: { connection_id, id }` shape. \*\*The workflow_config \_definitions* themselves (the nine files' top-level `entity_collection`/`entity_ref_key` and the app's `vars.entities` map) are config-shape and migrate under Part 57\*\*, not here — Part 57's updated `validateWorkflow` requires the nested `entity:` block, so Part 57 owns folding those flat fields into it. This part touches only the runtime callers above. (Both parts land in sequence — see [Relationship to Part 57](#relationship-to-part-57-dependency--boundary) — so the workflows-test app builds clean once both are applied.)
