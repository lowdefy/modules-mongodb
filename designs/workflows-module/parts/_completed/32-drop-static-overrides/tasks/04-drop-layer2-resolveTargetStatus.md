...
3. **Add runtime enum check on `preHookStatus`.** If `preHookStatus !== undefined && !ACTION_STATUSES.includes(preHookStatus)`, throw a `UserError` with `isReject: false` and a message naming the bad status and the action type. Place the check inside `resolveTargetStatus` (cleaner than spreading it across the handler).
   - **Deviation from original plan re. `ACTION_STATUSES`:** the task originally picked option (a) — export from `modules/workflows/resolvers/makeWorkflowsConfig.js` and import here. In practice this doesn't work: the plugin (`@lowdefy/modules-mongodb-plugins`) and the workflows module (`@lowdefy/modules-mongodb-workflows`) are separate workspace packages with no dep between them, no precedent for cross-package imports anywhere in the repo, and the plugin's `swc src → dist` build wouldn't include the cross-package file. Shipped option (b) instead: duplicated the constant locally in `resolveTargetStatus.js` with a head comment naming `makeWorkflowsConfig.js` as the canonical source. If the canonical enum changes, both files need updating (grep finds them).
...
- `ACTION_STATUSES` is mirrored locally in `resolveTargetStatus.js` with a head comment pointing back to `makeWorkflowsConfig.js` as the canonical schema source. Cross-package import was attempted and rejected — see § Task step 3's deviation note.
...
- `modules/workflows/resolvers/makeWorkflowsConfig.js` — **unchanged** in the shipped implementation (the original task plan called for adding `export` to `ACTION_STATUSES`, but the import on the plugin side wasn't viable; see § Task step 3's deviation note).
