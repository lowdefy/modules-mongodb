...
**Source rationale:** [workflows-module-concept/review/review-claude-1.md](review-fable-1.md) § Doc corrections (verified contradiction + resolution); supersedes the `request_changes` row of [Part 34 D6](../34-action-access-model/design.md) (deviation noted here — Part 34 is read-only history). **Layer:** engine handlers + concept docs. **Size:** S. **Repo:** `plugins/modules-mongodb-plugins/src/connections/shared/` (`phases/` + `render/`) + `designs/workflows-module-concept/`.
