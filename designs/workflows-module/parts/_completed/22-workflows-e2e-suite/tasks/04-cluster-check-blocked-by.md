...
- `blocked_by` semantics: an action's `blocked_by` entries may name an action **type** or an action-**group id** (groups are blocked*by \_targets*, never carriers — see the comment in `apps/demo/modules/workflows/workflow_config/onboarding/onboarding.yaml`). The unblock fixpoint logic itself is unit-owned (`planAutoUnblock.test.js`); this spec proves it fires through the wired app.
