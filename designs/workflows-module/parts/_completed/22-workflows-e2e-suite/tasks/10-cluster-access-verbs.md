...
- Multi-**app** verb-filter coverage (role unions across `app_name`s) is explicitly deferred by the design — one `app_name` only. This cluster is multi-_role_ within `test`.
