...
API call shape — the demo uses `CallApi` (the [part 01](../../01-call-api-primitive/design.md) primitive has shipped). Endpoints to call:
...
- Use `Button` with the `CallApi` action type ([part 01](../../01-call-api-primitive/design.md) has shipped). The design notes 20a does NOT block on part 01 for its core deliverable, but since the primitive is now available, the demo wiring can use it directly.
