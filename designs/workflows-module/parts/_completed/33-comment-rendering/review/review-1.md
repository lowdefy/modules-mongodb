...
- **`mergeEventOverrides` merges only one level deep at the `display` top level**
  ...
  `mergeEventOverrides` has already dropped `title` _before_ `foldCommentIntoEvent`
...
`metadata.comment` write (D2) makes all four fail. The Verification section lists _new_
...
> **Resolved.** Folded into #1. D4 rewritten: the static-description channel is the `event.{interaction}.display` override baked by Part 13's `emitEventOverrides`, **not** "Part 32 Layer 2" (Part 32 dropped the static _status_ layer, a different mechanism).
