...
- _"YAML override on metadata.foo does NOT clobber default metadata.comment
  (regression: layer 3 folded into layer 1)"_ — asserts `metadata.comment` survives a
  YAML override; migrates naturally onto `display.{app}.description` + the fold.
- _"pre-hook override on metadata.comment overrides layer-1 runtime comment"_ —
  asserts a **pre-hook can override the runtime comment**. D4 inverts this on
  purpose: the fold runs _after_ the merge (and render), so post-Part-33 a pre-hook
