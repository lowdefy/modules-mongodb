...
- Part 56 (+ its layout addendum) is where this renders: a conditional **description render** already sits at the top of the middle column (addendum DA2). After [Part 64](designs/workflows-module/parts/_completed/64-action-description/design.md) it is the small reusable fragment `action-description.yaml` (was `universal-fields-callout.yaml`, deleted by Part 64) `_ref`'d individually by each template; `action-workspace.yaml` is layout-only and renders whatever `middle` array the template passes, so there is no single shared composition file.
