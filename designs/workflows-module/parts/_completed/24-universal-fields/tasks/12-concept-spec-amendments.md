...
- `action-authoring/spec.md` — "Universal action fields" section (line ~169) documents the three fields and states "Updates flow through the per-action endpoint's `fields:` payload block… Atomic with the status transition (same Mongo `$set`)." — now true only for `kind: check`. `universal_fields_required` appears nowhere (the review-proposed flag was never added — nothing to remove, just confirm).
...
2. **`designs/workflows-module-concept/engine/spec.md`**:
   ...
   - Amend the universal-fields capability bullet (~197) to the kind-based contract: the `SubmitWorkflowAction` fields merge applies to `kind: check` only; `kind: form` universal fields are owned exclusively by `UpdateActionFields`.
