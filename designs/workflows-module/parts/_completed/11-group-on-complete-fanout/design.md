...
> - **Originating workflow only — the "Extends `fireTrackerSubscription`" scope is NOT implemented.** `runTrackerCascade` does not compute `completedGroups` per parent level, so a group that completes on a *parent* workflow via tracker propagation does **not** fire its `on_complete`. Only groups that flip to `done` on the submitted workflow fire. Documented as a limitation in `docs/workflows/concepts/groups-and-blocking.md`; the "Open questions" auth-context problem for parent-level fires is therefore moot for now. Fold parent-level fan-out back in if a concrete need appears (it would also need to resolve that auth question).
