...
Failure context lives where every other status-change context lives: on the `events` collection entry written by step 7. Pre-hooks that want to capture diagnostics on an `error` push do so via `event_overrides.metadata` on the return — same channel for all other status pushes ([Part 09 § event_overrides](../09-hook-invocation/design.md#pre-hook-return-merge)).
...
- Symmetric with the rest of the platform. A reject from a workflows pre-hook surfaces the same way a reject from any other API routine does — and now propagates correctly across nested routines for _all_ Lowdefy apps, not just workflows.
...
**Status — shipped.** The required Lowdefy change landed as commit `cc18b41e9` ("feat(api): Propagate :reject status across runRoutine throw boundary."). Remaining in-repo work is to pin the SDK version this monorepo consumes to a release that includes `cc18b41e9`. The original problem statement and PR scope are preserved below for context.

D5's soft-reject channel was unimplementable without the upstream change — `controlReject.js` and `controlThrow.js` both constructed identical `new UserError(message, { cause })`; `callRequestResolver.js` threw the same `error` object for both; and `runRoutine.js`'s outer catch unconditionally labelled every caught throw as `{ status: 'error' }`. So by the time a `:reject` had crossed any routine boundary as a throw, the discriminator was gone.

Hand-off scope (single PR against the Lowdefy repo — shipped as `cc18b41e9`):
...
- **`packages/api/src/routes/endpoints/runRoutine.js`** (lines 56-62) — rewrite the catch to branch on `isReject` _before_ `context.handleError` runs, so propagated rejects bypass `handleError` (preserving today's invariant: rejects never hit `handleError`) and only errors trigger it:
...
- **Duplicate-write windows widen.** Per the D1 table, any step ≥ 7 throw plus retry can double-write the events-log entry (step 7); any step ≥ 8 throw plus retry can double-fire notifications (step 8). Same risk class as today (Mongo transactions wouldn't help for the cross-API steps), and the existing acceptance applies: the `events` collection is append-only audit and the UI tolerates duplicates. Notifications idempotency is a contract concern of the notifications module, not Part 29 — the notifications module's `send-notification` API delegates the whole routine to a consumer-supplied `send_routine` (module var), and that routine is expected to dedupe on the `event_ids` it receives. The implementation we use today honours that contract, so a step ≥ 8 throw + retry resolves to a single user-visible notification and the user-facing blast radius is bounded. A consumer whose `send_routine` does not dedupe would double-send on retry; that's a notifications-module-contract concern, out of scope for this part.

The pre-hook `hook_error` removal _does_ affect part 9 (deletes the merge-and-write-error-transition branch from `invokePreHook.js`'s merge logic). Post-hook handling in part 9 also changes — `invokePostHook.js` no longer wraps the callApi in try/catch; thrown errors propagate.
...
- **`handleSubmit.js`** — remove the `try`/catch around steps 4–11 ([lines 302-333](../../../../plugins/modules-mongodb-plugins/src/connections/WorkflowAPI/SubmitWorkflowAction/handleSubmit.js)) that builds an `errorTransition` object and force-pushes a `{ stage: 'error' }` status entry. Let throws propagate. The `errorTransition` was only ever a _return-field_ surface — shipped `updateAction` always wrote a uniform `{ stage, event_id, created }` status entry (per D2a).
...
- **Status-selector inclusion of `error`.** Whether `error` shows up in a task action's status-selector dropdown ([Part 06 § Interaction → target-status mapping](../06-submit-action-writes/design.md#interaction--target-status-mapping-engine-default-only)) is an authoring decision (`task.statuses:` list). Default behaviour: not included; authors opt in.
