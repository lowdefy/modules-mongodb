...
- The module manifest exports five static pages (`module.lowdefy.yaml:154–160`): the three `workflow-action-*` check pages **plus `workflow-overview` and `workflow-group-overview`**. No cluster renders the two overview pages; `operational-lifecycle` hits the overview *APIs* headless (Tail) only.
