...
1. **The build → wire → serve seam.** The build-time resolvers ([`makeWorkflowApis`](../_completed/13-resolver-apis/design.md), [`makeActionPages`](../_completed/12-resolver-pages/design.md), `makeActionFormConfigs`) _emit_ endpoints and pages; the module manifest _wires_ the `WorkflowAPI` connection; the host app's `vars.yaml` _feeds_ `workflows_config` / `entities` / `app_name` / `user_schema`. Nothing proves the emitted `{type}-{action}-submit` endpoint is reachable over HTTP in a running app, that the shared action pages render, or that the connection is configured end-to-end.
...
- **`workflow_config/`** holds the cluster workflows below, each `_ref`'d from `workflow_config/workflows.yaml`. Authored in current vocabulary (`kind: check` / `form` / `tracker`; shared `workflow-action-edit` / `-view` / `-review` pages; per-verb `access`).
...
| `check-blocked-by`      | `check-blocked-by.spec.js`      | Check actions with a type dep and a group-id dep; completing the blocker fires engine `unblock` and the dependent becomes action-required.                                                                                                                              | Spine            |
...
| `field-gallery`         | `field-gallery.spec.js`         | One field per component in `modules/workflows/components/fields/`: rendering, `required`/`minItems` validation, `form_data` persistence, and the read-only review/error render variants.                                                                                | Spine (UI-heavy) |
| `operational-lifecycle` | `operational-lifecycle.spec.js` | `start` / `cancel` / `close` / `get-entity-workflows` / `get-workflow-overview` end-to-end through the real APIs; close sweep skips `required_after_close: true`; already-completed close is a no-op; already-cancelled close rejects.                                  | Tail             |
| `access-verbs`          | `access-verbs.spec.js`          | Per-verb button and page visibility under different roles: `actions-on-entity` renders per `visible_verbs`, buttons gate on the per-verb `action_allowed` bag, role missing a signal's verb is rejected at the endpoint.                                                | Spine (UI)       |
...
- **CAS conflict / retry-no-double-transition** → `SubmitWorkflowAction.test.js` + `commitPlan.test.js`. (See salvaged technique below for the one e2e touch.)
...
Three things from the prior version are genuinely valuable and carry forward:
...
- **The CAS-conflict technique.** To exercise the conflict path without injecting a fake throw, drive a **genuine CAS conflict with two concurrent submits** ([part 38 D15](../_completed/38-engine-rebuild/design.md) made it deterministic). The old `transient-throw-retry` intent survives as a single tail assertion in `operational-lifecycle` or `cascade-keyed`: concurrent submits → one wins, the other surfaces a retryable `concurrent_submit`, retry converges with exactly one status entry. (The deep version already lives in `SubmitWorkflowAction.test.js`; the e2e touch only confirms it surfaces through the real endpoint.)
...
- Every emitted surface is proven reachable in the running app: each action page renders, each `{type}-{action}-submit` endpoint is callable and role-gated, each operational API returns its documented shape.
...
- **Test-app name/location.** `apps/workflows-test/` proposed; confirm against any repo naming convention.
...
- **Every shipping part (5–20, 23, 38–40, 43)** keeps its single Verification line pointing here for end-to-end coverage; its own verification stays unit-tests + handler-level integration.
