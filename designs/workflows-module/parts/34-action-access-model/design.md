...
The change is foundational. It touches the action grammar (concept spec), the engine's submit gate, the resolver's build-time page filter, the query-time filter on `get-entity-workflows`, the link computation in [Part 30](../_rejected/30-status-map-rendering/design.md) (which today reads "slug's verbs" as a flat list), and the client-side `action_role_check` from [Part 18](../_completed/18-entity-components/design.md). It also unblocks downstream work in Part 24 (universal-fields surface) where the question "can a triage user update metadata without form-submit rights" needs an answer that the current binary role gate can't give.
