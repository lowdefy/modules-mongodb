...
- **Plugin dual-runtime build complexity.** SWC building both client-side React blocks and server-side Mongo handlers from one package may surface tooling issues. Mitigation: split `src/blocks/` and `src/connections/` clearly; verify the SWC config emits clean entry points before declaring the plugin done. ([engine](engine/design.md) sub-design.)
