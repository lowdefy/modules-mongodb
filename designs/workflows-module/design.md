...
| 22  | [workflows-e2e-suite](designs/workflows-module/parts/_completed/22-workflows-e2e-suite/design.md)   | [concept § Worked example](../workflows-module-concept/design.md#worked-example--end-to-end-across-all-seven-sub-designs)                                                                 | M    |
...
| 28  | [custom-action-kind](designs/workflows-module/parts/_completed/28-custom-action-kind/design.md)     | [action-authoring spec § Action kinds](../workflows-module-concept/action-authoring/spec.md) + [submit-pipeline spec](../workflows-module-concept/submit-pipeline/spec.md)                | S    |
| 36  | [extra-action-buttons](designs/workflows-module/parts/_completed/36-extra-action-buttons/design.md) | [action-authoring § Decision 8 — Per-page chrome](../workflows-module-concept/action-authoring/design.md) + [ui § Decision 4 — Why fixed names](../workflows-module-concept/ui/design.md) | S    |
...
| 36  | extra-action-buttons      | 16, 4, 39, 46/48; after 56, 57                 |
...
- **Part 56** — Three-tier action pages: wraps every resolver-emitted action surface in a shared three-column "entity workspace" shell (left `actions-on-entity`, middle the action surface, right `Tabs` of a read-only `entity_view` Details slot + the `workflows-events-timeline` History). Adds an `entity_view: { reference_field, slot }` field to the workflow config (read-only, `_ref`-able, baked at build via `_var`, stripped from the materialised engine config). Promotes check actions from the shared `workflow-action-*` pages to a per-workflow `{workflow_type}-check` page that derives mode like `check-action-modal` (Option A, single-page variant); `computeEngineLinks` check branch retargets accordingly (one contained change). [Part 55](parts/55-check-modal-page-ownership/design.md)'s no-modal degrade path handles left-panel navigation for check rows. Amends shipped parts 16/17/40; reuses 24/42/46/50. See [parts/56-three-tier-action-pages/design.md](designs/workflows-module/parts/_completed/56-three-tier-action-pages/design.md).
- **Part 36** — Extra action buttons: opens the form-action floating-actions bar to author-supplied buttons via a `pages.{verb}.buttons.extra:` array (full Lowdefy `Button` blocks), concatenated after the template-shipped signal buttons across all four bar verbs (`edit`/`view`/`review`/`error`). The engine's _signal vocabulary_ stays locked (new signals remain deliberate engine-side parts); only the bar's _composition_ opens. Spun out rather than threaded through an active part because it spans four shipped templates, the `makeWorkflowsConfig` validator (a `RESERVED_BUTTON_IDS` guard against id collisions), both concept designs, the authoring-grammar reference, and a demo exercise. S. Depends on shipped Parts 16, 4, 39, 46/48; sequences **after** Parts 56 + 57, which reshape the templates and entity config it builds on. See [parts/36-extra-action-buttons/design.md](designs/workflows-module/parts/_completed/36-extra-action-buttons/design.md).
...
- **No unit tests in `apps/demo/`.** The Lowdefy app is YAML consumed by the runtime; coverage is Playwright e2e via [part 22](designs/workflows-module/parts/_completed/22-workflows-e2e-suite/design.md).
