...
> **Revised by review-3 #5.** Previously the wrapper let consumers _replace_ the whole form via `form_blocks`. That contract mismatched the rest of the contacts module (`form_profile.yaml` / `fields.profile` module var, etc.) where forms expose an extension slot fed through `_build.array.concat`. Renamed to `form_extra_blocks` and switched to append-only. Consumers who want a radically different form fork the file or call the block directly — no longer first-class.
