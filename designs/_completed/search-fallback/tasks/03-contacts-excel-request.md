...
- `properties.pipeline` is already a `_build.array.concat` at the root, splicing `request_stages.get_all_contacts` between the query stages and `$skip`/`$limit`. **That stays** — it is the shape task 2 established for every converted request, and this file is the precedent for it: line 74 already holds a runtime `_if` returning a `$sort` stage inside that build-time concat, which is exactly how the builder's gated pieces sit.
...
- Its `$addFields` combines the `score` projection with the `updated_at`/`created_at` derived fields in one stage, so the score projection has to be split out.

## Task

Rewrite `modules/contacts/requests/get_contact_excel_data.yaml`'s `properties.pipeline`, keeping the root `_build.array.concat` and leaving `id`, `type`, `connectionId`, and `payload` (including the `~ignoreBuildChecks` marker) untouched. Target shape:
             ...
             - _array.filter:
                 ...
                 - _function:
                     __ne:
                       ...
                       - null
   ```
   Use the exact same `paths` list and clause set as `get_all_contacts` — the two requests must agree on what "a visible contact matching the search" means.
3. `_ref` `../shared/search/score_stage.yaml` (same vars) — the `score` projection moves **out** of the combined `$addFields` and into the shared gated stage.
       ...
       then:
         ...
         $sort:
           _object.assign:
             ...
             - _id: 1
   ```
5. `- _module.var: request_stages.get_all_contacts` — the consumer splice point, unchanged, in the same position it occupies today (after the query/sort stages, before pagination).
6. A literal group with `$skip` and `$limit`, unchanged.

## Acceptance Criteria

- `properties.pipeline` keeps its root `_build.array.concat`, now splicing the two builder refs alongside the existing groups.
...
- `modules/contacts/requests/get_contact_excel_data.yaml` — modify — filters → `$match` `$and`, text + score + sort gate via the shared builder, score projection split out of the combined `$addFields`.
