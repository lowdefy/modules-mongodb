...
**Source rationale:** [workflows-module-concept/design.md § Worked example](designs/workflows-module-concept/design.md#worked-example--end-to-end-across-all-seven-sub-designs). **Layer:** integration verification + dedicated test-app fixture config. **Repo:** new `apps/workflows-test/` (app + `e2e/`), with the curated happy-path smoke staying in `apps/demo/e2e/`.
...
1. **The build → wire → serve seam.** The build-time resolvers ([`makeWorkflowApis`](designs/workflows-module/parts/_completed/13-resolver-apis/design.md), [`makeActionPages`](designs/workflows-module/parts/_completed/12-resolver-pages/design.md)) _emit_ endpoints and pages (and the form metadata the pages consume); the module manifest _wires_ the `WorkflowAPI` connection; the host app's `vars.yaml` _feeds_ `workflows_config` / `entities` / `app_name` / `user_schema`. Nothing proves the emitted `{type}-{action}-submit` endpoint is reachable over HTTP in a running app, that the action pages render — both surfaces: the **per-action form pages** `makeActionPages` emits (one per form action × verb, id `{type}-{action}-{verb}`, from `templates/{verb}.yaml.njk`; non-form kinds are skipped) and the **static shared check pages** (`workflow-action-edit` / `-view` / `-review`, addressed by `?action_id=`, serving `kind: check`) — or that the connection is configured end-to-end.
...
4. **Engine correctness is the unit/integration layer's job — and stays open.** The boundary is not a wall: per the top-level [E2E vs. unit split](designs/workflows-module/design.md#testing-conventions), a bug that could exist in plugin JS without the runtime needs a _unit_ test. If authoring this suite surfaces such a gap, **add the jest test** rather than contorting Playwright to reach it.
...
- `workflow.start({ workflow_type, entity, ...overrides })` → `{ workflow_id, action_ids }` — drives the operational `start-workflow` API ([part 19](designs/workflows-module/parts/_completed/19-operational-apis/design.md)).
- `workflow.submit(action_id, { signal, fields?, form?, form_review?, comment? })` — drives the per-action `{type}-{action}-submit` endpoint ([part 13](designs/workflows-module/parts/_completed/13-resolver-apis/design.md)). `signal` is the wire field ([part 38](designs/workflows-module/parts/_completed/38-engine-rebuild/design.md)); there is no `interaction`, `target_status`, or `force`.
...
The module surface this suite drives: [part 38](designs/workflows-module/parts/_completed/38-engine-rebuild/design.md) (rebuilt handlers, FSM tables, per-verb access), [parts 39](designs/workflows-module/parts/_completed/39-form-submit-buttons/design.md)/[40](designs/workflows-module/parts/_completed/40-simple-action-surfaces/design.md) (signal-firing button bars), [part 43](designs/workflows-module/parts/_completed/43-rename-simple-kind-to-check/design.md) (`kind: check` + `action-*` pages), [part 19](designs/workflows-module/parts/_completed/19-operational-apis/design.md) (operational APIs), [parts 12](designs/workflows-module/parts/_completed/12-resolver-pages/design.md)/[13](designs/workflows-module/parts/_completed/13-resolver-apis/design.md)/[15](designs/workflows-module/parts/_completed/15-resolver-form-builder/design.md) (emitted pages/apis/forms), [part 20a](designs/workflows-module/parts/_completed/20a-module-manifest-static/design.md)/[20b](designs/workflows-module/parts/_completed/20b-module-manifest-dynamic/design.md) (manifest). Parts 38, 39, 43, 12/13/15, 19, and 20a/20b have shipped; parts [40](designs/workflows-module/parts/_completed/40-simple-action-surfaces/design.md), [46](designs/workflows-module/parts/_completed/46-debundle-workflow-config/design.md), [48](designs/workflows-module/parts/_completed/48-render-config-off-connection/design.md), and [49](../_next/49-request-changes-verb-gate/design.md) are in flight and this design is written against their target state — sequencing is tracked in [implementation-plan.md](implementation-plan.md).
...
- The top-level [Testing conventions § E2E vs. unit split](designs/workflows-module/design.md#testing-conventions) is the governing rule this part applies; this part adds no new convention, it instantiates that one.
