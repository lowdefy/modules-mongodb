...
### D7 — The Part 24 field-update path carries no comment (supersedes the universal-fields bullets below)

**Implementation deviation (recorded per the repo's design-vs-code rule).** The original Files-changed list below wired `comment_visibility` through the Part 24 `UpdateActionFields` path and added the shared/internal control to `universal-fields.yaml`, on the premise that the field-update surface carries a comment. It does not: `update_fields_actions.yaml` hardcoded `comment: null` and there is no comment input anywhere on that surface — editing assignees / due date is a metadata edit, not a note-bearing action. Adding a comment box to the "edit details" modal is a feature addition with no concrete need ("build for concrete needs").

So the field-update operation carries **no comment at all**. The full dead chain was removed instead of extended:

- `components/universal-fields/update_fields_actions.yaml` drops `comment: null` (sends no comment).
- `makeWorkflowApis.js` update-fields endpoint drops both `comment` and `comment_visibility` payload mappings (the submit endpoint keeps both).
- `planFieldsUpdate.js` / `UpdateActionFields.js` drop the `comment` / `comment_visibility` params; the `action-fields-updated` event never carries a description.

Net effect on this part's goal: unchanged. Shared-vs-internal visibility is a property of comments, and field updates have none. The universal-fields bullet under "Files changed" and the `comment_visibility` additions to the three Part 24 plugin files are **superseded by this decision** — read them as "removed the comment plumbing" rather than "added comment_visibility."

### D6 — All comments are text-only (the shared fragment has no image branch)

The single shared "comment input + visibility control" fragment is **text-only on every surface**: the `TiptapInput`'s Image extension is disabled (`properties.image.disabled: true`) and `s3PostPolicyRequestId` is left unset, so `comment.fileList` is always empty and `comment.html` is always real text. Consequently every comment input's `validate` is just "`comment.text` non-empty" — no `fileList` clause anywhere.

This is the deliberate generalisation of [Part 62](designs/workflows-module/parts/_completed/62-changes-requested-callout/design.md)'s narrower change. Part 62 made only the **request-changes** comment inputs text-only (so its read-only `Alert` callout never has to render an attachment). Because Part 61 funnels **every** comment surface through one fragment, the choice is binary: either the fragment carries an `allow_images` var (request-changes text-only, the rest image-capable) or all comments are text-only. We take the latter — a branch-free fragment is the whole point of the extraction (one correct way), and an image var would reintroduce exactly the per-site variation it exists to remove.

The cost is a real but small new restriction extended beyond request-changes to the edit-submit, error-recovery, optional-surface, and Part 24 universal-fields comments: those comments can no longer carry inline images. This is acceptable — workflow comments are short review notes, the comment value shape (`{ html, text, fileList }`) is unchanged so nothing downstream breaks, `fileList` simply stays empty, and it keeps the fan-out fragment uniform. (File _attachments_ to an action remain a separate, unaffected concern — this is only about inline images inside the comment editor.)
...
payload:
  comment: { _state: <comment-state-path> } # { html, text, fileList } — shape unchanged; fileList always empty (text-only, D6)
...
- **Comment surfaces** — add the shared/internal control next to **every** comment `TiptapInput`, shown only when `enable_internal_comments` is set, posting `comment_visibility`. To avoid drift across surfaces (one-correct-way), extract a **single shared "comment input + visibility control" fragment** that all surfaces `_ref`, parameterised by state path and the gating module var — rather than copying the control into each surface. The fragment is **text-only** (D6): Image extension disabled, `s3PostPolicyRequestId` unset, `validate` reduced to "`comment.text` non-empty" (no `fileList` clause). This **generalises [Part 62](designs/workflows-module/parts/_completed/62-changes-requested-callout/design.md)**, which made only the request-changes inputs text-only; once this fragment lands it owns the text-only configuration for **all** comment inputs and supersedes Part 62's inline per-input edits. Today the comment inputs are inline `TiptapInput` blocks (not routed through the existing `components/fields/tiptap_input.yaml`), so this extraction also consolidates them. The fragment is dropped at every comment-input site:
...
- **Text-only (D6):** every comment input rejects inline images (Image extension disabled, no `s3PostPolicyRequestId`); `comment.fileList` is always empty and each input validates on `comment.text` alone.
...
- **[Part 62 — changes-requested comment callout](designs/workflows-module/parts/_completed/62-changes-requested-callout/design.md)** (in implementation, lands first) — reads the request-changes comment from the calling app's bucket and inherits Part 61's `shared`/`internal` scoping. Part 62 made the request-changes comment inputs text-only so its read-only `Alert` never renders an attachment; Part 61 generalises that to all comment inputs via the single shared fragment (D6), superseding Part 62's inline per-input edits. Since Part 62 lands first, the non-request-changes comment inputs only become text-only once this part's fragment lands.
