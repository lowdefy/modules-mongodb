...
>
> - **Phase, not `fireGroupOnComplete.js`.** Implemented as `plugins/…/shared/phases/dispatchGroupOnComplete.js`, called from `handleSubmit.js` **post-commit, AFTER the tracker cascade and before the post-hook** (lifecycle reordered vs this design's step numbering: tracker subscription = step 9, group on_complete fan-out = step 10). The fan-out must follow the cascade because a parent completion doesn't exist until that parent's cascade level commits.
> - **Parent-level fan-out IS implemented** (the design's "Extends `fireTrackerSubscription`" scope, adapted to the cascade loop). A group that transitions to `done` on a PARENT workflow via tracker propagation fires its `on_complete`, `context.workflow` set to the parent doc — not just the originating workflow's groups. `planTrackerLevel` computes each level's `completedGroups` diff; `runTrackerCascade` accumulates them paired with the committed parent doc; `handleSubmit` dispatches the **union** of the originating workflow's `plan.completedGroups` and every cascade level's.
> - **Shipped `callApi` contract + endpoint resolution.** `callApi({ endpointId, payload })` with a build-resolved, pre-scoped endpoint id (no runtime `workflow-{type}-group-{id}-on-complete` synthesis). `makeWorkflowApis` emits `group_on_complete` on the submit endpoint as a **`{ workflow_type → group_id → { _module.endpointId } }` bundle spanning the workflow + its ancestors** (same ancestor walk as `render_config`), so a parent group resolves from the child submit's `params.group_on_complete`. The dispatcher resolves each completion by its `workflow_type`. Same `_module.endpointId` mechanism as hooks.
> - **Error policy: throws propagate** (post-commit, idempotent contract — identical to the post-hook), *not* the log-and-continue + `fan_out_results` return this design specifies. There is no `fan_out_results` key. The handler response's `completed_groups` stays **originating-only** (`plan.completedGroups`) — the union is used for dispatch, not surfaced; revisit if a consumer needs parent completions in the response.
> - **Payload.** `{ workflow_id, workflow_type, group_id, user, context: { workflow } }`, where `workflow` is the completion's OWN committed doc (parent doc for parent-level groups). No `group_title`, no `event_id` — mirrors the post-hook `context` so a routine can reach `context.workflow.entity.id`. Revisit if a concrete consumer needs more.
> - **The "Open questions" auth-context problem is moot.** The emitted `on_complete` endpoints are `InternalApi` with no role gate (engine-only), so parent-level fires under `context.user` never auth-fail — none of the design's three options (elevated identity / per-fire synthesized roles) are needed.
> - **Not fired on cancel/close.** Only the submit path (and its cascade) fans out; `CancelWorkflow`/`CloseWorkflow` don't dispatch `on_complete`, so a parent group completed by a close-triggered cascade doesn't fire (consistent — close/cancel never fire `on_complete`).
> - **Worked example.** `apps/demo/…/onboarding/onboarding.yaml` — the `qualification` group's `on_complete` advances the lead's pipeline status to `qualified` (moved out of `qualify`'s post-hook). It keeps a `$cond` idempotency guard: `on_complete` fires on every transition to `done`, and a `done` form/check action can be reopened (`request_changes`/`activate`) and re-completed, so the group can complete more than once — routines must tolerate re-firing (contract identical to post-hooks).
