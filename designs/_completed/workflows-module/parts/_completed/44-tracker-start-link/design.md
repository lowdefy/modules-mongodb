...
3. **The `edit` verb on trackers acquires meaning** — "may start the child." The access validator already accepts any verb on any kind; no schema change. Per Part 34 D7, verb _presence_ gates emission and per-verb _role gates_ filter `visible_verbs` at read time — both apply to the start link exactly as to form/check edit links. View-only trackers stay display-only.
...
In both cases the tracker row is where the user _sees_ "this needs to happen" — `status_map.action-required` says "Create the installation ticket" — but the row offers no way to get there. `computeEngineLinks.js:68-79` emits a tracker link only when `child_workflow_id != null`, and that link points at the _child's_ overview. Before the child exists: nothing.
