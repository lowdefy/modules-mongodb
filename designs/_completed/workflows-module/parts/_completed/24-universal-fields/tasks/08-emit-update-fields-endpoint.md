...
> **Rev 2:** the endpoint is now **one per workflow type** (`{workflow_type}-update-fields`), action*id-dispatched, mirroring the shipped `{workflow_type}-submit` — \_not* per-action-type. The build-time `action_type` literal is dropped.
