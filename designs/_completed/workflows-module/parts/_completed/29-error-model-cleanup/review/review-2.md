...
- Move the proposed tweak above the `handleError` call: branch on `isReject` _before_ invoking `context.handleError`, so rejects bypass `handleError` and only errors call it. Concretely:
...
- The throw at `handleSubmit.js:34-37` for missing `current_status` means task `submit_edit` _requires_ the caller to supply one; there's no fallback. Out-of-scope for Part 29 (it's existing behavior) but the "no engine change" framing in D2 elides this — task `submit_edit` is more constrained than form `submit_edit`, not just an opt-in feature.
