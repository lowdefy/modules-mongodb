...
src/connections/WorkflowAPI/
  ...
  UpdateActionFields/                # Part 24 — state-orthogonal universal-fields write
    UpdateActionFields.js            # handler: load (edit verb) → planFieldsUpdate → commitPlan
  StartWorkflow/
    StartWorkflow.js
    createActions.js
  CancelWorkflow/
...
- **Universal action fields** (`assignees`, `due_date`) — kind-split write path (Part 24; Part 64 dropped the editable `description`). For **`kind: check`** they flow through `SubmitWorkflowAction` payload's `actions[].fields` and merge into the per-action `$set` atomically with the status transition (`null` clears; omitted leaves unchanged). For **`kind: form`** they are stripped from the submit payload and owned exclusively by `UpdateActionFields` (below); `kind: tracker` carries them on the doc with no UI surface in v1. (The action body `description` is **not** a universal field — it is author-authored config read via `actionConfig.description`, not stored on the action doc; see action-authoring "Action `description`".)
- **`UpdateActionFields`** (Part 24) — a state-orthogonal operation that writes the universal fields on one action with **no FSM transition and no workflow doc write**. Loads the action by `action_id`, gates on the per-app **`edit`** verb (`access.{app}.edit`), applies `$set` for the fields (`null` clears, omitted preserves), refreshes the change stamp, and **re-renders the status-map cell** against the planned doc (so the entity-page card never shows stale assignee/due-date). Emits an **`action-fields-updated`** event; an optional `comment` is rendered into `display.{app}.description` (Part 33), **not** event metadata. Writes no workflow doc, so there is no CAS gate — concurrent fields-updates are last-write-wins. No stage/lifecycle restriction: editable in any stage the caller has `edit` on, including `done` / `not-required` / `error` and on a `completed` / `cancelled` workflow (`required_after_close` does **not** apply). Emitted as one **`{workflow_type}-update-fields`** endpoint per surface-bearing workflow, dispatched by `action_id`. Returns `{ action_id, event_id }`.
...
Reserved keys: `_id`, `workflow_id`, `type`, `entity_id`, `entity_collection`, `key`, `status`, `summary`, `created`, `updated`, `assignees`, `due_date`, `tracker`, `child_workflow_id`, `child_entity_id`, `child_entity_collection`, `parent_action_id`, `parent_entity_id`, `parent_entity_collection`. Collisions are silently overridden by core fields; no validation throws in v1. (Part 64 removed `description` from the action doc — it is author-authored config, not a doc field, so it is no longer a reserved payload key.)
