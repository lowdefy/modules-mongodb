...
Related versioning confusion worth fixing while here: the Current State table says references are "at the v0.17.0 shape", and that is true of the _content_ (`git diff v0.17.0 HEAD -- modules/deals` is version and changelog lines only). But `v0.17.0` is the **repo tag**, while `modules/deals/module.lowdefy.yaml` is at module version **0.22.0**. An implementer reading "v0.17.0" and looking for that module version will not find it.
...
Separately, `$mergeObjects`/merge precedence is undefined in the design — when two workflows _do_ collide, which wins is currently whatever `$lookup` ordering happens to produce.
