...
3. A newly documented deployment requirement: the committed Atlas Search index definitions (with `storedSource: true` where the requests read the stored copy) and the regular `mongod` indexes the browse and fallback paths need.
...
Add `.changeset/search-atlas-fallback.md` bumping the four module packages — `@lowdefy/modules-mongodb-contacts`, `-companies`, `-activities`, `-deals` — plus `-user-admin` for task 11. Lead with the breaking `filter_match` change (it is what a consumer must act on), then the new `atlas_search` var, then the committed search indexes — stating explicitly that **this version changes the Atlas Search index definition**, so a consumer upgrading must update their cluster's index to match the definition shipped in the version they move to (the mappings narrow to text fields only, and `storedSource` changes per collection). Give `user-admin` one non-breaking line: its members search now treats regex metacharacters in the search box as literals (previously a `(` errored and `.*` matched everything), and its filter stage composes clauses with `$and`. Follow the shape of `.changeset/app-operator-slug-migration.md`: a bolded one-line summary of the break, then a "Migration for consumers" list.

Cover in the migration list: rewrite any `request_stages.filter_match` clauses in Mongo query syntax; set `atlas_search: false` on each searchable module entry for a non-Atlas deployment; create the `default` search indexes from the committed JSON (`storedSource: true` on `user-contacts` and `companies`, no `storedSource` on `activities` and `deals`), and regenerate the companies one if `name_field` is overridden; add the regular `mongod` indexes before running the fallback at any size.
