...
- **This request must keep `returnStoredSource` off — pass `returnStoredSource: false` to `text_lead.yaml`.** The flag is absent here by deliberate decision, not oversight: `modules/activities/CHANGELOG.md:126` (PR #68) dropped it "so post-write refetches return the live doc immediately instead of waiting on Atlas Search index replication." Re-adding it would reinstate that staleness bug for any refetch with a search term active — which is exactly what the quick-capture modal produces. (`deals/get_deals_list.yaml`, task 7, is the other request that has never set it and opts out for the same reason.) Consequence: the documented `activities` search-index requirement (task 8) omits `storedSource`, since no query here reads `mongot`'s stored copy.
...
- No `$search` block is authored in this file; the Atlas stage comes from `text_lead.yaml` and carries `returnStoredSource: false`.
...
- Built artifact for the activities list page shows the gated `$search` with `returnStoredSource: false` under the default flag, and the `$or` regex clause with no `$search`/`$meta` when the flag is temporarily flipped to `false`.
...
- `modules/activities/requests/get_activities.yaml` — modify — filters → `$match` `$and`, text/score/sort via the shared builder, `returnStoredSource: false` passed through.
