...
1. Add `slug:` to `apps/demo/lowdefy.yaml` and delete `apps/demo/app_config.yaml`. Its consumers were the old `app_name:` module-var indirection (the six demo `vars.yaml` files + the two `events/vars.yaml` `_ref`s) **and** the demo auth adapter's `appName` property, which scopes auth records by app — that one is rewritten to `{ _build.app: slug }` (build-time literal, matching the `_ref` it replaces; see [§Files changed](#files-changed)).
...
- `plugins/modules-mongodb-plugins/src/connections/WorkflowAPI/schema.js` — connection property `app_name` → `slug`, and update the consumer-facing `description` strings that still narrate the old wiring (the `slug` property's "wire this from `_module.var: app_name`", and the `user` property's `[app_name]` / `user.apps.{app_name}.roles` placeholders → `{slug}`).
...
- `plugins/modules-mongodb-plugins/package.json` — version bump `0.7.0` → `0.8.0` (breaking schema change); rebuild `dist/` (not git-tracked) via the `build` script. Update the `plugins:` version constraint in `modules/workflows/module.lowdefy.yaml` from `^0.6.0` → `^0.8.0`. (The `^0.6.0` constraint is already stale — under 0.x caret semantics it excludes the `0.7.0` the package currently declares — so this fixes a pre-existing off-by-one rather than a clean increment.)
...
- `apps/demo/lowdefy.yaml` — add `slug: demo` and rename `name: Module Demo App` to `name: Modules Demo` so the new `app_title: { _app: name }` default reads cleanly in user-admin labels. Also rewrite the `MultiAppMongoDBAdapter` `appName` property (`auth.adapter.properties.appName`, ~line 48) from `{ _ref: { path: app_config.yaml, key: app_name } }` to `{ _build.app: slug }` — this is the auth partition key, so it must reproduce today's literal `"demo"`; `_build.app` matches the build-time inline the `_ref` performed, whereas `_app` would leave an unevaluated runtime object in the auth config (init-path evaluation unverified). The property key `appName:` is the external plugin's schema name and is unchanged by the in-repo `app_name → slug` rename. Done in Task 01 so the adapter is migrated before Task 05 deletes `app_config.yaml`.
...
Scope and cost: ~110 occurrences across the workflows resolvers and the plugin package, plus a plugin **version bump** (the connection-schema property rename is breaking; bump `package.json` `0.7.0` → `0.8.0` per the 0.x policy and update the `version:` constraint in `modules/workflows/module.lowdefy.yaml`'s `plugins:` list from `^0.6.0` → `^0.8.0` — the existing `^0.6.0` is already stale, excluding the package's current `0.7.0`). `dist/` is a build artifact (not git-tracked) — rebuild with the plugin's `build` script; don't hand-edit it. The rename does **not** touch the stored `created.app_name` field or any literal stored key (see [Non-goals](#non-goals)).
