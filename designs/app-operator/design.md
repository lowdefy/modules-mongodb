...
**Manifest var defaults: prefer `_build.app` when the var is consumed at any build-time site.** A var default is a _single_ value substituted into every consumption site, so it cannot be `_app` at some sites and `_build.app` at others. If any consumer is inside a `_build.*` operator, the default must be `{ _build.app: … }` — it bakes a literal at build, which is then safe at runtime sites too (it is already a plain string by then). A default of `{ _app: … }` left as a runtime object breaks any `_build.*` consumer. `events.display_key` has a build-time consumer — `modules/events/components/note-capture.yaml` feeds `_module.var: display_key` straight into a `_build.object.fromEntries` map key — so its default is `{ _build.app: slug }`, which then also satisfies the runtime consumer on `connections/events-timeline.yaml`. This also confirms why we _remove_ `app_name` rather than redirect it to a default: per-occurrence replacement lets each site pick `_app` vs `_build.app`, which one default cannot.
...
**`deals` module** — landed after this design was written; migrate it exactly like the other scoping modules. Its sites split as: `_build.app: slug` for the `_build.object.fromEntries` map keys in `api/create-deal.yaml` and `api/update-people.yaml` and the `_build.string.concat` keys in `requests/get_mentionable_users.yaml` (the same deeper variant called out for `activities`); `_app: slug` for the runtime outcome stamp in `api/record-loss.yaml`. The two `display_key:` `_ref` vars in `components/detail_panel.yaml` are **dead config** — neither the `events-timeline` nor the `note-capture` component declares a `display_key` `_var` (both read the events module's own `_module.var: display_key`), so those vars are silently ignored today. Delete them rather than migrating them.
