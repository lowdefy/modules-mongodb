...
`modules/ai-reporting/api/resolve-report.yaml` is the `Dynamic` block's resolver. It already (from
...
In `modules/ai-reporting/api/resolve-report.yaml`:
...
In `modules/ai-reporting/pages/report.yaml`:
...
- `modules/ai-reporting/api/resolve-report.yaml` — modify: thread provenance/`is_owner`/`conversation_id`/`resolvedAt` into the return and the `compileReport` call; add the pre-validate `:try` + `:log`.
- `modules/ai-reporting/pages/report.yaml` — modify: fallback subtitle copy only.
