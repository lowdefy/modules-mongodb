...
`modules/ai-reporting/api/emit-data-parts.yaml:269-292` does the owner-guarded `$set`. The parent
...
- **The provenance line.** "Who made it, when the spec last changed, when these numbers were
  ...
  today returns only `{ is_owner, blocks }` (`modules/ai-reporting/api/resolve-report.yaml`, final
