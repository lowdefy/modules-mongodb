...
- `modules/ai-reporting/pages/report.yaml` — a `Dynamic` block over the compiled config, with `types.blocks` / `types.actions` / `types.operators` declared, and a fallback slot.
...
- `docs/ai-reporting/` — the index's surfaces table; `concepts/implementation-walkthrough.md` (the compiled shape changes: provenance, co-located filters, the new emitted `Link`).
