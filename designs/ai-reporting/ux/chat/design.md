...
> delete, and tables-as-artefacts are live in `modules/ai-reporting/pages/chat*`.
...
### A persisted message needs an id, and the framework does not give it one

`save-conversation` persists the onFinish payload's `messages` verbatim, and every assistant message in it arrives with `id: ""`. The AI SDK derives a `responseMessageId` only when the caller passes `generateMessageId` (`toUIMessageStream`, `ai@6.0.176`), and `handleUIMessageStreamFinish` then seeds the streaming state with `messageId ?? ""` — with no `messageId` on the `start` chunk to override it. `handleAgentChat` passes no generator, so the id is empty on every turn. Probed against the installed SDK: two turns, `assistant ids: ["", ""]`.

That is invisible until a reload, and then it is severe. `MessageList` keys each bubble `key: msg.id` and looks its parts up in an `id -> parts` map built with last-wins semantics, and an assistant bubble renders _entirely_ from those parts — so every answer in a restored transcript shows the **last** answer. The questions survive, because user bubbles render the item's own `content`, which is what makes the failure read as "the conversation is doing something weird" rather than as a break. A live turn looks correct throughout: the client mints its own id while streaming (`Chat`: `messageId: this.generateId()`), and only the server's id is persisted. The same collision also aims `deleteMessage` and `regenerate` at every assistant message at once.

**Decision: fix the cause in the framework and heal the read.** The cause is one argument — `generateMessageId: generateId` on both stream constructions in `handleAgentChat`, carried as `patches/@lowdefy__ai-utils.patch` until it lands upstream — and that only helps turns saved after it. Every conversation already saved keeps its empty ids, so `get-conversation-results` backfills a blank id by array index (`restored-<i>`) on the way out. Two layers, because either alone leaves a real hole: the patch alone strands existing transcripts, and the heal alone leaves the module permanently reading around a framework bug that other consumers still hit. The index is a sound key precisely because `messages` only ever appends — the same message keeps the same synthesized id across reads, and continuing the conversation writes them back, so a document heals for good on its next save.

**A migration script instead of the read-side heal was rejected.** It is the cleaner shape — no permanent workaround in the read path — but this module ships to apps in other repos, so it would make every consumer run a script against their own database to see their own history, and a consumer who never runs it sees a silently wrong transcript rather than a missing feature. The heal is a few lines of MQL in the endpoint that already reads the field.
...
Recency grouping and the rail's sort read the existing `updated.timestamp` — no new field. The stamp comes from `modules/ai-reporting/defaults/change_stamp.yaml`, `_ref`'d like every other writer's — same reasoning as [ownership](../ownership/design.md#reporting-writes-its-own-change-stamp-for-now).
...
Every property, and the `welcome` var itself, carries full `description` / `type` / `default` in `modules/ai-reporting/module.lowdefy.yaml`; then `pnpm docs:gen`. No var derives from the catalog.
...
- `modules/ai-reporting/api/list-conversations.yaml` — `updated`, soft-delete filter, cap 200.
- New `modules/ai-reporting/api/delete-conversation.yaml`.
- New `modules/ai-reporting/api/query-data-tool.yaml` — the agent's read path with the 200 KB budget and the `display` summary; `query-data.yaml` is left alone.
...
- New `modules/ai-reporting/api/render-table.yaml` and a `validateTableSpec` in `plugins/modules-mongodb-plugins/src/analytics/`.
- `modules/ai-reporting/api/emit-data-parts.yaml` — a second `:for` running each table spec's query (same `:try` isolation and same 8 MB fetch budget as the chart loop), the table part and its per-turn budget; `id` / `created` minted here and mapped onto each part; `$slice: -50` on the `$push`.
...
- `modules/ai-reporting/api/save-conversation.yaml` and `set-conversation-title.yaml` — the per-writer `$setOnInsert` split.
...
- `modules/ai-reporting/api/get-conversation-results.yaml` — the third `data-report-table` branch, and the new fields projected through; its "baked option is a snapshot" comment stays true and gains the `created` date that dates it.
- `modules/ai-reporting/agents/reporting-assistant.yaml` — the mermaid-sketch prompt, the `render_table` tool with its spec contract, `query_data` repointed at `query-data-tool`, and the `display` string on the query tool's output so the trace line reads as a summary rather than a key list.
