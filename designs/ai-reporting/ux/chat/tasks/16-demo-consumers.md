...
`apps/demo/modules/ai-reporting/vars.yaml`, which today sets `catalog`, `share_roles` and
...
**`apps/demo/modules/ai-reporting/vars.yaml`** — add a `welcome` block set **partially**, as
...
- `apps/demo/modules/ai-reporting/vars.yaml` — modify — the partial `welcome` override
