...
> doc's older prose says "Saved from this chat"). `docs/ai-reporting/` is the source
...
- `modules/ai-reporting/pages/chat.yaml` — the results panel (`id: results_panel`, `chat.yaml:511`)
...
- `modules/ai-reporting/api/get-conversation-results.yaml` — the model for a conversation-scoped
...
- `modules/ai-reporting/api/create-report.yaml` — **today** the only populator of the report's
...
- `modules/ai-reporting/pages/chat/components/save_report_sheet.yaml` — its `onOk` saves via
...
- `modules/ai-reporting/api/get-conversation-results.yaml` — add the `reports-store` find step and
  the `saved_reports` return field (and `saved_reports: []` on the unauthenticated guard).
- New `modules/ai-reporting/pages/chat/components/saved_from_chat.yaml` — the section component
  (`_ref`'d into `results_panel.blocks`), matching how the save sheet and rail modals are `_ref`'d
  under `pages/chat/`.
- `modules/ai-reporting/pages/chat.yaml` — `_ref` the section as the first panel block; add
...
- `modules/ai-reporting/pages/chat/components/save_report_sheet.yaml` — drop the post-save `Link`
  navigation; on a successful save, refresh `saved_reports` (via `get-conversation-results`) and
  close.
- `modules/ai-reporting/api/emit-data-parts.yaml` — backfill `conversation_id` onto each
  `generate_report` result and emit a `data-report-saved` part (streamed, not persisted).
- `designs/ai-reporting/ux/design.md` (parent) — update the `conversation_id` inventory on **both**
...
- `designs/ai-reporting/ux/save-as-report/design.md` — a note that the sheet no longer navigates away
  (it refreshes this section), pointing here.
- `docs/ai-reporting/` — a note that the results panel surfaces a conversation's saved reports and
  fills on save.
