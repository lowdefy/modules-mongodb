...
- Locate the save-as-report seed (the report carrying a real `conversation_id`) in the demo module
  under `apps/demo/modules/ai-reporting/`.
...
Add a short note under `docs/ai-reporting/` (the chat/results-panel page, or a how-to) that the
...
In `designs/ai-reporting/ux/design.md`, the data-model section states:
...
In `designs/ai-reporting/ux/save-as-report/design.md`, add a short note where it describes the sheet's
...
- `apps/demo/modules/ai-reporting/**` — modify/seed — a conversation whose `_id` matches the seeded
  report's `conversation_id`.
- `docs/ai-reporting/**` — add the note.
- `designs/ai-reporting/ux/design.md` — modify — reader + populator inventory.
- `designs/ai-reporting/ux/save-as-report/design.md` — modify — sheet-navigation note.
