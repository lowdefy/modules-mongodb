...
`modules/ai-reporting/api/list-reports.yaml` is an aggregation endpoint that scopes, searches, sorts
...
`list-reports` has exactly two callers: `modules/ai-reporting/pages/reports-list.yaml` (rebuilt in
later tasks) and two e2e specs. `modules/ai-reporting/api/get-conversation-results.yaml` does its own
...
3. **Delete the obsolete paging e2e test.** In `apps/demo/e2e/ai-reporting/report-scopes.spec.js`,
...
4. **Note it on ownership.** In `designs/ai-reporting/ux/ownership/design.md`, find its
...
- `modules/ai-reporting/api/list-reports.yaml` — modify — remove `$skip`/`$limit`; rewrite the paging header comment.
- `apps/demo/e2e/ai-reporting/report-scopes.spec.js` — modify — delete the skip/page_size paging test and any now-unused helper.
- `designs/ai-reporting/ux/ownership/design.md` — modify — one-line note that list-reports no longer pages.
