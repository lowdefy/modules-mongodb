...
- `modules/ai-reporting/pages/reports-list.yaml` — modify — add the `⋯` column, the actions Modal, the delete confirm, the rename modal, the ★ click, and the shared refetch.
- `modules/ai-reporting/pages/reports-list/actions/*.yaml` — create (optional) — the extracted refetch (and any multi-step action sequences) if inlining bloats the page.
