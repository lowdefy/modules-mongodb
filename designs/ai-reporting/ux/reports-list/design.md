...
- `modules/ai-reporting/pages/reports-list.yaml` — `List` of `Card`s with Open and Delete buttons and a description paragraph. No favourites, search, sort, paging, visibility or contents preview.
- `modules/ai-reporting/api/list-reports.yaml` — own-only, no parameters at all. Rewritten by [ownership](../ownership/design.md#endpoints) to take `{ scope, search?, sort?, skip, page_size }` and return a total alongside the rows.
...
**Shipped 2026-08-14 — as a patch, and it cost more than one column definition.** `cell.type: menu` now exists, carried on `patches/@lowdefy__blocks-aggrid.patch` (a third `dist/` patch alongside `blocks-antd-x` and `ai-utils`) pending the upstream PR, [lowdefy/lowdefy#2310](https://github.com/lowdefy/lowdefy/pull/2310). The cell is the shape described above: `items[]` each with `eventName`, `title`/`titleField`, `icon`/`iconField`, `hidden`/`hiddenField`, `disabled`/`disabledField` and `danger`, behind an icon-only trigger. Two decisions worth recording, both forced by the cell config namespace:

- **A hidden item is dropped, not disabled**, and a row on which every item is hidden renders **no trigger at all** rather than a button opening an empty popover. A non-owner's menu still has Open and Duplicate, so this never fires on the reports list — it is the correct behaviour for the next caller, not for this one.
- **The trigger's `type` and `shape` are fixed, not configurable.** Both keys are already taken at cell level (`type` names the renderer, `shape` is the avatar's), so exposing them would collide. The trigger is a small text `⋯`, matching the `ButtonsCell` defaults it sits beside.

The cell also could not work without **two fixes to the block that have nothing to do with menus** — both defects the existing cells have always had, and only a cell that holds state makes visible:

- **The popover has to mount on `document.body`.** An ag-grid cell hides its overflow, so a dropdown rendered inside one is invisible. `SelectorCell` already passes `getPopupContainer: () => document.body` for exactly this reason; `MenuCell` follows it.
- **A cell renderer's identity has to be stable across renders.** `processColDefs` rebuilt every renderer on every render, and a renderer is a React element type — so ag-grid saw a different component each time (`CellCtrl.refreshCellRenderer` bails when `cellRendererClass !== componentClass`) and destroyed the mounted cell. Anything a cell held died with it: an open popover, and equally the focus and half-typed value in the `selector` / `textInput` / `paragraphInput` cells from PR 2201, which is a latent bug this repo simply had not tripped over. The patch keeps one adapter per column and swaps the closure behind it, so the cell survives and still renders current config. Because that also means nothing replaces a cell when its column definition changes — ag-grid refreshes body cells on data changes and does not listen for `colDefChanged` — the block now calls `refreshCells({ force: true })` when the authored `columnDefs` change, fingerprinting function values by identity since `JSON.stringify` cannot see them. Cells re-render in place; they are not recreated.

That second fix is the one to lead with upstream. It is not a feature request, it is a bug the block has today, and `menu` is only the first cell whose state is visible enough to notice.

The swap was not one column definition, because the `Modal` was **shared with the report page**. Behaviour was extracted to `modules/ai-reporting/actions/report_{publish,unpublish,duplicate,rename_open,delete_open}.yaml` so both surfaces could `_ref` the same files, with two definitions only of _when_ each item shows — the cell resolves `hide_*` booleans per row, the modal read `selected_report` in page state. That much is inherent: a grid cell cannot read page state per row, and a modal has no row.

**Later the same day the report page stopped using the modal too**, becoming a `DropdownMenu` compiled into its header — so those files are now the list's alone for publish, unpublish and duplicate, and the report page carries a second implementation of those three. Rename and delete stayed shared, because they only open the static modals that own their writes. Why that was the least-bad option, and what it obliges, is in [ownership](../ownership/design.md#the-report-pages-menu-is-compiled-and-what-that-duplicates). The `show_open` var went away with the modal — the report page was its only consumer and passed `false`.

One consequence for the list page: the `hide_*` booleans are computed in the grid's `rowData` `_js`, which cannot reach a build-time module var, so `share_roles` is resolved once into a `can_share` state key at mount. Viewer-level, not per-row — the same answer for every report on the page.
...
2. ~~**The row kebab opens a `Modal`, not a popover.**~~ **Closed 2026-08-14** — the `menu` cell shipped as a patch, so plate 4's dropdown is what the list renders. See [the menu cell](#the-one-real-gap-is-a-menu-cell). The report page's kebab followed the same day, as a `DropdownMenu` compiled into its header, so both surfaces are the same antd dropdown. What that cost is recorded in [ownership](../ownership/design.md#the-report-pages-menu-is-compiled-and-what-that-duplicates).
...
- **The `menu` cell is a second upstream ask, but not a risk to this page.** The list ships with `⋯` opening a `Modal` and swaps to the cell in one column definition. _(2026-08-14: the cell shipped, and it is carried as a `dist/` patch until the upstream PR ([#2310](https://github.com/lowdefy/lowdefy/pull/2310)) lands — so the risk changed shape rather than closing. A `blocks-aggrid` release that touches `cellRenderers/index.js` or `createDisplayMeta.js` fails the patch apply and breaks `pnpm install` until it is re-rolled. Loud, not silent — the same posture as the other two patches.)_
