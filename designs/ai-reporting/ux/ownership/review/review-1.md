...
Reviewed `designs/ai-reporting/ux/ownership/design.md` against the reporting endpoints, the
...
Alice, act as Bob, and assert per scope. `apps/demo/e2e/ai-reporting/formatted-report.spec.js`
...
(`refactor(reporting)!`) and which deleted `modules/ai-reporting/defaults/user_id.yaml`. That
...
still says the change adds "Two new fragments under `modules/ai-reporting/defaults/`" and
