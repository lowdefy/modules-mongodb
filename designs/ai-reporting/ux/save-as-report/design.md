...
> the sheet ships filterless as designed. `docs/ai-reporting/` is the source of
...
- `modules/ai-reporting/pages/chat.yaml` — result selection in the panel, the Save-as-report action, and the sheet mounted once.
- New `modules/ai-reporting/api/create-report.yaml`.
- New `modules/ai-reporting/defaults/new_report.yaml` — the shared insert document (owner, title, description, spec, defaults, `conversation_id`, change stamps), parameterised by the validated spec + `conversation_id`. Referenced by both creation endpoints.
- `modules/ai-reporting/api/generate-report.yaml` — migrate its inline insert onto `new_report.yaml` so the document shape has a single source.
- `modules/ai-reporting/module.lowdefy.yaml` — the endpoint export.
- `docs/ai-reporting/` — a how-to for the save-as-report flow.
