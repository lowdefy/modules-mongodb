...
`designs/ai-reporting/ux/save-as-report/design.md`: a shared insert-document fragment,
...
- `charts.$.id`, `tables.$.id`, `downloads.$.id` — minted onto every part in
  `modules/ai-reporting/api/emit-data-parts.yaml` (`_array.map` + `__uuid`). This is the stable
...
- **One stored document shape, one source.** The full insert document lives only in
  `modules/ai-reporting/defaults/new_report.yaml`; both endpoints `_ref` it. No second inline copy.
...
| 5   | `05-docs-how-to.md`              | `docs/ai-reporting/` how-to for the save-as-report flow                             | 4          |
...
**Source:** `designs/ai-reporting/ux/save-as-report/design.md`
**Context read:** `filter-picker/design.md` (sub-design, filterless-first split), and the code
the design names — `modules/ai-reporting/api/generate-report.yaml`,
`modules/ai-reporting/defaults/{owner,change_stamp}.yaml`, `modules/ai-reporting/pages/chat.yaml`,
...
`modules/ai-reporting/api/get-conversation-results.yaml`,
`apps/demo/scripts/seed-reporting-domain.mjs`, `docs/ai-reporting/` tree; plus
`designs/ai-reporting/ux/chat/design.md` + its `tasks/` (part-identity precondition).
