...
`modules/ai-reporting/api/generate-report.yaml` is currently the only report-creation path. Its
...
- **Produces:** `modules/ai-reporting/defaults/new_report.yaml` — a `_ref`-able fragment taking two
...
**Create `modules/ai-reporting/defaults/new_report.yaml`** emitting the full insert document,
...
# in one place — see designs/ai-reporting/ux/save-as-report/design.md ("Two authors, one
...
**Modify `modules/ai-reporting/api/generate-report.yaml`:** replace the entire inline `doc:` map on
...
- `modules/ai-reporting/defaults/new_report.yaml` exists and emits every field the old inline doc
...
- `modules/ai-reporting/defaults/new_report.yaml` — create — the parameterised full insert document.
- `modules/ai-reporting/api/generate-report.yaml` — modify — `insert_report.doc` becomes a `_ref` to
