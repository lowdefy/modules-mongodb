...
Reviewed `designs/ai-reporting/report-filters/design.md` against the engine source, the
...
### 2. `modules/ai-reporting/pages/report.yaml` is missing from _Files changed_ — and it is the one file whose omission 404s every report
...
Fix: add `modules/ai-reporting/pages/report.yaml` to _Files changed_ (`MultipleSelector` under
...
(`apps/demo/e2e/ai-reporting/formatted-report.spec.js:127-146`).
...
(`apps/demo/modules/ai-reporting/catalog.yaml:132-135`). So as designed, the marquee capability —
...
`docs/ai-reporting/reference/presentation-contract.md:51` states that zero rows is a legitimate
