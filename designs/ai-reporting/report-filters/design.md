...
> are live (`plugins/modules-mongodb-plugins/src/analytics/`). `docs/ai-reporting/`
...
- `modules/ai-reporting/agents/reporting-assistant.yaml` (:131-143) — the filter section contract and the base-collection-field rule.
- `docs/ai-reporting/reference/presentation-contract.md` — the consumer-facing filter-binding section.
...
- [`designs/ai-reporting/open-query-engine/design.md`](../open-query-engine/design.md) — the engine, the filter-binding decision this extends, and the two-layer security model. Its array-literal cap entry records the raise to 500 made here.
- [`designs/ai-reporting/ux/save-as-report/design.md`](../ux/save-as-report/design.md) — the save-report confirm sheet whose filter picker is the second author of an `optionsQuery`; it points back at the derivation rule above.
- [`designs/ai-reporting/ux/report-page/design.md`](../ux/report-page/design.md) — where the filter controls sit, which this design leaves open.
