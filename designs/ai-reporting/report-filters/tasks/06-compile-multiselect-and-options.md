...
- `modules/ai-reporting/pages/report.yaml` — modify — `MultipleSelector` added to the `Dynamic` block's `properties.types.blocks`.
...
Do not touch `modules/ai-reporting/api/resolve-report.yaml`: the whole point of routing options queries through `querySections` is that the routine does not change.
