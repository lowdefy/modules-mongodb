...
`docs/ai-reporting/reference/presentation-contract.md` is the consumer-facing reference for what a report section declares and how it is verified. Two of its sections are now wrong or too narrow.
...
- `docs/ai-reporting/reference/presentation-contract.md` — modify — filter-binding rewrite plus the two verification corrections.
...
If a `docs/ai-reporting/how-to/` page or the module index also describes filters in a way this change makes stale, fix it in the same task — `docs/` is the source of truth for authoring behaviour and drift between two doc pages is worse than a longer diff.
