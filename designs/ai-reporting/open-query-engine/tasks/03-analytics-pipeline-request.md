...
**Update `modules/ai-reporting/connections/reporting-data.yaml`:** add `catalog: { _module.var: catalog }` (the var is renamed `datasets` → `catalog` in task 4; use the new name — the build breaks until task 4 lands, which is fine on this branch) to the connection properties.
...
- `modules/ai-reporting/connections/reporting-data.yaml` — modify — wire `catalog` connection property
