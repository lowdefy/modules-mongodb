...
Today `modules/ai-reporting/api/query-data.yaml` takes a structured `spec` payload and wires `datasets: { _module.var: datasets }` + `roles: { _user: roles }` into `AnalyticsQuery`. Three consumers share it: the agent's `query_data` tool, report filter re-queries (CallAPI), and panel downloads. The agent (`modules/ai-reporting/agents/reporting-assistant.yaml`) gets the data dictionary injected into its instructions (around line 60, "The queryable data model") and declares four tools: `query_data`, `render_chart`, `generate_report`, `export_data`.

## Task

**Rewrite `modules/ai-reporting/api/query-data.yaml`:**
...
**Rewrite `modules/ai-reporting/agents/reporting-assistant.yaml`:**
...
- `modules/ai-reporting/api/query-data.yaml` — modify — pipeline payload + AnalyticsPipeline routine
- `modules/ai-reporting/agents/reporting-assistant.yaml` — modify — instructions + four tool contracts
