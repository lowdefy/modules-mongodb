...
2. **Wiring:** point the reporting module's database-URI secret at the read-only user's connection string; note the secret name as declared in `modules/ai-reporting/module.lowdefy.yaml`.
...
- `docs/ai-reporting/` security/deployment section (per task 8's structure) — modify — provisioning steps + caveats
