...
One naming decision made here (the design leaves it open): the module var is renamed `datasets` → `catalog`, and the demo file `apps/demo/modules/ai-reporting/datasets.yaml` → `catalog.yaml`. Breaking renames are in scope per the design's compatibility waiver.

## Scope

**Source:** `designs/ai-reporting/open-query-engine/design.md`
