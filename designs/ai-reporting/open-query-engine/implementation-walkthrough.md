# Reporting module — implementation walkthrough

An end-to-end trace of the `reporting` module as built: chat message in, rendered
output out, with the file and line references for each hop. Companion to
`design.md` (which carries the rationale and rejected alternatives) and to
`docs/ai-reporting/` (which is the source of truth for consumer-observable
behaviour). Where this file and `docs/` disagree about behaviour, `docs/` wins.

The same walkthrough is published for contributors at
`docs/ai-reporting/concepts/implementation-walkthrough.md` — keep the two in step
when the wiring changes.

Line references are against the tree at the time of writing — treat them as
...
| Layer             | Where                                            | Role                                                 |
| ----------------- | ------------------------------------------------ | ---------------------------------------------------- |
| **Config** (YAML) | `modules/ai-reporting/`                          | pages, agent definition, API routines, connections   |
...
it declares a _contract_ (`x`/`y`/`valueKey`/`columns`) and the server builds
every block.

## 2. Build time: the catalog is the whole security model

`modules/ai-reporting/module.lowdefy.yaml:35-61` declares the one required var,
`catalog`. It is simultaneously the data dictionary, the allowlist, and the
authorization boundary. It gets bound in exactly two places:
...
Per-collection `roles` are opt-in (see `apps/demo/modules/ai-reporting/catalog.yaml:7-13`):
...
`modules/ai-reporting/pages/chat.yaml:137-146` mounts an `AgentChat` block pointed at
