...
results panel (`modules/ai-reporting/pages/chat/components/chat_workspace.yaml:686-694`) renders each
...
2. **`modules/ai-reporting/pages/chat/components/chat_workspace.yaml`** — the results-panel `EChart`
...
- `plugins/modules-mongodb-plugins/src/analytics/buildDataParts.test.js` — modify — option shape +
  `height` assertions.
- `modules/ai-reporting/pages/chat/components/chat_workspace.yaml` — modify — height binding with
  `300` fallback.
