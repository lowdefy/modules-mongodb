...
9. **Document the appearance change** in `docs/ai-reporting/reference/presentation-contract.md`. The
...
- `modules/ai-reporting/api/query-data.yaml` — the open query endpoint, two consumers since the agent's
...
- `modules/ai-reporting/api/query-data-tool.yaml` — the agent's read path: same guard, same
...
- `modules/ai-reporting/pages/chat/components/chat_workspace.yaml:355` (`onDataPart`), `:686-694` (the
  results-panel `EChart` at `height: 300`).
- `modules/ai-reporting/pages/report.yaml:49` — the `Dynamic` block's `types.blocks` allowlist, which
  already includes `EChart` and needs no addition.
- `modules/ai-reporting/api/get-conversation-results.yaml:7-9,100` — charts persisted on a conversation
...
- `modules/ai-reporting/api/chart-data.yaml` — new; payload `{ chart, x, y, query, filters }`,
  revalidated with `validateChartSpec` before the pipeline runs.
- `modules/ai-reporting/api/resolve-report.yaml` — pass `chartEndpointId`.
- `modules/ai-reporting/module.lowdefy.yaml` — export `chart-data`; bump `version`.
- `modules/ai-reporting/pages/chat/components/chat_workspace.yaml` — bind the panel chart height with a
  `300` fallback.
- `docs/ai-reporting/reference/presentation-contract.md` — note that chart appearance is compiled by
