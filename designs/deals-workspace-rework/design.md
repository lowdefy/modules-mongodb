...
**Width: a 180px module constant.** After item 8 narrows the detail column, the strip has roughly 730px to work with on a 1920px viewport (workspace 19/24 of the page, detail column 12/24 of that, less 8px of card-body padding). The floor is the card's top row, which carries the deal code beside a stage chip — `Fulfillment` is the longest stage title today — so below about 150px the chips begin to clip. 180px shows about four cards before scrolling, with ten reachable behind it: enough to see the neighbourhood without turning a glance-and-click affordance into a browsing surface.
