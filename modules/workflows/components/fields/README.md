...
`itemTitle` is a Nunjucks template rendered against each list item on the read-only view, review, and overview surfaces (DataDescriptions) to title the item's collapsible card. The item's fields are the template context, plus `_index` — the item's 0-based position — so a template can reference multiple fields and emit HTML (e.g. `<b>{{ name }}</b> — {{ locations | length }} sites`). It falls back to `Item N` when absent or when the render is empty, and has no effect while editing.

The list's own `title` renders HTML on both surfaces (e.g. `title: "<b>Devices</b>"`): while editing it renders through a DOMPurify-sanitised `Html` block above the list (the core `ControlledList` escapes `properties.title`, so the component routes the title around it), and on the read-only surfaces through the `DataDescriptions` title slot. Plain strings work unchanged, and an operator value (e.g. a `_nunjucks` template) resolves at render since it is an ordinary block property.
