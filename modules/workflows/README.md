...
After a successful `submit` (the direct Submit button and the optional confirm-modal `onOk`), the edit template shows a success `DisplayMessage` and navigates back to the entity — to `action.entity_link` (`{ pageId, urlQuery }`, resolved server-side by `GetWorkflowAction` from the `workflow-api` connection's `entities` config), falling back to browser-back when no entity is configured. The success copy defaults to "Submitted successfully." and can be overridden per action via `pages.edit.buttons.submit.successMessage`. Both happen only on success — a failed `CallAPI` halts the action chain before either runs.

The Save Draft button (`progress`) saves form values without advancing the workflow. After a successful save it stays on the page, refetches the action (so status, key, and button map reflect the saved draft) and shows a success toast — default "Draft saved.", overridable via `pages.edit.buttons.progress.successMessage`. Like submit, both run only on success.
...
- **`actions-on-entity`** — Entity-page widget surfacing every workflow attached to one entity with its action list. Takes `entity_id` + `entity_collection` vars. Each action row renders the server-resolved status, message, and single navigation link from the `get-entity-workflows` response. Renders the action list only — the check-action modal is dropped by the **page**, not bundled here (see `check-action-modal`).
...
- **`check-action-modal`** — Standalone in-context modal for opening a `kind: check` action without a full page navigation. A single `Modal` block with the fixed blockId `check_action_modal`, wrapping the shared `check-action-surface` body. **Open contract** (fixed, so every host wires it identically): `SetState { check_action_modal: { action_id } }` then `CallMethod { blockId: check_action_modal, method: setOpen, args: [{ open: true }] }`. On open the modal runs the same `GetWorkflowAction` fetch the shared pages' `onMount` runs, derives the surface `mode` from the fetched stage/verbs, and renders the signal buttons dumb off the server-resolved `action.buttons`. **The page drops it** — a single `_ref`, exactly once per page — because the page is the only scope that sees every co-present surface and so owns `on_complete` (the refetch action sequence run after a successful signal, default `[]`). Surfaces no longer auto-bundle the modal; the baked-in `check-action-click` handler tries to open it and falls back to navigation when it is absent. Never drop it on a page that already defines a `get_workflow_action` request (the `workflow-action-*` pages).
- **`check-action-click`** — Shared action sequence `_ref`ed into the `onActionClick` of both `actions-on-entity` (`ActionSteps`) and `workflows-events-timeline` (`EventsTimeline`); a `try`/`catch` handler. `check` cards **try** to open `check_action_modal` and, when no modal is on the page, the `catch` navigates to `action.link`; every other kind navigates via `action.link` directly in the `try` body and never touches the modal. Baked into both hosts by default — consumers wire no click handler.
- **`workflows-events-timeline`** — Action-enriched events timeline panel backed by the `GetEventsTimeline` engine method: events for one entity with cross-stream live action cards (status, message, access-resolved link), verb-filtered and link-collapsed server-side. Drop into a sidebar tile:
  ...
  Required vars: `reference_field` (the event-references key to match, e.g. `lead_ids`) and `reference_value`. Optional vars: `reverse`, `contact_page_url`, `disable_contact_link`, `compact`, `s3GetPolicyRequestId`. The kind-branch click handler is baked in (`check` cards try to open the shared `check_action_modal`, every other kind navigates via `action.link`) — no `onActionClick` wiring needed. The in-context modal, if wanted, is dropped by the **page** (a single `check-action-modal` `_ref`), not by this component. Event-type display comes from the events module's `event_types` export; action-card status display from this module's `action_statuses` component. This replaces the events module's former inline action-card lookup — the generic [events timeline](../events/README.md) is events-only; use this surface wherever action cards are wanted. See [Live action cards](../../docs/idioms.md#live-action-cards).
