...
- **Source:** [parts/12-resolver-pages/design.md](modules-mongodb/designs/workflows-module/parts/_completed/12-resolver-pages/design.md)
- **Inputs:** `{ workflows: WorkflowYaml[], slug: string }` — `slug` is the host app's slug, passed from the manifest as `{ _build.app: slug }` so it arrives as a literal string at build time.
- **Output:** Array of `{ id, definition }` page shells — one per `(workflow_type, action_type, verb)` for form actions, gated by `access.{slug}`. Each shell `_ref`s a template at `templates/{verb}.yaml.njk` with `action_config`, `workflow_type`, `entity_collection`, `page_ids`, and `page_config` vars (the last is the per-verb slice of `action.pages.{verb}`).
- **Invocation:** Manifest scope, under the dynamic-pages resolver channel (task 3 blocked on upstream part 2 — see [part 12 tasks.md](modules-mongodb/designs/workflows-module/parts/_completed/12-resolver-pages/tasks/tasks.md)).
