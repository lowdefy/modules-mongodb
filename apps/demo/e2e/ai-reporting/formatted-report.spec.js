...
test("the reports list scopes by user id and hides soft-deleted reports", async ({
  ...
  await ldf.goto("/ai-reporting/reports-list");
...
test("a saved report with a formatted table column renders", async ({
  ...
  await ldf.goto("/ai-reporting/report?report_id=e2e-formatted-report");
  ...
  // Each section renders inside a card of its own. An undeclared container type
  // blanks the WHOLE report rather than dropping one panel — so the containers
  // are worth asserting here, through the real resolve → build → render stack,
  // and not just off the compiler's output. The page's own chrome is a Box, so
  // every .ant-card on it is a compiled section card. These sections are a kpi
  // and a table, so no paired-chart Box wraps either of them; report-render
  // covers that container.
  const cards = page.locator(".ant-card");
  await expect(cards.filter({ hasText: "5,484.75" })).toHaveCount(1);
  await expect(cards.filter({ hasText: "$5,234.50" })).toHaveCount(1);
});
