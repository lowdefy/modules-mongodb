...
test("the ⋯ opens as a dropdown, and the owner sees every item they may use", async ({
  ldf,
  page,
  mdb,
}) => {
...
test("Rename opens the static edit form, seeded from the report", async ({
  ldf,
  page,
  mdb,
}) => {
  ...
  await ldf.user(USER_A);
  await ldf.goto("/ai-reporting/report?report_id=e2e-menu-rename");
...
test("a reader without share_roles sees Duplicate alone", async ({
  ldf,
  page,
  mdb,
}) => {
  ...
  await ldf.goto("/ai-reporting/report?report_id=e2e-menu-shared");
...
test("Publish makes the report shared, from the compiled item", async ({
  ldf,
  page,
  mdb,
}) => {
  ...
  await ldf.user(USER_A);
  await ldf.goto("/ai-reporting/report?report_id=e2e-menu-publish");
...
test("Duplicate opens the copy in a new tab, owned by the copier", async ({
  ...
  await ldf.user(USER_B);
  await ldf.goto("/ai-reporting/report?report_id=e2e-menu-dup");
  ...
  expect(new URL(copyTab.url()).pathname).toBe("/ai-reporting/report");
