...
function makePlan({
  ...
  return {
    workflow: {
      ...
      operation,
      changeLog: {
        before: { _id: workflowId, status: [] },
        after: { _id: workflowId },
      },
    },
    actions,
    event: {
...
/**
 * Default callbacks mock per the shipped contract: `emitEvent` persists the
 * event (here a no-op) and `sendNotification` resolves null. Individual tests
 * make either reject to simulate step 3/4 failures (mock-throw, never
 * `{ success: false }` envelopes).
 */
function makeCallbacksMock() {
  return {
    emitEvent: jest.fn(async () => {}),
    sendNotification: jest.fn(async () => null),
  };
}

/**
 * Build a context object for standalone (no transactions). Lets the caller
 * override callbacks to simulate step 3/4 failures.
 */
function makeContext(
  db,
  {
    loadedTimestamp = new Date("2026-01-01T00:00:00Z"),
    callbacks = makeCallbacksMock(),
    connection = {},
  } = {},
) {
  return {
...
test("CAS miss: throws ConcurrentSubmitError BEFORE any action write", async () => {
  ...
  await expect(commitPlan(ctx, plan)).rejects.toBeInstanceOf(
...
// ── Callback ordering: steps 3/4/5 run after steps 1/2 ───────────────────────

test("emitEvent (step 3) is called after workflow and action writes commit", async () => {
  const loadedTs = new Date("2026-01-01T00:00:00Z");
  const freshTs = new Date("2026-01-02T00:00:00Z");
  ...
  const callOrder = [];
  // Capture DB state at the moment each callback fires.
  const recordCall = async (callback) => {
    const wf = await mongo.db
      .collection("workflows")
      .findOne({ _id: "wf-order" });
    callOrder.push({ callback, wfTimestamp: wf?.updated?.timestamp });
  };
  const callbacks = {
    emitEvent: jest.fn(async () => recordCall("emitEvent")),
    sendNotification: jest.fn(async () => recordCall("sendNotification")),
  };
  ...
  const ctx = makeContext(mongo.db, { loadedTimestamp: loadedTs, callbacks });
  await commitPlan(ctx, plan);

  // Both emitEvent and sendNotification fire after the workflow is committed.
  expect(callOrder.length).toBe(2);
  expect(callOrder[0].callback).toBe("emitEvent");
  expect(callOrder[0].wfTimestamp).toEqual(freshTs); // workflow already committed
  expect(callOrder[1].callback).toBe("sendNotification");
});

// ── Step-3 payload shape ──────────────────────────────────────────────────────

test("step 3 calls emitEvent with the full event doc", async () => {
  const loadedTs = new Date("2026-01-01T00:00:00Z");
  const freshTs = new Date("2026-01-02T00:00:00Z");
  ...
  const callbacks = makeCallbacksMock();
...
test("step 4 calls sendNotification with event_ids plural containing the event_id", async () => {
  const loadedTs = new Date("2026-01-01T00:00:00Z");
  const freshTs = new Date("2026-01-02T00:00:00Z");
  ...
  expect(callbacks.sendNotification).toHaveBeenCalledWith({
...
test("step-3 failure: recorded on dispatchErrors, step 4 skipped, step 5 still runs", async () => {
  ...
  const callbacks = makeCallbacksMock();
  callbacks.emitEvent.mockRejectedValueOnce(new Error("events down")); // step 3 fails — emitEvent throws
  ...
  // Step 4 was skipped (sendNotification never called).
  expect(callbacks.emitEvent).toHaveBeenCalledTimes(1);
  expect(callbacks.sendNotification).not.toHaveBeenCalled();
...
test("step-4 failure: recorded on dispatchErrors, does not throw from commitPlan", async () => {
  ...
  const callbacks = makeCallbacksMock(); // step 3 succeeds
  callbacks.sendNotification.mockRejectedValueOnce(
    new Error("notifications down"),
  ); // step 4 fails — sendNotification throws
...
test("step-5 failure: recorded on dispatchErrors, does not throw from commitPlan", async () => {
  ...
  expect(result.dispatchErrors[0].error.message).toMatch(
    /log-changes write failed/,
  );
});
...
describe("transaction path (MongoMemoryReplSet)", () => {
  ...
  test("transaction path: steps 1–2 commit atomically (happy path)", async () => {
    ...
    const wfDoc = await replDb
      .collection("workflows")
      .findOne({ _id: "wf-txn" });
    expect(wfDoc.updated.timestamp).toEqual(freshTs);
    const actDoc = await replDb
      .collection("actions")
      .findOne({ _id: "act-txn" });
    expect(actDoc.status[0].stage).toBe("done");
  });
  ...
  test("transaction path: steps 3/4/5 fire after the txn commits (not inside retry loop)", async () => {
    ...
    const callOrder = [];
    // Verify the workflow is already committed (fresh ts visible) when each
    // callback fires.
    const recordCall = async (callback) => {
      const wf = await replDb
        .collection("workflows")
        .findOne({ _id: "wf-txn-order" });
      callOrder.push({ callback, wfTimestamp: wf?.updated?.timestamp });
    };
    const callbacks = {
      emitEvent: jest.fn(async () => recordCall("emitEvent")),
  ...
  test("insert operation on transaction path: no CAS filter, steps 2–5 identical", async () => {
    ...
    const wfDoc = await replDb
      .collection("workflows")
      .findOne({ _id: "wf-txn-insert" });
    expect(wfDoc).toBeTruthy();
    const actDoc = await replDb
      .collection("actions")
      .findOne({ _id: "act-txn-insert" });
    expect(actDoc).toBeTruthy();
  });
});
