...
describe("invokePreHook — no-hook cases", () => {
  test("returns empty result when params.hooks is undefined", async () => {
    const result = await invokePreHook(makeLoadedState(), makeParams(), user);
  ...
  test("returns empty result when params.hooks[signal] is undefined", async () => {
    const hook = jest.fn();
    const params = makeParams({ hooks: { approve: { pre: hook } } });
    // signal is 'submit', hooks only has 'approve'
    const result = await invokePreHook(makeLoadedState(), params, user);
    expect(result).toEqual({
      actions: [],
      event_overrides: {},
      form_overrides: {},
  ...
  test("returns empty result when params.hooks[signal].pre is undefined (only .post declared)", async () => {
    ...
    const params = makeParams({ hooks: { submit: { post: postHook } } });
    const result = await invokePreHook(makeLoadedState(), params, user);
    expect(result).toEqual({
      actions: [],
      event_overrides: {},
...
describe("invokePreHook — dispatch", () => {
  test("invokes the pre-hook function with the full payload", async () => {
    ...
    const params = makeParams({
      signal: "submit",
      hooks: { submit: { pre: hook } },
      comment: "hi",
    });
    const loadedState = makeLoadedState();

    const result = await invokePreHook(loadedState, params, user);
    ...
    // The payload is the only argument (no extra { user } bag).
    expect(hook.mock.calls[0].length).toBe(1);

    expect(result.actions).toEqual([
      { type: "approve-docs", signal: "approve" },
    ]);
  });

  test("signal-keyed hooks map resolves and fires the correct pre-hook", async () => {
    const wrongHook = jest.fn(async () => null);
    const approveHook = jest.fn(async () => ({
      actions: [],
    ...
    const params = makeParams({
      signal: "approve",
      hooks: {
        submit: { pre: wrongHook },
        approve: { pre: approveHook },
      },
    });
    const loadedState = makeLoadedState({
    ...
    await invokePreHook(loadedState, params, user);

    expect(approveHook).toHaveBeenCalledTimes(1);
    expect(wrongHook).not.toHaveBeenCalled();
  });

  test("null return from the hook normalises to empty result", async () => {
    const hook = jest.fn(async () => null);
    const params = makeParams({ hooks: { submit: { pre: hook } } });
    const result = await invokePreHook(makeLoadedState(), params, user);
    expect(result).toEqual({
      actions: [],
      event_overrides: {},
  ...
  test("payload envelope has signal, no interaction, no current_status", async () => {
    ...
    await invokePreHook(makeLoadedState(), params, user);
...
describe("invokePreHook — invalid_prehook_response", () => {
  test("unknown key in entry throws WorkflowEngineError with code: invalid_prehook_response", async () => {
    ...
    const params = makeParams({ hooks: { submit: { pre: hook } } });
    await expect(
      invokePreHook(makeLoadedState(), params, user),
...
describe("invokePreHook — prehook_redirect", () => {
  test("entry matching current action by (type, key) throws with code: prehook_redirect", async () => {
    ...
    await expect(
      invokePreHook(loadedState, params, user),
    ).rejects.toMatchObject({
      name: "WorkflowEngineError",
...
describe("invokePreHook — error propagation", () => {
  test("generic throw from the hook propagates unchanged (no try/catch)", async () => {
    const hook = jest.fn(async () => {
  ...
  test("UserError(isReject: true) propagates unwrapped — not a WorkflowEngineError", async () => {
    ...
    const params = makeParams({ hooks: { submit: { pre: hook } } });
    await expect(invokePreHook(makeLoadedState(), params, user)).rejects.toBe(
      reject,
    );
  });
});
