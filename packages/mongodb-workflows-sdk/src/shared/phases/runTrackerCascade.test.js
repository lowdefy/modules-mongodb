 ...
 * in-memory Mongo (standalone, no transactions) with mock dispatch callbacks
 * per the shipped contract.
 */
...
function makeBaseContext({ callbacks } = {}) {
  return {
    mongoDb: mongo.db,
...
describe("3-level cascade", () => {
  ...
  test("propagates two levels; fires carry the FSM-resolved stage", async () => {
    ...
    const initial = [
      {
        parentWorkflowId: "wf-B",
        parentActionId: "track-C",
  ...
  test("each level writes its parent workflow + tracker action; B auto-completes, A does not", async () => {
    ...
    expect(
      events.every((e) => e.type === "action-internal-mirror-completed"),
    ).toBe(true);
...
test("wide shallow cascade (many parents, depth 1) does not trip the depth guard", async () => {
  ...
  const initial = Array.from({ length: 15 }, (_, i) => ({
    parentWorkflowId: `wf-${i}`,
    parentActionId: `track-${i}`,
    signal: "internal_mirror_child_completed",
  }));
...
test("depth guard throws TrackerCascadeDepthError on a chain deeper than MAX_DEPTH", async () => {
  ...
  for (let i = 0; i <= 12; i += 1) {
    ...
    await seedAction({
      ...
      kind: "tracker",
      stage: "in-progress",
      child_workflow_id: i > 0 ? `wf-${i - 1}` : null,
    });
  }
  ...
  await expect(
    runTrackerCascade(
      [
        {
          parentWorkflowId: "wf-0",
          parentActionId: "track-0",
          signal: "internal_mirror_child_completed",
        },
      ],
      makeBaseContext(),
...
test("FSM no-op level skips commit entirely: parent unwritten, no event, no fire, no error", async () => {
  ...
  const result = await runTrackerCascade(
    ...
    makeBaseContext({ callbacks: makeCallbacks({ calls }) }),
  );
...
describe("gone parent", () => {
  test("missing parent workflow records workflow_not_found and continues", async () => {
    ...
    const result = await runTrackerCascade(
      [
        {
          parentWorkflowId: "wf-gone",
          parentActionId: "track-x",
          signal: "internal_mirror_child_completed",
        },
        {
          parentWorkflowId: "wf-ok",
          parentActionId: "track-ok",
          signal: "internal_mirror_child_completed",
        },
      ],
      makeBaseContext(),
    ...
    expect(result.fires).toEqual([
      {
        parent_action_id: "track-ok",
        parent_workflow_id: "wf-ok",
        new_status: "done",
      },
    ]);
  });
