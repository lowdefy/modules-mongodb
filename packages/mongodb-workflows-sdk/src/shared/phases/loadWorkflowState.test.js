...
const rolesContext = (roles) => makeContext({ user: { roles } });
...
function makeContextWithRenderConfig(renderConfig) {
  return {
    ...
    params:
      renderConfig !== undefined ? { render_config: renderConfig } : undefined,
  };
}
...
test("submit mode: render_config splices status_map + event_overrides onto matching action configs", async () => {
  ...
  expect(loaded.actionConfig.status_map).toEqual({
    "action-required": "Docs Required",
  });
  expect(loaded.actionConfig.event_overrides).toEqual({
    submit: "Upload Documents",
  });

  // sibling action config also received its slice
  const finalAuditCfg = loaded.workflowConfig.actions.find(
    (a) => a.type === "final-audit",
  );
  expect(finalAuditCfg.status_map).toEqual({
    "action-required": "Audit Pending",
  });

  // action with no matching slice is untouched
  const signoffCfg = loaded.workflowConfig.actions.find(
    (a) => a.type === "compliance-signoff",
  );
...
test("workflowId mode: render_config splices onto action configs", async () => {
  ...
  const collectDocsCfg = loaded.workflowConfig.actions.find(
    (a) => a.type === "collect-docs",
  );
  expect(collectDocsCfg.status_map).toEqual({
    "action-required": "Docs Needed",
  });
});
...
test("no render_config on params leaves action configs unchanged", async () => {
  ...
  expect(collectDocsCfg.status_map).toEqual({
    "action-required": "original-label",
  });
...
test("render_config missing the loaded workflow_type leaves configs unchanged", async () => {
  ...
  const renderConfig = {
    "other-type": {
      "collect-docs": { status_map: { "action-required": "Other" } },
    },
  };
  const ctx = makeContextWithRenderConfig(renderConfig);
  const loaded = await loadWorkflowState(ctx, { workflowId: "wf-1" });
...
test("CAS retry simulation: calling twice with same context yields identical configs (idempotent)", async () => {
  ...
  const cfg1 = loaded1.workflowConfig.actions.find(
    (a) => a.type === "collect-docs",
  );
  ...
  const cfg2 = loaded2.workflowConfig.actions.find(
    (a) => a.type === "collect-docs",
  );
