...
describe("invokePostHook — skip cases", () => {
  ...
  test("returns null when params.hooks[signal].post is undefined (only .pre declared)", async () => {
    ...
    const params = makeParams({ hooks: { approve: { pre: preHook } } });
    const result = await invokePostHook(
      makeLoadedState(),
      makePlan(),
      makeCommitResult(),
...
describe("invokePostHook — dispatch", () => {
  test("invokes the post-hook function with the payload", async () => {
    const hook = jest.fn(async () => ({ logged: true }));
    const params = makeParams({
      signal: "approve",
      hooks: { approve: { post: hook } },
    });
  ...
  test("payload.context carries planned docs — post-commit fresh state", async () => {
    ...
    const params = makeParams({ hooks: { approve: { post: hook } } });
  ...
  test("successful response is returned verbatim", async () => {
    ...
    const hook = jest.fn(async () => response);
    const params = makeParams({ hooks: { approve: { post: hook } } });
    const result = await invokePostHook(
      makeLoadedState(),
      makePlan(),
...
describe("invokePostHook — error propagation", () => {
  test("throw from the hook propagates unchanged (no try/catch)", async () => {
    const hook = jest.fn(async () => {
