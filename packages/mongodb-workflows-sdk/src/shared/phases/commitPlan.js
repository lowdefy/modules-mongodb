...
/**
 * Step 3 — dispatch the per-invocation event via the injected `emitEvent`
 * callback (workflows-sdk-split D2). The callback persists the event doc
 * (in Lowdefy: the events module's new-event endpoint) and throws on failure.
 *
 * Returns the `event_id` string on success; an `emitEvent` throw propagates so
...
async function dispatchEvent(context, plan) {
  ...
  await context.callbacks.emitEvent(eventDoc);
 ...
 *   3. Event     — callbacks.emitEvent(plan.event.doc).
 ...
 *   context.connection        — engine instance config (collection names,
 *                               changeLog, app_name, …)
 *   context.loadedState       — LoadedState from load phase; .workflow.updated.timestamp
 *                               is the CAS anchor (D15)
 *   context.callbacks         — injected dispatch callbacks
