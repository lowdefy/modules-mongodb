 ...
 * from the per-call `audit` bag (in Lowdefy the adapter fills it from
 * `callRequestResolver`'s request context). When an invocation has no
 * page/block (e.g. a server-side trigger), `pageId`/`blockId` are `undefined`
 * for both plugin and engine.
 *
 ...
 * @param {Object} args.audit — opaque per-call request-context bag; reads
 *   `blockId`, `connectionId`, `pageId`, `requestId`, and `payload`.
...
function planChangeLog({
  ...
  const { blockId, connectionId, pageId, requestId, payload } = audit ?? {};
