...
test("present keys are written", () => {
  const result = plan({
    fields: { assignees: ["u-7"], due_date: new Date("2026-06-01") },
  });
...
test("non-universal keys in the bag are ignored (never written)", () => {
  const result = plan({
    fields: { assignees: ["u-7"], status: [{ stage: "hacked" }], type: "evil" },
  });
...
test("change-log: one MongoDBUpdateOne entry with correct before/after", () => {
  ...
  const context = makeContext({
    ...
    audit: { payload: {}, pageId: "p", requestId: "r" },
  });
