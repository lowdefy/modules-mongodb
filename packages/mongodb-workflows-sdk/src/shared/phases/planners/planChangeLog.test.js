...
const audit = {
  ...
  pageId: "pg-1",
  requestId: "req-1",
  payload: { action_id: "a-1", signal: "submit" },
};
...
test("payload is included on every entry", () => {
  ...
  for (const entry of result) {
    expect(entry.payload).toEqual(audit.payload);
  }
});
...
// ── Context fields from audit ─────────────────────────────────────────────────

test("pageId and blockId are undefined when absent (API trigger path)", () => {
  const bag = { connectionId: "conn-api", requestId: "req-api", payload: {} };
  const result = planChangeLog({
    planActions: [],
    planWorkflow: makeUpdateEntry({ id: "wf-1" }),
    connection,
