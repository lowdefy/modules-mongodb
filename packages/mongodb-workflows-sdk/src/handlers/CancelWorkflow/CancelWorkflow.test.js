/**
 * Integration tests for the cancelWorkflow verb (task 17). Drives the real
 * engine surface against an in-memory Mongo (standalone, no transactions)
 * with recorder callbacks — mirrors SubmitWorkflowAction.test.js.
 */
...
describe("cancel sweep + lifecycle", () => {
  test("sweeps all non-terminal actions to not-required and preserves done actions", async () => {
    ...
    const result = await makeEngine().cancelWorkflow(request, perCall(request));
  ...
  test("status[] gains exactly one cancelled entry carrying the invocation event_id (no phantom completed)", async () => {
    ...
    const request = { workflow_id: "wf-1", reason: "duplicate" };
    const result = await makeEngine().cancelWorkflow(request, perCall(request));
  ...
  test("emits exactly one workflow-cancelled event", async () => {
    ...
    const request = { workflow_id: "wf-1" };
    const result = await makeEngine({ calls }).cancelWorkflow(
      request,
      perCall(request),
    );
  ...
  test("change-log records each per-action not-required transition", async () => {
    ...
    // No changeLog configured → no entries (opt-out).
    const request = { workflow_id: "wf-1" };
    await makeEngine().cancelWorkflow(request, perCall(request));

    // Re-run with changeLog.
    await resetCollections();
    await seedWorkflow();
    await seedAction({ _id: "a1", type: "qualify", stage: "action-required" });
    await seedAction({ _id: "a2", type: "kickoff", stage: "blocked" });
    ...
    const engine = makeEngine({
      changeLog: { collection: "log_changes", meta: {} },
    });
    await engine.cancelWorkflow(request, perCall(request));
...
describe("preconditions", () => {
  test("a missing workflow_id param throws invalid_params", async () => {
    await expect(
      makeEngine().cancelWorkflow({}, perCall({})),
  ...
  test("a missing workflow throws workflow_not_found (intended tightening)", async () => {
    ...
    await expect(
      makeEngine().cancelWorkflow(request, perCall(request)),
...
describe("post-commit dispatch failure", () => {
  test("a failing event dispatch throws post_commit_dispatch_failed while the swept docs stay committed", async () => {
    ...
    const request = { workflow_id: "wf-1" };
    await expect(
      makeEngine({ failOn: "events/new-event" }).cancelWorkflow(
        request,
        perCall(request),
      ),
