 ...
 * Drives the real handler through the public engine surface
 * (engine.getWorkflowAction) against an in-memory Mongo.
 */
...
// Read verb: no stamp / emitEvent required — only `user` rides per call.
function getWorkflowAction({
  ...
  const engine = makeEngine({ app_name, workflowsConfig, callbacks });
  return engine.getWorkflowAction(request, { user });
}
...
describe("envelope shape", () => {
  test("returns a single object (not an array)", async () => {
    ...
    const result = await getWorkflowAction({ request: { action_id: "a1" } });
  ...
  test("envelope carries all allowlisted engine fields", async () => {
    ...
    // Part 26: no resolveEntityData callback configured, so the entity object
    // reduces to the always-present instance id; the dead connection_id
    // subfield is gone.
  ...
  test("entity_link resolves from wfConfig.entity (id from action.entity.id)", async () => {
    ...
    // null because no resolveEntityData callback is configured (Part 26).
  ...
  test("description is null when the action config declares none", async () => {
    ...
    const result = await getWorkflowAction({ request: { action_id: "c1" } });
...
// ─────────────────────────────────────────────────────────────────────────────
// entity data resolution (Part 26 / workflows-sdk-split D2)
// ─────────────────────────────────────────────────────────────────────────────
// The engine no longer reads `wfConfig.entity.data_endpoint` — the endpoint
// concept moved to the Lowdefy adapter. The gate is now the injected
// `callbacks.resolveEntityData({ workflow_type, entity_id })`: no callback →
// null, throwing callback → null, callback result → lifted onto the envelope.

describe("entity data resolution (Part 26)", () => {
  test("calls resolveEntityData with { workflow_type, entity_id }, lifts name onto entity_link, merges entity object", async () => {
    await seedWorkflow();
    await seedAction({ _id: "a1", type: "qualify" });
    const calls = [];
    const resolveEntityData = async (args) => {
      calls.push(args);
      return { name: "Acme Corp", email: "ops@acme.test", status: "warm" };
    };
    const result = await getWorkflowAction({
      request: { action_id: "a1" },
      callbacks: { resolveEntityData },
    });
    // Exactly one call, payload is { workflow_type, entity_id } only.
    expect(calls).toEqual([
      { workflow_type: "onboarding", entity_id: "lead-1" },
    ]);
  ...
  test("module-injected id wins over a host-returned id (injected last)", async () => {
    await seedWorkflow();
    await seedAction({ _id: "a1", type: "qualify" });
    const resolveEntityData = async () => ({ id: "host-supplied", name: "X" });
    const result = await getWorkflowAction({
      request: { action_id: "a1" },
      callbacks: { resolveEntityData },
    });
  ...
  test("routine returning no name → entity_link.name null, host fields still present", async () => {
    await seedWorkflow();
    await seedAction({ _id: "a1", type: "qualify" });
    const resolveEntityData = async () => ({ email: "ops@acme.test" });
    const result = await getWorkflowAction({
      request: { action_id: "a1" },
      callbacks: { resolveEntityData },
    });
  ...
  test("no resolveEntityData callback → entity data degrades to null (no throw)", async () => {
    await seedWorkflow();
    await seedAction({ _id: "a1", type: "qualify" });
    // callbacks carries no resolveEntityData — the read must still succeed,
    // with name null and the entity reduced to the bare instance id.
    const result = await getWorkflowAction({
      request: { action_id: "a1" },
      callbacks: {},
    });
...
describe("form_values — keyed action", () => {
  test("keyed action reads from form_data[type][key]", async () => {
    ...
    const result = await getWorkflowAction({ request: { action_id: "a-ka" } });
