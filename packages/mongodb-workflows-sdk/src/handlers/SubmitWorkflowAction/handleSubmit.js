 ...
 *
 * The engine context (mongoDb/mongoClient/useTransactions, callbacks, user,
 * connection, params, workflowsConfig, the per-invocation `event_id`/`now`/
 * `newId` mint, and the `audit` bag) is composed once at method entry by
 * `createContext` (called from the createWorkflowsEngine facade) and
 ...
 * submitAction (e.g. by calling back into the engine). That recursion
 ...
 *
 * @param {Object} context — the engine context from createContext.
 * @returns {Promise<{
 *   action_ids: string[],
...
async function handleSubmit(context) {
  ...
  const preHookResult = await invokePreHook(loadedState, params, user);
