 ...
 * Drives the public engine surface (engine.getEventsTimeline) against an
 * in-memory Mongo (workflows-sdk-split).
 */
...
function makeEngine({ app_name = "test-app" } = {}) {
  return createWorkflowsEngine({
    ...
    contactsCollection: "user-contacts",
    app_name,
    callbacks: {},
  });
}
...
describe("basic return shape", () => {
  test("returns an empty array when no matching events", async () => {
    const result = await makeEngine().getEventsTimeline(
      { reference_field: "lot_ids", reference_value: "lot-999" },
      { user: defaultUser },
    );
  ...
  test("returns events with title, description, info display fields from app_name block", async () => {
    ...
    const result = await makeEngine().getEventsTimeline(
      { reference_field: "lot_ids", reference_value: "lot-1" },
      { user: defaultUser },
    );
