/**
 * Integration tests for engine.startWorkflow (task 17). Drives the public
 * engine surface against an in-memory Mongo (standalone, no transactions)
 * with recorded callbacks — mirrors SubmitWorkflowAction.test.js.
 */
...
function startWorkflow(request, { user = DEFAULT_USER, ...engineOpts } = {}) {
  return makeEngine(engineOpts).startWorkflow(request, {
    user,
    stamp: changeStamp,
...
describe("started as a tracker child", () => {
  ...
  test("parent tracker lands in-progress with the child link fields and a mirror event", async () => {
    ...
    const result = await startWorkflow(
      ...
      { workflowsConfig: makeParentChildConfig(), calls },
    );
