 ...
 * Drives the public engine surface (engine.getWorkflowActionGroupOverview)
 * against an in-memory Mongo (workflows-sdk-split).
 */
...
describe("group null collapse", () => {
  test("workflow null + group null when workflow not found", async () => {
    const result = await makeEngine().getWorkflowActionGroupOverview(
      { workflow_id: "no-such", group_id: "phase-1" },
      { user: defaultUser },
    );
  ...
  test("group is null when all actions in the group are dropped by access filter", async () => {
    ...
    const result = await makeEngine().getWorkflowActionGroupOverview(
      { workflow_id: "wf-1", group_id: "phase-1" },
      { user: defaultUser },
    );
  ...
  test("group is null when group_id is not a declared config group", async () => {
    ...
    const result = await makeEngine().getWorkflowActionGroupOverview(
      { workflow_id: "wf-1", group_id: "unknown-group" },
      { user: defaultUser },
    );
    expect(result.group).toBeNull();
...
describe("return shape", () => {
  ...
  test("entity_link resolves from wfConfig.entity", async () => {
    ...
    expect(result.workflow.entity_link).toEqual({
      ...
      title: "Lead",
      // Part 26: null because no resolveEntityData callback is configured
      // (workflows-sdk-split: absent callback → entity data degrades to null).
      name: null,
  ...
  test("entity_link.name lifts from the resolveEntityData callback result", async () => {
    await seedWorkflow();
    await seedAction({ _id: "a1", type: "qualify", action_group: "phase-1" });
    const calls = [];
    const result = await makeEngine({
      callbacks: {
        resolveEntityData: async (payload) => {
          calls.push(payload);
          return { name: "Acme Lead", other: "host-owned" };
        },
      },
    ...
    expect(result.workflow.entity_link.name).toBe("Acme Lead");
  });

  test("entity_link.name is null when the resolveEntityData callback throws", async () => {
    await seedWorkflow();
    await seedAction({ _id: "a1", type: "qualify", action_group: "phase-1" });
    const result = await makeEngine({
      callbacks: {
        resolveEntityData: async () => {
          throw new Error("forced failure: entity-data");
        },
      },
    }).getWorkflowActionGroupOverview(
      { workflow_id: "wf-1", group_id: "phase-1" },
    ...
    // Never fails the read — a throwing routine degrades to name: null.
    expect(result.workflow.entity_link.name).toBeNull();
  });
