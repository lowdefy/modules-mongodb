...
**Module-shipped templates supported.** With a Lowdefy release that includes module-level notifications, modules can ship their own templates and dispatch them directly. (The user-admin module's invite templates were dropped in this reconciliation — invitation email is engine-owned on the BetterAuth auth engine via `auth.email`.)
