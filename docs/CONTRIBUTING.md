...
Every file in `docs/` must open with a YAML front-matter block. The linter (Task 4) validates this schema at CI time.
...
- Do not hand-author `docs/llms.txt` — it is generated by the build (Task 4).
- Do not invent links to doc pages that do not exist yet — link to the existing `modules/{name}/README.md` until the target page is written.
