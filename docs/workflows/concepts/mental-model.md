...
- **Four operational APIs.** `start-workflow`, `cancel-workflow`, `get-entity-workflows`, `get-workflow-overview` — callable from any app page.
- **Resolver-emitted surface.** `makeActionPages` generates per-action page sets for form actions. `makeWorkflowApis` generates one `{workflow_type}-{action_type}-submit` endpoint per form/check action. Both are derived automatically from your `workflows_config`.
