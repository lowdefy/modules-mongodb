...
- **Entity-page components.** `actions-on-entity` (renders grouped action lists), `workflow-progress` (its presentation variant — collapsible sections of status-colored action buttons), `open-actions` (a compact list of an entity's open actions for sidebars and summaries).
