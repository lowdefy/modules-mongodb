...
The bidirectional link between a tracker action and its child workflow is established by the child type's `{type}-start` endpoint in a single call. The endpoint is type-scoped — its id is built from the workflow type (here `device-installation-start`), so the workflow type is **not** passed in the payload:
