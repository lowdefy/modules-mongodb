...
### 4. Wire `start-workflow` on the child-creation page

On the page the `start_link` points to, call the `start-workflow` endpoint with `parent_action_id` populated from the URL query param. This establishes the bidirectional link in a single call:
