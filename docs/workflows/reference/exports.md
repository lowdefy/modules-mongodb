...
| ID                        | Path                                 | Description                                                                                                                                                                     |
| ------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `workflow-overview`       | `/{entryId}/workflow-overview`       | Workflow detail page — header + progress bar + status-tinted collapsible action cards, each rendering its `form_data` in a `DataDescriptions`. Addressed by `?workflow_id=<id>` |
...
| `open-actions`              | Compact card list of an entity's OPEN workflow actions — a lighter sibling of `actions-on-entity` for sidebars/summaries that don't need the full stepper. Same `entity_id` + `entity_connection_id` vars and `get-entity-workflows` fetch; flattens every workflow's groups and keeps only non-terminal actions (excludes `done`/`not-required`), colour-keyed off `action_statuses`. Actions-only — reads no tasks or activities.                                                                                                                                                    |
| `workflow-progress`         | Presentation variant of `actions-on-entity` — renders the entity's workflows as collapsible sections of grouped, status-colored action buttons (the `WorkflowProgress` block), same data contract and baked-in `check-action-click` handler. Vars: `entity_id` + `entity_connection_id` (required); `active_action_id`, `default_active_keys` (optional); `active_keys` + `on_change` (optional — controlled expansion the page owns); `fetch_on_mount` (optional, default `true` — set `false` when the page owns the `entity_workflows` fetch, e.g. via `entity-workflows-refetch`). |
