...
| `check-action-click` | Shared action sequence for `onActionClick` — kind-branch handler: `check` → open `check_action_modal`; else → `Link` to `action.link`. Baked into `actions-on-entity` and `workflows-events-timeline` by default. |
| `workflows-events-timeline` | Action-enriched events timeline panel backed by `GetEventsTimeline`. Events with cross-stream action cards, verb-filtered and link-collapsed server-side. Required vars: `reference_field`, `reference_value`. Optional vars: `reverse`, `contact_page_url`, `disable_contact_link`, `compact`, `s3GetPolicyRequestId`, `include_modal` (default `false` — set `true` only on a timeline-only page with no `actions-on-entity`), `on_action_complete` (default `[]`). |
...
| ID | Description |
|---|---|
| `start-workflow` | Instantiate a workflow on an entity. Emits a `workflow-started` event. Optional payload: `actions` override (seeds at `action-required` or `blocked`), `metadata` (merged onto every seeded action), `parent_action_id` (links new workflow as child of an existing tracker action) |
| `cancel-workflow` | Push `cancelled` to workflow status; sweep remaining open actions to `not-required` via `internal_cancel_action` signal. Emits `workflow-cancelled`. |
| `close-workflow` | User-initiated normal termination — push `completed`; sweep non-terminal actions honoring `required_after_close`. Emits `workflow-closed`. Idempotent no-op on an already-completed workflow; rejected on a cancelled one. |
...
### Per-action endpoints (resolver-emitted)
