...
| `contacts_collection` | string | `user-contacts` |  | Contacts collection joined to resolve each event author's avatar (`created.user.id` → `_id`, projecting `profile.picture`). Defaults to `user-contacts`; avatars fall back to initials when an author has no matching contact.  |
