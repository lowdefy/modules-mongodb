...
Used by the `events` module to back its `events-timeline` component. The block doesn't fetch — it renders an array of pre-shaped event documents.
...
`onActionClick` is optional. When it's **not** wired, the block navigates to the action's server-resolved `action.link` by itself via the Lowdefy `Link` component — so the affordance works with zero host wiring. Wire `onActionClick` only when the host needs to intercept the click (e.g. open an in-context modal) instead of navigating.

The affordance button is labelled from `action.link.title`, falling back to `"View"` when the link carries none. The workflows module sets this title per resolved verb (see [Custom actions › button label](../workflows/how-to/custom-actions.md#the-action-card-button-label)).

The matching request resolves each event against its per-app display template — so `event.title`, `event.description`, and `event.info` arrive as already-rendered HTML strings (sanitized client-side).
