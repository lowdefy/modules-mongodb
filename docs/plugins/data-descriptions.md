...
| `{ key: "items", title, itemKey?, form: [...] }`      | An array field. The block iterates the array at `key`, applies the nested `form` to each item (replacing `$` in nested keys with the index), and renders each as a collapsible card. `itemKey` (dot notation, relative to each item, e.g. `name`) titles each card from the item's own data; without it — or when the value is missing or empty — the card is titled `Item N`. |
