...
modules:
  - id: ai-reporting
    source: "github:lowdefy/modules-mongodb/modules/ai-reporting@v0.38.0"
    vars:
      catalog:
...
- [The AI summary](concepts/ai-summary.md) — the report page's opt-in (`ai_summary` var), on-demand, model-written reading of the report as currently filtered: server-side re-resolution under the viewer's roles and filter values, the scope line, mark-stale on filter change, excluded sections, nothing persisted
