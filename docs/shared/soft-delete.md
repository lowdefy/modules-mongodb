...
`activities`, `files` and `ai-reporting` delete their own documents in-module (`ai-reporting` writes the stamp inline rather than via the events component — it declares no dependencies — but the field name, shape and read predicate are identical). `companies` and `user-admin` filter on `deleted` in their reads but expect the host app to write the delete stamp. `contacts` applies the same predicate when it looks up companies, but does not currently filter its own contact reads on `deleted`. Wherever a module reads `deleted`, it uses the same field name, shape, and read predicate.
