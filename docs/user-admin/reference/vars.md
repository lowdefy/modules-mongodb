...
|---|---|---|---|---|
| `org_slug` | string |  | Yes | Slug of the organization this instance administers. Under the `pinned` organizations policy the organization's id IS its slug, so this value feeds every read $match on `organizationId` and every write step's `organizationId`. Two admin surfaces in one app are two module instances with different `org_slug` values. Deployment ordering: this module does not create the organization. The app whose `auth.organizations.org` names this slug creates it at startup, so an instance administering ANOTHER app's organization requires that app to have booted against the same database first. Until it has, reads render EMPTY with no error and `InviteMember` throws ORGANIZATION_NOT_FOUND. The alternative is inserting the organization row alongside the bootstrap invitation. Renaming the organization's slug strands every member row, because the engine's startup ensure is by slug — a rename mints a fresh organization rather than renaming one. |
...
| `admin_roles` | array | `["user-admin"]` |  | Catalog role id(s) that gate every routine endpoint's `auth.api.roles` (design Decision 3). This is the "show me the UI" gate; it is not the authority to write. Every auth-owned write is additionally floored by the engine against the caller's member row in the organization the step names — `org_slug`'s — so an app role alone changes nothing (defense in depth). Each id must be a declared `auth.roles` catalog id. |
| `org_authority` | boolean | `true` |  | Gates the organization-authority control on the access modal — promoting a member to `admin` or `owner` in the administered organization, or revoking them back to `member` (the no-authority value) — and the matching `orgRole` field on the invite form. Holding `admin` in an organization is what lets a person administer it, so this control is how a deployment grants that. Set false in a deployment where org authority is granted out of band. |
...
| `impersonation` | boolean | `false` |  | Gates the "View as user" (ImpersonateUser) client action on the Security tile (design Decision 5). Off by default; the action only works for a caller the deployment's admin-plugin access control lets impersonate. |
