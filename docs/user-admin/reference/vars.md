...
| `org_authority` | boolean | `true` |  | Gates the organization-authority control on the access modal, promoting a member to `admin` in the administered organization, or revoking them back to `member` (the no-authority value; there is no empty tier and the write refuses one), and the matching `orgRole` field on the invite form. `owner` is the protected creator tier, capability-identical to `admin`, and is not settable from either control — an owner member is shown read-only. Holding `admin` in an organization is what lets a person administer it, so this control is how a deployment grants that. Set false in a deployment where org authority is granted out of band. When false the control does not render AND `update-org-role` rejects, so a deployment granting org authority out of band cannot have the tier written through this module by a caller who reaches the endpoint directly. The endpoint stays registered either way. The rejection is the routine's first step, not an absence. When true the write is floored by the engine against the caller's member row in the administered organization. |
