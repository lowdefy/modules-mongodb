 ...
 *     --include <a,b>   Catalog only these collections (exact names or *-globs,
 *                       e.g. `demo_*`). Default: every non-system collection.
 *     --exclude <a,b>   Skip these collections (exact names or *-globs). Applied
 *                       after --include. Use it to drop operational collections
 *                       (sessions, tokens, change logs) that are noise for a
 *                       reporting catalog and bloat the model payload.
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (a) {
      ...
      case "--no-model":
        ...
        break;
      case "--include":
        args.include.push(...splitList(argv[(i += 1)]));
        break;
      case "--exclude":
        args.exclude.push(...splitList(argv[(i += 1)]));
        break;
      case "--help":
      case "-h":
        args.help = true;
...
/**
 * Turn an --include/--exclude entry into a matcher. A `*` matches any run of
 * characters; every other character is literal. Anchored full-match, so
 * `demo_*` selects `demo_orders` but not `demo-log-changes`.
 */
...
/**
 * Choose which collections to catalog from the full list. Sampling the whole
 * database also drafts (and sends to the model) every operational collection —
 * sessions, tokens, change logs — which is noise for a reporting catalog and,
 * on a real database, a model payload large enough to fail. `include` (when
 * non-empty) keeps only matching collections; `exclude` then drops matching
 * ones. Both accept exact names or `*`-globs. Returns the sorted selection plus
 * any include patterns that matched nothing, so the caller can warn (a typo in
 * --include would otherwise silently catalog nothing).
 */
export function selectCollections(allNames, { include = [], exclude = [] } = {}) {
  const inc = include.map(patternToRegExp);
  const exc = exclude.map(patternToRegExp);
  let selected = allNames;
  if (inc.length) selected = selected.filter((n) => inc.some((re) => re.test(n)));
  if (exc.length) selected = selected.filter((n) => !exc.some((re) => re.test(n)));
  const unmatchedIncludes = include.filter(
    (p, i) => !allNames.some((n) => inc[i].test(n))
  );
  return { selected: [...selected].sort(), unmatchedIncludes };
}
...
function modelStatusLine({ modelStatus, model }) {
  switch (modelStatus) {
    case "used":
      return `Descriptions/hints/relationships drafted by model "${model}" (temperature 0).`;
    case "failed":
      return (
        `MODEL FAILED — the "${model}" call errored, so this is a type-inference-only ` +
        "draft (descriptions empty, no relationships). Re-run to draft them; if the " +
        "payload is large, scope it with --include/--exclude."
      );
    default:
      return "MODEL SKIPPED — type-inference-only draft (descriptions empty, no relationships).";
  }
}

function header({ dbName, sampleSize, flattenDepth, model, modelStatus, generatedCount }) {
  const lines = [
    ...
    modelStatusLine({ modelStatus, model }),
    "",
...
function usage() {
  return [
    ...
    "  --no-model      Skip the model call; emit a type-inference-only draft.",
    "  --include <a,b> Catalog only these collections (exact names or *-globs).",
    "  --exclude <a,b> Skip these collections (exact names or *-globs).",
    "  --help          Print this usage and exit.",
    "",
    "Env:",
...
async function main() {
  ...
  try {
    ...
    const nonSystem = infos.filter((c) => !c.name.startsWith("system."));
    const { selected, unmatchedIncludes } = selectCollections(
      nonSystem.map((c) => c.name),
      { include: args.include, exclude: args.exclude }
    );
    for (const p of unmatchedIncludes) {
      console.warn(`  --include ${p}: matched no collection.`);
    }
    if (args.include.length || args.exclude.length) {
      console.error(
        `Selected ${selected.length} of ${nonSystem.length} collections ` +
          `(include: ${args.include.join(", ") || "all"}; ` +
          `exclude: ${args.exclude.join(", ") || "none"}).`
      );
    }
    if (selected.length === 0) {
      console.warn(
        "No collections selected — the draft will be empty. Check --include/--exclude."
      );
    }
    const selectedSet = new Set(selected);
    const collections = nonSystem
  ...
  // Model drafting (graceful on failure). modelStatus distinguishes a model
  // that was never called (--no-model / no key) from one that was called and
  // errored — the header reports them differently so a silent failure is not
  // mistaken for a deliberate skip.
  let draft = {};
  let modelStatus = wantModel ? "attempted" : "skipped";
  if (wantModel && collectionNames.length) {
    try {
      ...
      console.warn(
        `Model drafting failed (${err.message}) — emitting type-inference-only ` +
          "draft. If the payload is large, scope it with --include/--exclude and re-run."
      );
      draft = {};
      modelStatus = "failed";
    }
