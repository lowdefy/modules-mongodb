...
const FloatingPanel = ({
  ...
  const launcher =
    properties.launcher === false ? null : properties.launcher || {};
  ...
  useEffect(() => {
    methods.registerMethod(
      ...
      (args) => applyOpen(Boolean(args && args.open)),
    );
    methods.registerMethod(
    ...
    methods.registerMethod(
      get(rename, "methods.setExpanded", { default: "setExpanded" }),
      (args) => applyExpanded(Boolean(args && args.expanded)),
    );
  });
  ...
  const renderIcon = (spec, fallback, key, size) => {
    ...
    const iconProps =
      typeof spec === "string" ? { name: spec, size } : { size, ...spec };
    return (
      <Icon
        blockId={`${blockId}_${key}`}
        events={events}
  ...
  return (
    ...
    >
      ...
      >
        <header
          className={cn("fp-header", classNames.header)}
          style={styles.header}
        >
          <span
            className="fp-avatar"
          ...
          <div className="fp-identity">
            {properties.title ? (
              <div className="fp-title">{properties.title}</div>
            ) : null}
            {properties.subtitle ? (
          ...
          {content.header ? (
            <div className="fp-header-extra">{content.header()}</div>
          ) : null}
        ...
        {content.toolbar ? (
          <div
            className={cn("fp-toolbar", classNames.toolbar)}
        ...
        <div
          ref={bodyRef}
          className={cn("fp-body", classNames.body)}
          style={styles.body}
        >
        ...
        {content.footer ? (
          <div
            className={cn("fp-footer", classNames.footer)}
            style={styles.footer}
          >
            {content.footer()}
          </div>
      ...
      {launcher ? (
        <button
          ...
          aria-label={
            open ? "Close assistant" : launcher.ariaLabel || "Open assistant"
          }
        ...
        >
          ...
          <span className="fp-launcher-icon">
            {open ? (
              ...
              renderIcon(launcher.icon, <IconBubble />, "launcher_icon", 24)
            )}
          </span>
