...
export default {
  ...
  slots: {
    content:
      "The panel body. Scrolls; everything else in the panel is fixed chrome.",
    header:
      "Optional extra content in the header row, between the title and the window controls.",
    toolbar:
  ...
  events: {
    ...
    onToggle:
      "Fired on every open/close, before onOpen / onClose. _event = { open }.",
    onExpand:
      "Fired when the expand/collapse control is used. _event = { expanded }.",
  },
  methods: {
    setOpen: {
    ...
    toggleOpen: {
      description:
        "Toggle the panel open/closed. Same thing the launcher does.",
    },
    setExpanded: {
  ...
  properties: {
    ...
    properties: {
      title: {
        type: "string",
        description:
          "Panel header title. Set in the app display font when one is defined.",
      },
      subtitle: {
        type: "string",
        description:
          "Small line under the title — an identity or status line, not a sentence.",
      },
      avatar: {
        type: "object",
        additionalProperties: false,
        ...
        properties: {
          icon: {
            type: ["string", "object"],
            description:
              "Icon name (host Icon component). Defaults to a built-in spark glyph.",
      ...
      launcher: {
        ...
        properties: {
          icon: {
            type: ["string", "object"],
            description:
              "Closed-state icon. Defaults to a built-in chat glyph.",
            docs: { displayType: "icon" },
          },
          label: {
            type: "string",
          ...
          ariaLabel: {
            ...
            description:
              "Accessible name for the launcher. Always set one when there is no label.",
          },
          badge: {
            type: ["number", "string"],
            description:
              "Count or dot shown on the launcher. Omit or 0 for none.",
          },
        },
      },
      width: {
