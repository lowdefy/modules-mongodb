...
function validateDictionaryIds(dataset) {
  ...
  for (const field of [...(dataset.dimensions ?? []), ...(dataset.measures ?? [])]) {
    if (!ID_REGEX.test(String(field.id))) {
      fail(`dataset "${dataset.id}" field id "${field.id}" is not a valid identifier.`);
    }
  }
}
...
function validateQuerySpec({ spec, datasets, roles }) {
  ...
  if (datasetRoles.length > 0 && !datasetRoles.some((r) => userRoles.includes(r))) {
    fail(`you are not authorized to query dataset "${dataset.id}".`);
  }
  if (typeof dataset.source?.collection !== "string" || dataset.source.collection === "") {
  ...
  if (select.length > MAX_SELECT) fail(`select allows at most ${MAX_SELECT} dimensions.`);
  const seenSelect = new Set();
  for (const dimId of select) {
    if (!dimensionsById.has(dimId)) {
      fail(
  ...
  if (!Array.isArray(rawMeasures)) fail("measures must be an array of { id, agg }.");
  if (rawMeasures.length > MAX_MEASURES) fail(`measures allows at most ${MAX_MEASURES} entries.`);
  const measures = [];
  const seenMeasureKeys = new Set();
  for (const entry of rawMeasures) {
    if (!entry || typeof entry !== "object") fail("each measure must be { id, agg }.");
    ...
    if (!AGGREGATIONS.includes(agg)) {
      fail(`measure "${entry.id}" aggregation "${agg}" is not one of ${AGGREGATIONS.join(", ")}.`);
    }
    const allowed = measure.type === "count" ? ["count"] : (measure.aggregations ?? []);
  ...
  const rawFilters = (spec.filters ?? []).filter(
    (f) => !(f && typeof f === "object" && (f.value === null || f.value === undefined))
  );
  if (!Array.isArray(spec.filters ?? [])) fail("filters must be an array.");
  if (rawFilters.length > MAX_FILTERS) fail(`filters allows at most ${MAX_FILTERS} entries.`);
  const filters = [];
  for (const filter of rawFilters) {
    if (!filter || typeof filter !== "object") fail("each filter must be { field, op, value }.");
    ...
    const fieldType = dimension?.type ?? (measure?.type === "number" ? "number" : undefined);
    if (!fieldType) {
      fail(`filter field "${filter.field}" is not a filterable field on dataset "${dataset.id}".`);
    }
    ...
    if (filter.op === "in" || filter.op === "nin") {
      if (!Array.isArray(filter.value)) fail(`filter op "${filter.op}" requires an array value.`);
      if (filter.value.length > MAX_IN_VALUES) {
        fail(`filter op "${filter.op}" allows at most ${MAX_IN_VALUES} values.`);
      }
      for (const v of filter.value) {
        if (!isScalarOfType(v, fieldType)) {
          fail(`filter on "${filter.field}" has a value that is not a valid ${fieldType}.`);
        }
      }
    } else if (!isScalarOfType(filter.value, fieldType)) {
      fail(`filter on "${filter.field}" value must be a ${fieldType}.`);
    }
    filters.push({ field: filter.field, op: filter.op, value: filter.value, type: fieldType });
  }

  // ── sort ──
  const rawSort = spec.sort ?? [];
  if (!Array.isArray(rawSort)) fail("sort must be an array of { field, dir }.");
  if (rawSort.length > MAX_SORT) fail(`sort allows at most ${MAX_SORT} entries.`);
  const outputColumns = new Set([...select, ...measures.map((m) => m.key)]);
  const sort = [];
  for (const entry of rawSort) {
    if (!entry || typeof entry !== "object") fail("each sort entry must be { field, dir }.");
    ...
    if (dir !== "asc" && dir !== "desc") fail(`sort dir "${dir}" must be "asc" or "desc".`);
  ...
  return { dataset, select, measures, filters, sort, limit };
}
