      ...
      actions: [
        {
          ...
          access: {
            'test-app': { view: true, edit: ['account-manager'], review: ['reviewer'] },
          },
        },
      ],
    },
...
async function seed({ workflowStage = 'active', actionStage = 'done', action = {} } = {}) {
...
test('writes assignees / due_date / description; omitted keys preserved', async () => {
  ...
  const result = await UpdateActionFields(
    buildContext({
      request: {
        action_id: 'A1',
        fields: { assignees: ['u-7'], description: { text: 'new', html: '<p>new</p>' } },
      },
    }),
  );
...
test('re-renders the status-map cell against the new field values', async () => {
  await seed();
  await UpdateActionFields(
    buildContext({ request: { action_id: 'A1', fields: { assignees: ['u-7'] } } }),
  );
  const doc = await mongo.db.collection('actions').findOne({ _id: 'A1' });
...
test('dispatches an action-fields-updated event with no metadata.comment', async () => {
  ...
  const ev = await mongo.db.collection('events').findOne({ _id: result.event_id });
