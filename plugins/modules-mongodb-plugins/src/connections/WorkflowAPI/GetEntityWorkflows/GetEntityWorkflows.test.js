  ...
  return [
    {
      ...
      actions: [
        ...
        },
        {
          ...
          kind: 'check',
          action_group: 'phase-1',
          access: { 'test-app': { view: false, edit: false, review: ['reviewer'] } },
        },
      ],
    },
  ];
...
async function seedWorkflow({ _id = 'wf-1', workflow_type = 'onboarding', entity_id = 'lead-1', entity_collection = 'leads-collection', overrides = {} } = {}) {
...
describe('GetEntityWorkflows return shape', () => {
  test('returns { workflows: [] } when no workflows match', async () => {
    const result = await GetEntityWorkflows(
      buildContext({ request: { entity_collection: 'leads-collection', entity_id: 'no-such' } }),
    );
...
describe('action cards include _id and kind', () => {
  test('each action card has _id, kind, type, status, allowed, message, link', async () => {
    ...
    expect(card.allowed).toEqual({ view: true, edit: true, review: false, error: false });
...
describe('access drop', () => {
  test('actions with no accessible verb are dropped', async () => {
    ...
    await seedAction({
      ...
      extra: {
        access: { 'test-app': { view: false, edit: false, review: ['reviewer'] } },
        'test-app': { links: { view: null, edit: null, review: null, error: null }, message: 'review msg' },
      },
    });
    const result = await GetEntityWorkflows(
      buildContext({ request: { entity_collection: 'leads-collection', entity_id: 'lead-1' } }),
...
describe('handler meta', () => {
  test('has schema and meta with both check flags false', () => {
    ...
    expect(GetEntityWorkflows.meta).toEqual({ checkRead: false, checkWrite: false });
  });
});
