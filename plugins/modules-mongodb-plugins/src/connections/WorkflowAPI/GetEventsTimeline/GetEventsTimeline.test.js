...
async function seedAction({
  ...
  const defaultAccess = { [app_name]: { view: true, edit: ['account-manager'] } };
...
describe('basic return shape', () => {
  test('returns an empty array when no matching events', async () => {
    const result = await GetEventsTimeline(
      buildContext({ request: { reference_field: 'lot_ids', reference_value: 'lot-999' } }),
    );
  ...
  test('schema and meta are set correctly', () => {
    ...
    expect(GetEventsTimeline.meta).toEqual({ checkRead: false, checkWrite: false });
  });
});
...
describe('workflow card access drop', () => {
  ...
  test('drops card if user has only reviewer role but action requires admin', async () => {
    await seedAction({
      _id: 'a-admin',
      stage: 'action-required',
      access: { 'test-app': { view: ['admin'], edit: ['admin'], review: ['admin'], error: ['admin'] } },
    });
...
describe('resolved link on workflow cards', () => {
  test('edit link is collapsed when user has edit access and edit link is set', async () => {
    await seedAction({
      ...
      stage: 'action-required',
      links: {
        view: { pageId: 'workflows/action-view', urlQuery: { action_id: 'a-edit' } },
        edit: { pageId: 'workflows/action-edit', urlQuery: { action_id: 'a-edit' } },
        review: null,
        error: null,
      },
      access: { 'test-app': { view: true, edit: ['account-manager'] } },
  ...
  test('view link is used when edit link is null', async () => {
    await seedAction({
      ...
      stage: 'done',
      links: {
        view: { pageId: 'workflows/action-view', urlQuery: { action_id: 'a-view' } },
        edit: null,
        review: null,
        error: null,
      },
