 ...
 *
 * Resolves `params.hooks?.[params.signal]?.post` — signal-keyed. Any level
 * undefined → return null without calling callApi.
 *
...
async function invokePostHook(
  ...
  const hookId = params?.hooks?.[params?.signal]?.post;
  ...
  return callApi({ endpointId: hookId, payload });
}
