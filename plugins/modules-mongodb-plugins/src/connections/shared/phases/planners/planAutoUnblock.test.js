...
function action(type, stage, { _id = `${type}-id`, key = null, action_group = null } = {}) {
  return {
    _id,
    workflow_id: 'wf-1',
    type,
...
test('linear: a terminal type-dep unblocks the dependent with a fully composed transition', () => {
  ...
  expect(entry.doc.status[0]).toEqual({ stage: 'action-required', event_id, created: now });
...
test('chained unblocks: not-required terminals cascade across iterations', () => {
  ...
  const fired = run({
    actions: [action('a', 'done'), action('b', 'blocked'), action('c', 'blocked')],
    actionsConfig: [
      config('a'),
...
test('cycles do not deadlock: mutually blocked actions terminate with no fires', () => {
  const fired = run({
    ...
    actionsConfig: [config('a', { blocked_by: ['b'] }), config('b', { blocked_by: ['a'] })],
  });
  expect(fired).toEqual([]);
});
