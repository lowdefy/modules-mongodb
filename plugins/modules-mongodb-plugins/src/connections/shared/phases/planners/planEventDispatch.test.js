...
test("Submit: YAML override on display replaces default display for that app key", () => {
  const { doc } = dispatch({
    ...
    signal: "submit",
    yamlEventOverrides: {
      display: { demo: { title: "Custom {{ signal }} title" } },
    },
  });
...
test("StartWorkflow: yamlEventOverrides display override is applied", () => {
  const { doc } = dispatch({
    ...
    yamlEventOverrides: {
      display: {
        demo: { title: "Workflow kicked off by {{ user.profile.name }}" },
      },
    },
  });
...
test("CancelWorkflow: yamlEventOverrides display override is applied", () => {
  const { doc } = dispatch({
    ...
    allTouchedActionDocs: [],
    yamlEventOverrides: {
      display: { demo: { title: "Cancelled by {{ user.profile.name }}" } },
    },
  });
...
test("CloseWorkflow: yamlEventOverrides display override is applied", () => {
  const { doc } = dispatch({
    ...
    allTouchedActionDocs: [],
    yamlEventOverrides: {
      display: { demo: { title: "Closed: {{ workflow.workflow_type }}" } },
    },
  });
...
test("tracker-mirror: yamlEventOverrides display override wins; non-overridden keys fall through", () => {
  ...
  const { doc } = dispatch({
    ...
    status_after: "done",
    yamlEventOverrides: {
      display: { demo: { title: "Mirror: {{ status_after }}" } },
    },
  });
