...
test("callApi (step 3) is called after workflow and action writes commit", async () => {
  ...
  const callApi = jest
    .fn()
    .mockImplementation(async ({ endpointId, payload }) => {
      ...
      return endpointId === "events/new-event"
        ? { eventId: payload._id }
        : null;
    });
...
test("step-3 failure: recorded on dispatchErrors, step 4 skipped, step 5 still runs", async () => {
  ...
  const callApi = jest.fn().mockRejectedValueOnce(new Error("events down")); // step 3 fails — callApi throws
