...
test("resolves the none creation row for a pseudo-action (upsert spawn)", () => {
  ...
  expect(
    resolveSignal({ action: pseudo, signal: "activate", actionConfig: {} }),
  ).toBe("action-required");
  expect(
    resolveSignal({ action: pseudo, signal: "block", actionConfig: {} }),
  ).toBe("blocked");
  expect(
    resolveSignal({ action: pseudo, signal: "submit", actionConfig: {} }),
  ).toBeNull();
});
