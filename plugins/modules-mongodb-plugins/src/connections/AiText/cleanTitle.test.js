...
describe('cleanTitle', () => {
  ...
  test('strips wrapping quotes and trailing punctuation', () => {
    expect(cleanTitle('"Evidence for board charter"')).toBe('Evidence for board charter');
    expect(cleanTitle('“Outstanding onboarding items”')).toBe('Outstanding onboarding items');
    expect(cleanTitle('Outstanding onboarding items.')).toBe('Outstanding onboarding items');
    expect(cleanTitle('Payroll run schedule:')).toBe('Payroll run schedule');
  });

  test('keeps internal quotes and punctuation', () => {
    expect(cleanTitle(`Editing a contact's address`)).toBe(`Editing a contact's address`);
  });
