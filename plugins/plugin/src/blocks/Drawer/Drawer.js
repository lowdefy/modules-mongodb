...
const triggerSetOpen = ({ state, setOpen, methods, rename }) => {
  ...
  if (state) {
    methods.triggerEvent({
      name: get(rename, "events.onOpen", { default: "onOpen" }),
    });
  }
  methods.triggerEvent({
    name: get(rename, "events.onToggle", { default: "onToggle" }),
  });
  setOpen(state);
};
...
const DrawerBlock = ({
  ...
  useEffect(() => {
    ...
    methods.registerMethod(
      ...
      ({ open }) => triggerSetOpen({ state: !!open, setOpen, methods, rename }),
    );
  });
  return (
    <Drawer
      ...
      footerStyle={methods.makeCssClass(properties.footerStyle, true)}
      maskStyle={methods.makeCssClass(properties.maskStyle, true)}
      contentWrapperStyle={methods.makeCssClass(
        properties.contentWrapperStyle,
        true,
      )}
    >
